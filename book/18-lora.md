...
ได้ 0.942 ตัวเลขน้อยกว่าสิบเท่า ผลใกล้กันในระดับที่ใช้งานได้ นี่คือ LoRA ทั้งหมด และบน
model จริงอัตราส่วนนั้นคือความต่างระหว่างการ์ดที่เช่าได้กับการ์ดที่ไม่มีใครมี
...
อย่างในหน่วยความจำ ตัวเลขทั้งหมดของ model ตัวเลข gradient ของทุกตัว และสถานะของ
...
parameter ต้องการราวร้อยยี่สิบกิกะไบต์แค่จะก้าวได้หนึ่งก้าว การ์ดที่ใหญ่ที่สุดที่เช่าได้มี
แปดสิบ

## 2. การเปลี่ยนแปลงมักเรียบง่าย และนั่นคือช่องที่ LoRA เข้า
...
จึงเป็นศูนย์ และก้าวที่ศูนย์คือ model ที่ตรึงไว้เป๊ะๆ adapter (ตัวปรับ คือคู่ตารางบางที่ LoRA
เรียนรู้ แยกเก็บจาก model เดิม) มองไม่เห็นจนกว่าจะเริ่มฝึก `check.py` พิสูจน์ด้วยการฝึกศูนย์
ก้าวแล้วเทียบ ถ้าเริ่มทั้งสองตารางแบบสุ่ม ก้าวแรกจะเริ่มจาก model ที่ถูกรบกวนแล้ว และ
ความรู้เดิมบางส่วนหายไปก่อนที่จะได้เรียนอะไร
...
บนข้อความใหม่ มันผิด 1.924 บนของเก่า หลัง LoRA ผิด 2.466 ทั้งสองวิธีลืม และ LoRA ลืมมาก
กว่า
...
ไม่ใช่วิธี ทุกสายฝึกจริงผสมข้อมูลทั่วไปสัดส่วนหนึ่งกลับเข้าไปในข้อมูลเฉพาะทางเสมอ และบทที่
17 คือที่ที่สัดส่วนนั้นถูกตัดสิน
...
หลังฝึกเสร็จมีสองทางเลือก เก็บ adapter แยกไว้ แล้วตอนใช้งานคำนวณผลคูณเพิ่มทุกครั้ง หรือ
บวกผลคูณเข้าตารางเดิมครั้งเดียว ได้ตารางธรรมดาที่ไม่มีอะไรบอกว่าเคยมี adapter และการ
ให้บริการไม่ต้องจ่ายอะไรเพิ่ม
...
ใส่ chat template ให้แต่ละบรรทัด

สามค่าใน script ที่ควรเข้าใจ ไม่ใช่ลอก rank สิบหกคือค่าเริ่มต้นที่คนส่วนใหญ่ใช้ และ alpha
สองเท่าของ rank คือตัวคูณของผลคูณ ซึ่งเป็นธรรมเนียมมากกว่าหลักการ learning rate สองในหมื่น
คือร้อยเท่าของที่ fine-tune เต็มใช้ เพราะมีแค่ adapter ที่ขยับและมันเริ่มจากศูนย์ และรายการ
ตารางที่ได้ adapter รวม feed-forward ด้วย เพราะบทพื้นฐานที่ 6 บอกว่าสองในสามของ parameter
อยู่ตรงนั้น และงานวิจัยหลังบทความต้นฉบับพบว่าการใส่ adapter ตรงนั้นช่วยมากกว่าที่ใส่แค่
attention
...
- ตารางขึ้นเริ่มที่ศูนย์ ก้าวที่ศูนย์จึงคือ model เดิมเป๊ะ
