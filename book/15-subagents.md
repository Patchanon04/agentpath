...
บทนี้คือสองอย่างที่เพิ่มเข้ามาได้โดยไม่แตะ agent loop (วงรอบของเอเจนต์ คือลูปที่ถาม
model แล้วรัน tool สลับกันไปจนได้คำตอบ) เลย คือ agent (เอเจนต์ คือโปรแกรมที่ให้ model
ตัดสินใจแล้วรัน tool ให้) ที่มอบงานให้ agent อีกตัวได้ และกลไกที่รัน agent หลายตัวพร้อมกัน
แล้วรวมผลกลับมาอ่านรู้เรื่อง ทั้งสองอย่างมีกับดักของตัวเอง และกับดักทั้งสองเงียบแบบ
เดียวกับไฟล์ตอนเปิดบท

## 1. subagent ไม่ใช่กลไกใหม่ มันคือ tool ที่ข้างในมี agent

ชื่อของมันชวนให้คิดว่าเป็นความสามารถพิเศษของ framework ความจริงคือมันคือ tool (เครื่อง
มือ คือฟังก์ชันที่ model ขอให้เรารันได้) ธรรมดาตัวหนึ่ง ที่บังเอิญข้างในไปรัน agent อีกตัว
...
ครั้งแรกไว้ครบ ซึ่งพังสองทาง ทางแรกคือมันค่อยๆ สะสมความรกแบบเดียวกับที่ subagent มีไว้
เพื่อกำจัด ทางที่สองแย่กว่า คืองานที่สองจะเริ่มจากตรงไหนก็ตามที่งานแรกไปจบ ซึ่งอาจเป็น
สถานะที่ผิดสำหรับงานใหม่ทั้งหมด

การส่งฟังก์ชันเข้าไปทำให้ทุกการเรียกได้บทสนทนาที่ว่างเปล่าใหม่ทุกครั้ง และคนที่ประกอบ
ระบบยังคงควบคุมได้ว่าลูกจะได้ tool ชุดไหน system prompt อะไร และงบเท่าไหร่ เพราะทั้งหมด
นั้นอยู่ในฟังก์ชันที่เขาเขียนเอง
...
การแยก context คือเหตุผลทั้งหมดที่เราอยากได้ subagent และมันคือเหตุผลทั้งหมดที่แม่กับ
ลูกเห็นโลกคนละเวอร์ชัน
...
บทเรียน 20 ไม่ได้อธิบายเรื่องนี้เฉยๆ มันสาธิตให้ดู และการสาธิตนั้นรันทุกครั้งที่มีการ push
...
นั่นคือกรณีที่ thread (เธรด คือสายการทำงานคู่ขนานในโปรแกรมเดียว) จัดการได้ดี thread ที่
บล็อกอยู่กินแค่ stack กับที่หนึ่งช่องในตารางของ scheduler ส่วน GIL (Global Interpreter
Lock คือกุญแจที่ยอมให้ thread เดียวรัน Python ได้ในหนึ่งขณะ) ซึ่งเป็นสิ่งที่คนเตือนกันถูก
ต้องแล้ว จะถูกปล่อยระหว่างรอ I/O ดังนั้นสี่ thread ที่รอสี่ socket รอพร้อมกันได้จริง
...
async ก็ไม่ใช่คำตอบ ทั้งที่ async เกิดมาเพื่อเรื่องนี้ เหตุผลไม่ได้อยู่ที่เครื่อง มันอยู่ที่ผู้อ่าน
ทุก module ในโปรเจกต์นี้เป็นแบบ synchronous `providers` เรียก httpx แล้วบล็อก `tools`
เรียก subprocess แล้วบล็อก `agent` คือ for loop ธรรมดา การใส่ asyncio ไม่ใช่การเติมคำ
สำคัญหนึ่งคำ มันเปลี่ยนสีของทุกฟังก์ชันที่มันแตะ แล้วสีนั้นลามออกไปเรื่อยๆ จนถึง
`provider.stream` จนถึง client HTTP จนถึงทุก for ที่วนบน generator และ tool ที่ยังบล็อก
อยู่ก็ยังต้องมี thread pool อยู่ดี
...
อาจแปลว่างานกำลังคิดอยู่ คำตอบคือ sentinel (ตัวเฝ้า คือค่าพิเศษที่ใช้เป็นสัญญาณจบ) ซึ่ง
...
คำว่าเสมอในย่อหน้าที่แล้วคือทั้งหมดของความถูกต้อง ถ้ามีเส้นทางไหนที่งานจบโดยไม่ได้หย่อน
sentinel คนอ่านจะรอตัวที่ไม่มีวันมา แล้วโปรแกรมค้างถาวร นั่นคือเหตุผลที่มันอยู่ใน `finally`
และเป็นเหตุผลที่แม้แต่งานที่มีรูปร่างผิดตั้งแต่แรกก็ยังต้องหย่อน
...
บทเรียน 21 มีตัวอย่างที่รันจริง สองงาน แต่ละงานพ่นทีละตัวอักษร เขียนลงจอตรงๆ โดยไม่ติด
ป้าย
...
หกหัวข้อที่แล้วเป็นเรื่องการรวม output หัวข้อนี้เป็นเรื่อง state ที่ใช้ร่วมกัน และมันคือ
...
นี่คือ lost update (การอัปเดตที่หายไป คือกรณีที่การเขียนสองครั้งทับกัน) ซึ่งเป็นปัญหาเก่า
แก่หลายสิบปี และบทเรียน 21 ให้ดูของจริง สองงานแก้คนละบรรทัด ซึ่งเป็นกรณีที่ทุกคนคิดว่า
ปลอดภัย
...
ให้อยู่กับความจริงว่าการวินิจฉัยเรื่องนี้แย่แค่ไหน ไม่มี exception ไม่มีคำเตือน agent ทั้ง
สองตัวจะบอกคุณอย่างซื่อสัตย์ว่ามันแก้เสร็จแล้ว เพราะจากข้างในของแต่ละตัวนั่นคือสิ่งที่เกิด
ขึ้นจริง ไฟล์ session บันทึกการแก้ที่สำเร็จไว้สองครั้ง ทางเดียวที่จะรู้คือเปิดไฟล์ดูทีหลังแล้ว
สังเกตว่ามีอะไรหายไป และถ้าสิ่งที่หายคือบรรทัดที่คุณเพิ่ม ไม่ใช่บรรทัดที่คุณลบ คุณอาจไม่รู้
ตัวไปอีกเป็นสัปดาห์
...
เดียว ข้อควรรู้คือแค่นี้ยังไม่แก้ปัญหา มันทำให้การเขียนเป็นหน่วยเดียวจบ ซึ่งลำดับข้างบนก็
...
จะทำอะไรต่อ ซึ่งคำตอบคืออ่านใหม่แล้วทำงานนั้นใหม่

ตระกูลที่สองคือให้ worker แต่ละตัวมีสำเนาโฟลเดอร์งานของตัวเอง แล้วเอามารวมตอนจบเป็น
merge ซึ่งเป็นปัญหาที่มีเครื่องมือรองรับมาสี่สิบปี นี่คือสิ่งที่ harness ที่จริงจังทำ และเป็น
เหตุผลที่เครื่องมือ agent สมัยใหม่มีเรื่อง worktree เข้ามาเกี่ยวข้อง ราคาของมันคือการคัด
ลอกโฟลเดอร์ไม่ฟรี
