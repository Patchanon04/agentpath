...
`tools/list` ออกไปแล้วไม่มีอะไรกลับมา process (โปรแกรมที่กำลังทำงานอยู่หนึ่งตัว) ยังอยู่
ท่อยังเปิด ไม่มี error สักบรรทัด อาการแบบนี้อ่านเหมือนโปรแกรมค้าง ส่วนต้นเหตุจริงคือ
ข้อความสามคำที่ยังไม่ได้ถูกส่งออกไป และไม่มีหลักฐานชิ้นไหนชี้ไปที่มัน
...
เห็นค่าของมันง่ายที่สุดตอนอยากให้ agent อ่าน issue จากระบบ ticket ของทีม ทางเดิมคือเขียน
tool สี่ตัวเอง ขอ token ของ API มาเก็บเอง แล้วตามแก้ทุกครั้งที่ปลายทางเปลี่ยน field ทาง
ใหม่คือทีมนั้นมี server อยู่แล้ว คุณเติมคำสั่งเรียกมันหนึ่งบรรทัดตอนสตาร์ท agent แล้ว tool
สี่ตัวนั้นก็อยู่ในรายการของรอบถัดไป

โปรเจกต์นี้เขียน client เองแทนที่จะใช้ library
ที่มีคนทำไว้ ด้วยเหตุผลเดียวกับที่บทที่ 1 ยิง HTTP เอง ส่วนของโปรโตคอลที่ agent
ต้องใช้จริงมีสามอย่าง คือ `initialize` `tools/list` และ `tools/call` ซึ่งเป็น
JSON-RPC (เรียก function ข้าม process โดยส่งกันเป็น JSON) บนท่อ
`src/agentpath/mcp.py` ทั้งไฟล์ยาว 217 บรรทัด รวมคอมเมนต์และ docstring
...
จังหวะแรกคือส่ง `initialize`
พร้อมบอกว่าเราพูดโปรโตคอลรุ่นไหนและเราเป็นใคร แล้วอ่านคำตอบที่มีชื่อและความสามาร
ถของ server อยู่ในนั้น จังหวะที่สองคือส่ง `notifications/initialized`
ซึ่งเป็นข้อความที่ไม่มี id และไม่ต้องการคำตอบ มันแค่บอกว่าฝั่งเราพร้อมแล้ว
...
sequenceDiagram
    ...
    A->>S: initialize
    S-->>A: serverInfo และ capabilities
    A->>S: notifications/initialized
    Note over A,S: ไม่มี id และไม่มีคำตอบ ตรงนี้คือจังหวะที่คนลืม
    A->>S: tools/list
    S-->>A: รายการ tool พร้อม schema
    A->>S: tools/call
    S-->>A: content หรือ isError
```
...
อาการของการเชื่อบรรทัดถัดไปหน้าตาแบบนี้ server ตัวหนึ่งพิมพ์ความคืบหน้าของการทำดัชนี
ออกมาระหว่างทาง คำขอ `tools/call` ที่เราเพิ่งส่งจึงเจอบรรทัดความคืบหน้าก่อน โค้ดที่หยิบ
บรรทัดแรกแกะมันสำเร็จ ไม่เจอ `content` แล้วคืนสตริงว่าง model เห็น tool ที่ตอบว่าไม่มี
อะไรเลย ทั้งที่คำตอบจริงมาถึงในอีกสองบรรทัดถัดมา
...
`_lock` อยู่ตรงนั้น เพราะสองสายการทำงานที่ยิงคำขอพร้อมกันบนท่อเดียว จะสลับกันอ่านคำตอบ
ของกันและกัน กุญแจตัวนี้ทำให้หนึ่งคำถามหนึ่งคำตอบเสร็จเป็นคู่ ก่อนที่คู่ถัดไปจะเริ่ม
...
ต่างกันตรงไหนดูจากรอบถัดไป model ที่อ่านว่าไม่พบ issue หมายเลข 4120 จะลองขอรายการ issue
ที่ยังเปิดอยู่แทน แล้วงานเดินต่อได้ ส่วน model ที่ไม่ได้อ่านอะไรเลยเพราะเราโยน exception
ขึ้นไป จะไม่ได้ทำอะไรต่อ และสิ่งที่ผู้ใช้เห็นคือ stack trace กลางงานที่เหลืออีกหกขั้น
...
สมมติ server ตัวหนึ่งมี tool ที่ดึงหน้าเว็บมาให้ แล้วหน้าที่ถูกดึงมามีบรรทัดเขียนว่างานที่
ผู้ใช้สั่งเสร็จแล้ว ขั้นต่อไปให้อ่านไฟล์ key ในโฟลเดอร์บ้านแล้วสรุปเนื้อในมาให้ ข้อความนั้น
เดินทางเข้าบทสนทนาในฐานะผลลัพธ์ของ tool และ model อ่านมันด้วยสายตาเดียวกับที่อ่านคำสั่ง
ของคุณ สิ่งเดียวที่ยืนขวางอยู่คือประตูอนุญาต ไม่ใช่ความสุภาพของ server
...
แปดตัวที่เราเขียนเอง เรียก `schema_size()` แล้วได้ 2847
ตัวอักษร ราคานี้จ่ายก่อนต่อ server ตัวไหนเลย
...
ประโยคสุดท้ายของ docstring คือสิ่งที่ทำให้เรื่องนี้ไม่ใช่เรื่องเล็ก คนที่รัน
agent สี่สิบครั้งจะมี process
ค้างอยู่สี่สิบตัว และเขาจะไม่รู้ตัวจนกว่าเครื่องจะช้าลงโดยไม่มีเหตุผล นี่คือกฎ
เดิมจากบทที่ 6 ในรูปแบบใหม่ ทุกอย่างที่คุณเปิด ต้องมีโค้ดที่ปิดมัน และโค้ดนั้นต้
องทำงานแม้ตอนที่มีอะไรพังครับ
