...
คุณส่ง `tools/list` ไปแล้วไม่มีอะไรกลับมา process ยังอยู่ ท่อยังเปิด ไม่มี error สักบรรทัด
คุณไล่ดู buffer ไล่ดูว่าท่อตันไหม ไล่ดูว่า server ตายหรือเปล่า สาเหตุคือข้อความสามคำที่
คุณไม่ได้ส่ง และไม่มีอะไรในโลกชี้ไปที่มัน

บทนี้คือ client (ไคลเอนต์ คือฝั่งที่เป็นคนเรียก) ที่ต่อกับ server ตัวไหนก็ได้ที่พูด
โปรโตคอลนี้ แล้วเอา tool (เครื่องมือ คือฟังก์ชันที่ model ขอให้เรารันได้) ของเขามาใส่
agent (เอเจนต์ คือโปรแกรมที่ให้ model ตัดสินใจแล้วรัน tool ให้) ของคุณโดยไม่ต้องแก้
...
MCP (Model Context Protocol คือโปรโตคอลสำหรับให้ agent ใช้ tool ที่คนอื่นเขียน) คือข้อ
ตกลงว่าโปรแกรมสองตัวจะคุยกันยังไงเรื่อง tool ฝั่งที่ให้บริการเรียกว่า server และมันรัน
เป็นคนละ process (โพรเซส คือโปรแกรมที่กำลังทำงานอยู่หนึ่งตัว) กับ agent ของคุณ server
บอกได้ว่าตัวเองทำอะไรได้บ้าง และทำให้เมื่อถูกขอ นั่นคือทั้งหมด ที่มันสำคัญเพราะมันเปลี่ยน
server ทุกตัวในโลกให้กลายเป็น tool ที่ agent ของคุณเรียกได้ โดยที่คุณไม่ต้องเขียนสักตัว
...
`tools/list` และ `tools/call` ซึ่งเป็น JSON-RPC (เจสันอาร์พีซี คือรูปแบบการเรียกฟังก์ชัน
ข้ามโปรแกรมด้วย JSON) บนท่อ `src/agentpath/mcp.py` ทั้งไฟล์ยาว 217 บรรทัด รวมคอมเมนต์
และ docstring
...
จังหวะแรกคือส่ง `initialize` พร้อมบอกว่าเราพูดโปรโตคอลรุ่นไหนและเราเป็นใคร แล้วอ่านคำ
ตอบ ซึ่งมีชื่อและความสามารถของ server อยู่ในนั้น จังหวะที่สองคือส่ง
...
ข้อความนั้นไม่มี id เพราะใน JSON-RPC ข้อความที่ไม่มี id คือ notification (การแจ้ง คือข้อ
ความที่ไม่ต้องการคำตอบ) การใส่ id ให้มันแปลว่าเรากำลังรอคำตอบที่จะไม่มีวันมา ซึ่งเป็นวิธี
ทำให้ค้างอีกวิธีหนึ่ง
...
ส่วน server ที่พัง คือปัญหาของโปรแกรมเรา ไม่ใช่ของ model
...
แปลว่า schema (สคีมา คือคำอธิบายรูปร่างของข้อมูลในรูป JSON) ของ tool ทุกตัวคือค่าใช้จ่าย
...
ตัวเลขแรกมาจาก `build_tools` ใน `src/agentpath/cli.py` ซึ่งคือ tool แปดตัวที่เราเขียน
เองทั้งหมด เรียก `schema_size()` แล้วได้ 2847 ตัวอักษร นั่นคือราคาก่อนต่อ server ตัวไหน
เลย

ตัวเลขที่สองมาจากบทเรียน 19 ซึ่ง `check.py` วัดก่อนและหลังต่อ server ทดสอบตัวเล็กที่สุด
...
เลขตั้งต้นสองอันไม่เท่ากัน ทั้งที่เป็น tool แปดตัวเดียวกัน เพราะบทเรียน 19 เก็บ schema ใน
...
กินตัวละราวสามสิบสองตัวอักษร คูณแปดตัวได้ 254 ซึ่งคือระยะห่างระหว่าง 2847 กับ 3101 พอดี
