...
คือเหตุผลทั้งหมดที่บทที่ 12 ของหนังสือมี interface กลางหนึ่งตัวและ provider สองแบบ
ข้างหลังมันครับ

### 7.1 hallucination ไม่ใช่บั๊ก มันคือกลไกเดียวกับที่ทำให้ model ตอบได้
...
(อาการหลอน คือการที่ model สร้างข้อความที่อ่านเหมือนข้อเท็จจริงแต่ไม่จริง) และคนมักคิด
...
ไหนน่าจะมาต่อ กลไกเดียวกับที่ทำให้มันตอบคำถามที่ไม่เคยเห็นเป๊ะๆ ได้ คือกลไกที่ทำให้มัน
ตอบคำถามที่ไม่มีคำตอบได้ด้วย มันไม่แยกสองกรณีนี้ออกจากกัน เพราะสำหรับมันสองกรณีนี้คือ
กรณีเดียว
...
### 7.2 ทำไมใหญ่กว่าถึงดีกว่า และดีกว่าเท่าไหร่

ตัวเลขที่บทที่ 4 เรียกว่า parameter ใน model ที่คุณเรียกมีหลักหมื่นล้านถึงล้านล้าน และ
มีเหตุผลที่มันโตขึ้นเรื่อยๆ ราวปี 2020 มีการทดลองที่วัด loss ของ model หลายขนาดที่ฝึกบน
ข้อความหลายปริมาณ แล้วพบว่า loss ลดลงอย่างคาดการณ์ได้ตามขนาด ตามปริมาณข้อความ
และตามปริมาณการคำนวณ เป็นเส้นตรงบนกราฟ log ไม่มีจุดที่มันหยุด ผลนี้ชื่อ scaling law
(กฎการขยายขนาด คือความสัมพันธ์ที่วัดได้ระหว่างขนาดของ model ปริมาณข้อมูล และความ
ผิดพลาด) และมันคือเหตุผลที่บริษัทยอมจ่ายเงินมหาศาลเพื่อ model ที่ใหญ่กว่าเดิม เพราะ
กราฟบอกล่วงหน้าว่าจะได้อะไร

สองอย่างที่ควรรู้จากกฎนี้ อย่างแรกคือมันวัด loss ซึ่งคือ perplexity ของบทที่ 4 ไม่ได้วัด
ว่า model แก้โจทย์ของคุณได้ไหม model ที่ทายคำถัดไปเก่งขึ้นสิบเปอร์เซ็นต์อาจแก้ bug ได้
เท่าเดิม อย่างที่สองคือขนาดกับข้อมูลต้องโตไปด้วยกัน model ใหญ่ที่ฝึกบนข้อความน้อยเกิน
ไปแพ้ model เล็กที่ฝึกบนข้อความมากกว่า ซึ่งเป็นเหตุผลที่ model ขนาดกลางรุ่นใหม่ชนะ
model ขนาดใหญ่รุ่นเก่าอยู่เรื่อยๆ

### 7.3 เมื่อไหร่ควร fine-tune และทำไมคำตอบมักเป็นยังไม่ถึงเวลา

การฝึกรอบที่สองในหัวข้อที่ 1 ทำเองได้ เอา model ที่ฝึกมาแล้ว ป้อนตัวอย่างคู่คำถามคำตอบ
ของงานคุณ แล้วก้าวลงเขาต่ออีกหน่อย ชื่อ fine-tuning (การปรับจูน คือการฝึก model ที่ฝึก
มาแล้วต่อบนข้อมูลของงานเฉพาะ) และเป็นสิ่งแรกที่คนอยากทำเมื่อ model ตอบไม่ถูกใจ

มันควรเป็นสิ่งสุดท้าย ด้วยเหตุผลสามข้อที่เรียงตามลำดับ ข้อแรกคือปัญหาส่วนใหญ่ไม่ใช่
ปัญหาที่ fine-tuning แก้ model ที่ไม่รู้ข้อมูลของบริษัทคุณไม่ได้ต้องการการฝึก มันต้องการ
ให้ข้อมูลนั้นอยู่ใน context ซึ่งคือ retrieval และ fine-tuning ใส่ข้อเท็จจริงเข้าไปใน model
ได้แย่มาก มันเก่งเรื่องรูปแบบ น้ำเสียง และรูปร่างของคำตอบ ไม่เก่งเรื่องเนื้อหา ข้อสองคือ
ราคา คุณต้องมีตัวอย่างหลายร้อยถึงหลายพันคู่ที่คนตรวจแล้วว่าดี ต้องมีเครื่อง ต้องมีวิธีวัด
ว่าดีขึ้นจริง และทุกครั้งที่ model รุ่นใหม่ออก คุณต้องทำใหม่ ข้อสามคือ prompt กับ
retrieval แก้ได้ในบ่ายเดียวและย้อนกลับได้ในบ่ายเดียว fine-tuning ไม่ใช่ทั้งสองอย่าง
...
รับไม่ได้ และงานที่ model เล็กที่ fine-tune แล้วถูกกว่า model ใหญ่ที่ prompt ยาวมาก ภาค
ท้ายของหนังสือว่าด้วยการทำมันจริง เมื่อถึงเวลานั้นครับ
