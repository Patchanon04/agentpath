...
parameter อีกตัว แปลงมันกลับมาอยู่ในพื้นที่เดียวกับ token ตั้งต้น แล้วบวก token ตั้งต้นก
ลับเข้าไป การบวกนั้นชื่อ residual connection (เอา input ของชั้นมาบวกกับ output ของมันเอ
ง) และมันคือเหตุผลที่ซ้อนหลายสิบชั้นแล้วยังฝึกได้ แต่ละชั้นปรับ token แทนที่จะแทนที่มั
น ชั้นที่ตารางยังใกล้ศูนย์ส่ง input ผ่านไปเกือบไม่เปลี่ยน แทนที่จะทำลายมัน และ gradient จ
ากบทที่ 4 มีทางเดินตรงย้อนกลับผ่านการบวกนั้นไปถึงชั้นล่างๆ ได้ `check.py` ยืนยันทั้งสองอ
ย่าง ตารางศูนย์ให้ token เดิมกลับมาทุกตัวเลข และตารางที่ไม่ใช่ศูนย์ให้ token ที่ถูกปรั
บ ไม่ใช่ถูกแทนที่
...
softmax ของบทที่ 4 นั่นเองครับ

## 7. KV cache และเหตุผลที่ token ขาออกแพงกว่าขาเข้า

model สร้างข้อความทีละ token ตามบทที่ 3 ทุกก้าวคือการรัน block
