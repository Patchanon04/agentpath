...
ของบทพื้นฐานที่ 4 และสิ่งที่มันเรียนคือ token ไหนควรมองที่ token ไหนเพื่อให้ทายคำถัดไปได้
แม่นขึ้น
...
model ในบทพื้นฐานที่ 3 มองย้อนหลังได้หนึ่งหรือสองคำ ตายตัว คำที่สามข้างหลังไม่มีอยู่สำหรับ
...
สิ่งที่ต้องการคือกลไกที่ให้แต่ละคำเลือกเองว่าจะมองคำไหน ไม่ว่าอยู่ห่างแค่ไหน และมองมากน้อ
ยต่างกันได้ นั่นคือ attention (กลไกที่ให้แต่ละ token เลือกว่าจะดึงข้อมูลจาก token ไหนมาก
แค่ไหน)
...
กับบทพื้นฐานที่ 4 บรรทัดสุดท้ายเอาน้ำหนักไปผสม value ของทุก token ได้ token ใหม่ที่เป็น
...
ตารางสามตาราง w_query w_key w_value คือ parameter ในความหมายของบทพื้นฐานที่ 4 ไฟล์นี้
...
หน้า จึงต้องปิดตาครึ่งหนึ่ง mask (ตารางที่บวกเข้ากับคะแนนเพื่อปิดบางช่อง)
...
ที่เห็นบ่อยตอนฝึกเองคือลืมใส่ mask แล้ว loss
ลงเร็วผิดปกติจนดูเหมือนสำเร็จตั้งแต่รอบแรกๆ สิ่งที่ model
เรียนไม่ใช่ภาษา มันเรียนว่าคำตอบอยู่ในช่องถัดไปทางขวาให้ลอก พอเอาไปใช้จริงที่ไม่
มีช่องถัดไปให้ดู มันเขียนอะไรไม่ได้เลย
...
เท่า และเป็นเหตุผลที่บทที่ 4 ของหนังสือทั้งบทว่าด้วยการตัดสินใจว่าอะไรควรอยู่ใน context และ
อะไรควรออก

ที่เห็นเป็นอาการคือทีมที่ยัด log ทั้งไฟล์ลง context เพื่อให้ model หาบั๊ก พอไฟล์โต
จากสองหมื่น token เป็นสี่หมื่น เวลารอไม่ได้เพิ่มเท่าตัว มันเพิ่มราวสี่เท่า แล้วคน
ใช้จะรายงานว่าระบบช้าลงเฉยๆ ทั้งที่ไม่มีใครแก้โค้ดสักบรรทัด
...
bag of words ในบทพื้นฐานที่ 5 the cat sat down กับ down sat cat the คือประโยคเดียวกันสำหรับ
...
positional encoding (vector ที่บวกเข้าไปใน embedding เพื่อบอกว่า token อยู่ตำแหน่งไหน) ตั
วนี้คือรูปที่บทความปี 2017 ใช้ ตารางของ sine กับ cosine ที่ความถี่ต่างกัน ตำแหน่งแต่ละตำ
แหน่งได้ลายมือของตัวเอง และตำแหน่งที่ใกล้กันได้ลายมือที่คล้ายกัน vector ของ token กลายเป็
น สิ่งที่มันเป็น บวก ที่ที่มันอยู่ แล้ว attention ที่เดิมแยกลำดับไม่ออก ก็แยกออก
...
position embedding คือใส่ตำแหน่งด้วยการหมุน vector แทนการบวก) ซึ่งทำให้คะแนนระหว่างสอง
token ขึ้นกับระยะห่างของมัน ไม่ใช่ตำแหน่งสัมบูรณ์ ทุกรูปแบบตอบคำถามเดียวกัน attention ไม่
ถือลำดับ input จึงต้องถือแทน และนี่คือหนึ่งในเหตุผลที่ model มีความยาว context สูงสุด เพร
าะตำแหน่งที่ไม่เคยเห็นตอนฝึกคือตำแหน่งที่ model ไม่รู้ว่าจะอ่านยังไงครับ

## 6. จากหนึ่งหัวเป็น block และจาก block เป็น transformer

สิ่งที่หัวข้อที่ 2 มีคือ attention หนึ่งหัว transformer (model ที่เอา attention มาซ้อนกัน
หลายชั้น) คือหัวแบบนั้นหลายหัวในหนึ่งชั้น หลายชั้นซ้อนกัน และระหว่างหัวกับชั้นถัดไปมีอีก
สี่ชิ้น ที่แต่ละชิ้นเล็กพอจะเขียนได้ในบรรทัดเดียว
...
ผลของหัวคือ value ที่ผสมแล้ว และมันอยู่ในพื้นที่ของ value ตาราง `w_out`
ซึ่งเป็น parameter อีกตัว แปลงมันกลับมาอยู่ในพื้นที่เดียวกับ token
ตั้งต้น แล้วบวก token ตั้งต้นกลับเข้าไป การบวกนั้นชื่อ residual connection (เอา
input ของชั้นมาบวกกับ output ของมันเอง)
และมันคือเหตุผลที่ซ้อนหลายสิบชั้นแล้วยังฝึกได้ แต่ละชั้นปรับ token
แทนที่จะแทนที่มัน ชั้นที่ตารางยังใกล้ศูนย์ส่ง input
ผ่านไปเกือบไม่เปลี่ยน แทนที่จะทำลายมัน และ gradient จากบทพื้นฐานที่ 4
มีทางเดินตรงย้อนกลับผ่านการบวกนั้นไปถึงชั้นล่างๆ ได้ `check.py`
ยืนยันทั้งสองอย่าง ตารางศูนย์ให้ token
เดิมกลับมาทุกตัวเลข และตารางที่ไม่ใช่ศูนย์ให้ token ที่ถูกปรับ ไม่ใช่ถูกแทนที่

ก่อนจะมีการบวกนั้น network ที่ลึกเกินราวยี่สิบชั้นฝึกไม่ขึ้น gradient ที่ต้องคูณ
ย้อนผ่านทุกชั้นจะเล็กลงทีละชั้น พอถึงชั้นล่างสุดมันเหลือแทบเป็นศูนย์ ชั้นล่างจึงไม่
ขยับเลยตลอดการฝึก และ model ที่ลึกกว่ากลับให้ผลแย่กว่า model ที่ตื้นกว่า

ชิ้นที่สองคือหลายหัว หนึ่งหัวมีตารางสามตาราง จึงเรียนรู้รูปแบบการมองได้แบบเดียว กริยามองป
ระธานคือหนึ่งแบบ วงเล็บปิดมองวงเล็บเปิดคืออีกแบบ สรรพนามมองชื่อที่มันแทนคือแบบที่สาม
multi-head attention (รัน attention หลายชุดพร้อมกัน แต่ละชุดมีตารางของตัวเอง) รันหลายหัวบ
น token ชุดเดียวกัน ต่อ output ของทุกหัวเรียงกัน แล้วให้ตารางเดียวผสมกลับมาเป็นขนาดของ
token หนึ่งชั้นจึงถือหลายรูปแบบพร้อมกันได้ แค่นั้น

ลองนึกถึงหนึ่งบรรทัดในโค้ดที่มีวงเล็บซ้อนกันสามชั้น และตัวแปรที่ถูกประกาศไว้ตั้งแต่
บรรทัดบน หัวเดียวต้องเลือกว่าจะตามวงเล็บหรือจะตามตัวแปร สิบหกหัวไม่ต้องเลือก แต่ละ
หัวมองสิ่งที่มันถนัด แล้ว output ของทุกหัวถูกต่อเรียงกันก่อนส่งให้ชั้นถัดไป
...
attention คือส่วนที่ token คุยกัน feed-forward (ชั้นที่แปลง vector ของแต่ละ token แยกกั
น ไม่มอง token อื่นเลย) คือส่วนที่แต่ละ token คิดคนเดียวกับสิ่งที่มันเพิ่งรวบรวมมา ดันผ่า
นตารางที่กว้างกว่า ตัดค่าลบทิ้งเป็นศูนย์ แล้วดึงกลับมาขนาดเดิมไม่มี token ไหนเห็น token
อื่นตรงนี้ `check.py` ยืนยันด้วยการเปลี่ยน token ตัวเดียวแล้วดูว่า output เปลี่ยนแค่แถว
เดียว ประมาณสองในสามของ parameter ทั้งหมดของ model จริงอยู่ในตารางสองตารางนี้ และงานวิจัย
ที่ตามหาว่าข้อเท็จจริงถูกเก็บไว้ตรงไหนใน model ชี้มาที่นี่

ชิ้นที่สี่คือ layer norm (ปรับ vector ของแต่ละ token ให้ค่าเฉลี่ยเป็นศูนย์และการกระจายเป็
นหนึ่ง) ที่อยู่หน้าแต่ละครึ่ง แล้วทั้งหมดประกอบเป็น block เดียว
...
คุยกัน แล้วคิด แต่ละครึ่งบวกเข้ากับ token แทนที่จะแทนที่ และแต่ละครึ่งมี layer norm
อยู่หน้ามัน ของจริงมีตัวคูณที่เรียนรู้ต่อท้าย และ model รุ่นหลังใช้รูปที่ตัดค่าเฉลี่ย
ทิ้งชื่อ RMSNorm ไฟล์นี้ข้ามตัวคูณนั้นเพื่อให้สั้น ถ้าไม่มี layer norm เลย หลายสิบชั้นที่บวก
เข้าไปเรื่อยๆ จะปล่อยให้ตัวเลขโตเท่าไหร่ก็ได้ และ softmax
บนคะแนนที่ใหญ่มากกลายเป็นการเลือก token เดียวแข็งๆ
...
เส้นที่อ้อมกล่องไปหาเครื่องหมายบวกคือ residual connection ทั้งสองเส้น output
ของ block มีรูปร่างเดียวกับ input และนั่นคือสิ่งที่ทำให้ซ้อนได้ block
ถัดไปเริ่มจาก token ที่ถือสิ่งที่ block นี้รวบรวมมาแล้ว และ model
ที่คุณเรียกคือ block นี้ซ้อนกันหลายสิบชั้น ตัวเลขที่คนพูดถึง เช่นเจ็ดหมื่นล้าน
parameter คือขนาดรวมของตารางทุกตารางในทุก block บวกตาราง embedding
ที่ทางเข้า และตารางที่แปลง token สุดท้ายเป็นคะแนนของคำถัดไปที่ทางออก ซึ่งคือ
softmax ของบทพื้นฐานที่ 4 นั่นเองครับ

## 7. KV cache และเหตุผลที่ token ขาออกแพงกว่าขาเข้า

model สร้างข้อความทีละ token ตามบทพื้นฐานที่ 3 ทุกก้าวคือการรัน block
ทั้งหมดบนลำดับที่ยาวขึ้นหนึ่ง token ถ้าทำแบบตรงไปตรงมา ก้าวที่ร้อยคำนวณ key กับ
value ของ token ทั้งร้อยใหม่ ทั้งที่เก้าสิบเก้าตัวแรกไม่ได้เปลี่ยน key กับ
value ของ token เก่าไม่เปลี่ยนเมื่อมี token ใหม่ต่อท้าย เพราะ mask ในหัวข้อที่
3 ห้ามมันมองไปข้างหน้า สิ่งที่มาทีหลังจึงแตะมันไม่ได้
...
query ของตัวเองกับ key ทุกตัวในรายการ นี่คือ KV cache (เก็บ key กับ value ของ token ก่อนห
น้าไว้ ไม่ต้องคำนวณใหม่ทุกก้าว) `check.py` ยืนยันว่าป้อน token ผ่านมันทีละตัว ได้แถวเดียว
กันทุกตัวเลขกับ masked attention ในหัวข้อที่ 3 ที่ทำทั้งหมดในครั้งเดียว
...
บทความปี 2017 มี transformer สองครึ่ง encoder (กอง block ที่อ่านทั้งประโยคทีเดียว ทุก
token มองเห็นกันหมด) อ่าน input ทั้งหมดโดยไม่มี mask ทุก token เห็นทั้งซ้ายและขวา
decoder (กอง block ที่เขียนทีละ token มองได้แค่สิ่งที่มาก่อน) คือหัวข้อที่ 3 ของบทนี้ มี
mask ปิดอนาคต และเขียนทีละ token งานต้นฉบับคือแปลภาษา encoder อ่านประโยคต้นทาง decoder
เขียนประโยคปลายทาง
...
embedding model ที่บทเรียนที่ 16 เรียกใช้ ตระกูล GPT เอาแค่ decoder ได้ model ที่
...
ที่ยังเห็น encoder ทำงานอยู่คืองานที่ไม่ต้องเขียนอะไรออกมาเลย จัดตั๋วสนับสนุนลูกค้า
เข้าสิบหมวด หรือหาชื่อบริษัทในสัญญาห้าร้อยฉบับ model ขนาดร้อยล้าน parameter ทำสอง
งานนี้ได้เร็วกว่าและถูกกว่า model ที่ต้องเขียนคำตอบเป็นประโยคหลายเท่า
...
ทั้งหมดอยู่ใน [foundations/06-attention/attention.py](../foundations/06-attention/attention.py) รัน `python attention.py` เพื่อดูตารางว่าใครมองใคร จำนวนคะแนนที่ context สามขนาดต้องการ การทดลองกลับลำดับ token ก่อนและหลังใส่ตำแหน่ง และจำนวนแถวที่ cache ประหยัดได้ แล้วรัน `python check.py` เพื่อยืนยันทุกข้ออ้าง ลองแก้ `hand_built_grids` ให้ down มองที่ sat แล้วดูว่าตารางเปลี่ยนไปยังไง แล้วลองเปลี่ยน `size` ใน `position_vectors` ให้เล็กกว่าจำนวน token แล้วดูว่าตำแหน่งไหนเริ่มแยกกันไม่ออก
