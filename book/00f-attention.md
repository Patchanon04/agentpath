...
เป็นหนึ่ง แล้วคูณกลับด้วยตัวเลขที่เรียนรู้ได้) อยู่หน้ามัน ของจริงมีตัวคูณที่เรียนรู้ต่อท้าย และ
model รุ่นหลังใช้รูปที่ตัดค่าเฉลี่ยทิ้งชื่อ RMSNorm ไฟล์นี้ตัดทั้งสองอย่างออก ถ้าไม่มีมัน หลายสิบชั้นที่บวกเข้าไปเรื่อยๆ จะปล่อยให้ตัวเลขโตเท่าไหร่ก็ได้
...
caching ที่บทเรียนที่ 15 พูดถึง คือผู้ให้บริการเก็บ cache นี้ของ system prompt
...
- KV cache เก็บ key กับ value ของ token เก่าไว้ จึงไม่ต้องคำนวณซ้ำ งานลดลงราวครึ่งหนึ่ง
  ของความยาวเท่า token ขาออกแพงกว่าขาเข้าเพราะต้องผลิตทีละตัว ส่วนขาเข้าคำนวณพร้อมกัน
  ได้ในรอบเดียว
