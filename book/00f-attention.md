...
ซ้อนกัน และระหว่างหัวกับชั้นถัดไปมีอีกสามชิ้นที่แต่ละชิ้นเล็กพอจะเขียนได้ในบรรทัดเดียว
...
กลับเข้าไป การบวกนั้นชื่อ residual connection (การเชื่อมส่วนที่เหลือ คือการเอา input
ของชั้นมาบวกกับ output ของชั้น) และมันคือเหตุผลที่ซ้อนหลายสิบชั้นแล้วยังฝึกได้ แต่ละ
ชั้นปรับ token แทนที่จะแทนที่มัน ชั้นที่ยังไม่ได้เรียนรู้อะไรส่ง input ผ่านไปโดยไม่ทำ
ลาย และ gradient จากบทที่ 4 มีทางเดินตรงย้อนกลับผ่านการบวกนั้นไปถึงชั้นล่างๆ ได้
`check.py` ยืนยันทั้งสองอย่าง ตารางศูนย์ให้ token เดิมกลับมาเป๊ะ และตารางที่ไม่ใช่
...
คุยกัน แล้วคิด แต่ละครึ่งบวกเข้ากับ token แทนที่จะแทนที่ และแต่ละครึ่งมี `layer_norm`
อยู่หน้ามัน ซึ่งปรับ vector ของแต่ละ token ให้ค่าเฉลี่ยเป็นศูนย์และการกระจายเป็นหนึ่ง
ก่อนเข้าชั้น ถ้าไม่มีมัน หลายสิบชั้นที่บวกเข้าไปเรื่อยๆ จะปล่อยให้ตัวเลขโตเท่าไหร่ก็ได้
และ softmax บนคะแนนที่ใหญ่มากกลายเป็นการเลือก token เดียวแข็งๆ

output ของ block มีรูปร่างเดียวกับ input นั่นคือสิ่งที่ทำให้ซ้อนได้ block ถัดไปเริ่มจาก
token ที่ถือสิ่งที่ block นี้รวบรวมมาแล้ว และ model ที่คุณเรียกคือ block นี้ซ้อนกันหลาย
...
## 7. KV cache คือเหตุผลที่ token ขาออกแพงกว่าขาเข้า
...
ใหม่ ทั้งที่เก้าสิบเก้าตัวแรกไม่ได้เปลี่ยน key และ value ของ token ขึ้นกับ token นั้นตัว
เดียว ค่าเดิมทุกก้าว
...
query ของตัวเองกับ key ทุกตัวในรายการ นี่คือ KV cache (แคชคีย์และแวลู คือการเก็บ key
กับ value ของ token ก่อนหน้าไว้ในหน่วยความจำ แทนที่จะคำนวณใหม่ทุกก้าว) `check.py`
ยืนยันว่าป้อน token ผ่านมันทีละตัว ได้แถวเดียวกันเป๊ะกับ masked attention ในหัวข้อที่ 3
ที่ทำทั้งหมดในครั้งเดียว
...
ตัวเลขนี้อธิบายสามอย่างในบิลของคุณ หน่วยความจำของ model ที่ให้บริการโตตามความยาวของ
บทสนทนา เพราะ cache คือสิ่งที่ถูกเก็บ token ขาออกแพงกว่าขาเข้า เพราะขาเข้าทั้งก้อน
คำนวณได้พร้อมกันในรอบเดียว ส่วนขาออกต้องทีละตัว โดยแต่ละตัวรอตัวก่อนหน้า และ prompt
caching ที่บทที่ 15 ของบทเรียนพูดถึง คือผู้ให้บริการเก็บ cache นี้ของ system prompt
ของคุณไว้ข้ามคำขอ แล้วคิดเงินส่วนนั้นถูกลง เพราะเขาไม่ต้องคำนวณมันใหม่จริงๆ ครับ
...
chatbot ทุกตัวที่คุณจะเรียกในหนังสือเล่มนี้เป็น decoder ล้วน ไม่ใช่เพราะ encoder แย่ แต่
เพราะโจทย์ทายคำถัดไปเป็นโจทย์เดียวที่ข้อความทุกชิ้นบนโลกเป็นข้อมูลฝึกให้ได้โดยไม่ต้อง
มีใครติดป้าย และเพราะ model ที่เขียนได้ทำงานอ่านได้ด้วยการเขียนคำตอบออกมา ส่วน
model ที่อ่านได้อย่างเดียวทำงานเขียนไม่ได้เลย เมื่อต้องเลือกครึ่งเดียว ครึ่งที่ทำได้ทั้ง
สองอย่างชนะ
...
- KV cache เก็บ key กับ value ของ token เก่าไว้ คำตอบเดิม งานน้อยลงพันเท่า และมันคือ
  เหตุผลที่ token ขาออกแพงกว่าขาเข้า
