...
ประโยคที่คนส่วนใหญ่ใช้คือ tool (เครื่องมือ คือฟังก์ชันที่ model ขอให้เรารันได้) เป็น
...
ถ้าคุณเคยเขียน tool มาก่อน หัวข้อที่ 2 เรื่อง structured output ข้ามได้ มันเชื่อมสองเรื่อง
ที่คุณรู้อยู่แล้วเข้าด้วยกัน ไม่ใช่ของใหม่ที่หัวข้อหลังจากนั้นต้องใช้

## 1. tool คืออะไรจริงๆ

tool หนึ่งตัวประกอบด้วยสามอย่าง ชื่อ คำอธิบาย และ JSON Schema (สคีมา JSON คือคำอธิบาย
รูปร่างของข้อมูลในรูปแบบ JSON) ที่บอกว่ารับ argument อะไรบ้าง สามอย่างนี้คือสิ่งที่ model
ได้เห็น ส่วนอย่างที่สี่คือฟังก์ชัน Python ซึ่ง model ไม่เคยเห็นและไม่เคยแตะ
...
และ argument ที่มันแต่งขึ้นเป็นข้อความ JSON เราแยก JSON นั้นออกมา ตรวจสอบ ตัดสินใจว่า
จะรันหรือไม่ ถ้ารันก็เรียกฟังก์ชันของเราเอง แล้วส่งผลลัพธ์กลับเข้าบทสนทนา ภาคพื้นฐาน
บทที่ 7 แสดงว่าคำขอนั้นคือข้อความธรรมดาที่ model เขียนต่อจาก template เหมือนคำตอบ
อื่นทุกประการ
...
คนมักคิดว่า tool calling กับ structured output (ผลลัพธ์มีโครงสร้าง คือการบังคับให้
model ตอบเป็น JSON ตามรูปที่กำหนด) เป็นคนละเรื่อง อย่างที่สองคืออย่างแรกที่ไม่ได้รัน
อะไร
...
อย่าเขียนวิธีใช้ tool ไว้ใน system prompt ประโยคว่า เวลาจะแก้ไฟล์ให้ใช้ edit_file ไม่ใช่
write_file อยู่ผิดที่ ถ้ามันอยู่ใน `description` ของ `write_file` มันจะเดินทางไปพร้อมกับ
tool ตัวนั้นเสมอ รวมถึงตอนที่ tool ถูกยืมไปใช้ที่อื่น และตอนที่คุณลืมว่าเคยเขียนกฎนั้นไว้
...
แทรกกลาง subagent ในบทที่ 15 ใช้ช่องนี้เพื่อบอกลูกว่างานของมันคืออะไร โดยที่
`BEHAVIOUR` ยังเป็น prefix ชุดเดิมทุกตัวอักษร
...
ตัวอย่างที่ชัดอีกอันอยู่ใน `search.py` ตอนที่ model เขียน regular expression (นิพจน์ปกติ
คือรูปแบบสำหรับค้นหาข้อความ) ที่อันตราย
