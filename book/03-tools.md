...
เอา `read_file` เป็นตัวอย่าง ตัวฟังก์ชันจริงใน `src/agentpath/tools/files.py` ยาวห้า
บรรทัด และเรียกตัวช่วยอีกสองตัวที่กักพาธกับตัดผลลัพธ์ที่ยาวเกิน model ไม่เคยเห็นบรรทัด
ไหนในนั้นเลย สิ่งที่มันเห็นคือก้อนนี้
...
ทั้งหมดที่ model รู้คือชื่อหนึ่งชื่อ ประโยคหนึ่งประโยค และช่องที่ชื่อ `path` มันไม่รู้ว่า
พาธถูกกักไว้ ไม่รู้ว่าไฟล์ยาวเกินจะถูกตัด และไม่รู้ด้วยซ้ำว่ามีโฟลเดอร์รากอยู่
...
คนมักคิดว่า tool calling กับ structured output
เป็นคนละเรื่อง อย่างที่สองคืออย่างแรกที่ไม่ได้รันอะไร
...
ที่ได้กลับมาคือ JSON ที่ตรงตาม schema เช่น `{"label": "negative", "confidence": 0.82}`
แล้วคุณก็หยิบมันไปใช้โดยไม่ต้องรันฟังก์ชันอะไรเลย
...
ภาพของความเสียหายชัดกว่านั้นเมื่อใส่ตัวเลข ไฟล์ `config.py` ยาวสี่ร้อยบรรทัด ผู้ใช้ขอ
ให้แก้ค่า timeout หนึ่งบรรทัด model เลือก `write_file` แล้วส่ง content มาสิบสองบรรทัด
เท่าที่มันจำได้จากตอนอ่าน อีกสามร้อยแปดสิบแปดบรรทัดหายไปเงียบๆ และ tool คืนค่ากลับมาว่า
เขียนสำเร็จ
...
ไม่ใช่แค่แพงขึ้น ต่อ server สามตัวแล้วได้ tool สี่สิบตัว schema รวมกันราวหนึ่งหมื่นสี่พัน
ตัวอักษร คือประมาณสามพันห้าร้อย token ที่เดินทางไปทุกคำขอ ก่อนที่ model จะได้อ่านคำแรก
ของงานจริง
...
สามบรรทัดนั้นดูน้อยจนน่าขำ แต่ลองเอาออกดู model
ที่ไม่รู้ว่ากำลังยืนอยู่โฟลเดอร์ไหนจะเดาพาธ model ที่ไม่รู้ว่าอยู่บน Windows
จะเรียก `run_shell` ด้วย `ls -la`
ได้ข้อความว่าไม่รู้จักคำสั่งกลับมา แล้วรอบถัดไปค่อยเดาใหม่เป็น `dir`
สองรอบนั้นเสียไปเปล่าๆ ข้อเท็จจริงที่ราคาสามสิบ token
ตัดการเดาที่ราคาหนึ่งรอบเต็มออกไป

ทั้งสี่ก้อนนี้ไหลมารวมเป็นคำขอเดียวก่อนออกจากเครื่อง เรียงตามความนิ่งจากหน้าไปหลัง

```mermaid
flowchart TB
    B["BEHAVIOUR<br/>นิสัยและมาตรฐานงาน"] --> R
    F["facts<br/>โฟลเดอร์ ระบบปฏิบัติการ รุ่นภาษา"] --> R
    S["schema ของ tool ทุกตัว<br/>ชื่อ description parameters"] --> R
    U["user message<br/>งานของครั้งนี้"] --> R
    R["คำขอหนึ่งก้อน"] --> M["model"]
```
...
แปลว่ามันคือ input ที่ไม่น่าเชื่อถือ เหมือน input
จากผู้ใช้ทางอินเทอร์เน็ตทุกประการ การเรียกที่ผิดจึงต้องกลายเป็นข้อความที่ model
อ่านแล้วแก้ทางได้ ไม่ใช่ exception ที่ฆ่า loop
...
สมมติมีคนเพิ่ม `delete_file` เข้ามาบ่ายวันศุกร์แล้วลืมคิดเรื่องนี้ ถ้าค่าเริ่มต้นเป็น
`True` เช้าวันจันทร์ agent ลบไฟล์ที่ไม่มีใครสั่งให้ลบไปแล้วโดยไม่มีคำถามขึ้นจอสักครั้ง
ถ้าเป็น `False` สิ่งที่เกิดขึ้นคือคำถามที่คนเห็นแล้วรู้ทันทีว่าลืมอะไรไป
