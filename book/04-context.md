...
คำขอครั้งถัดไปโดนปฏิเสธด้วย HTTP 400 คุณไล่ดูโค้ดที่ประกอบคำขอ ไล่ดู header ไล่ดู
ชื่อ model ทุกอย่างถูกหมด ต้นเหตุคือฟังก์ชันตัด context ที่ทำงานผ่านไปแล้วเมื่อหลาย
วินาทีก่อน ในเส้นทางโค้ดคนละที่ และมันตัดกลางคู่ข้อความที่ห้ามแยกออกจากกันพอดี นี่คือ
บั๊กที่หาที่มายากที่สุดในหลักสูตรนี้ และบทนี้มีไว้เพื่อให้มันเกิดไม่ได้ตั้งแต่โครงสร้าง

คำว่า context window (หน้าต่างบริบท คือปริมาณข้อความสูงสุดที่ model รับได้ในการเรียก
ครั้งเดียว) ทำให้คนเข้าใจผิดตั้งแต่ชื่อ เพราะคำว่าหน้าต่างชวนให้นึกถึงพื้นที่เก็บของ
เหมือน RAM ที่ของอยู่ในนั้นจนกว่าจะถูกเขียนทับ มันไม่ใช่แบบนั้นเลย context window คือ
ขนาดสูงสุดของคำขอหนึ่งครั้ง มันคืองบประมาณที่คุณจ่ายใหม่ทุกครั้งที่กดส่ง ไม่ใช่บัญชี
ที่ยอดสะสมอยู่ ภาคพื้นฐานบทที่ 6 บอกว่าทำไมงบนั้นถึงมีเพดาน ทุก token คูณทุก token
...
ปฏิเสธทั้งคำขอด้วย HTTP 400 (รหัสสี่ร้อย คือคำตอบที่บอกว่าคำขอผิดรูป) ทันที
...
หั่น list ตรงตำแหน่งที่ผู้ใช้พูด แล้วถือว่าทุกอย่างหลังจากนั้นจนถึงคำพูดถัดไปของผู้ใช้
เป็นของก้อนเดียวกัน แค่นั้น
...
ทางเลือกอีกทางคือส่ง list ที่มีแต่ system message ซึ่ง model จะตอบกลับมาแบบไร้ทิศทาง
...
และมันใส่ข้อความว่าไม่ได้รัน แทนที่จะใส่ค่าว่าง เพราะ model ต้องรู้ว่าเกิดอะไรขึ้น
ผลลัพธ์ว่างเปล่าถูกตีความว่าคำสั่งสำเร็จแต่ไม่พิมพ์อะไร ซึ่งเป็นความเข้าใจผิดคนละ
เรื่องกับความจริง ประโยคที่บอกตรงๆ ว่าการรันถูกหยุดก่อน ทำให้ model กลับมาทำงานต่อ
ได้ถูกเมื่อมี resume

## 5. prompt caching และกฎข้อเดียวที่ตามมาจากมัน

prompt caching (การแคช prompt คือการที่ผู้ให้บริการเก็บผลการประมวลผลส่วนหน้าของ
คำขอไว้ใช้ซ้ำ) คือส่วนลดที่ใหญ่ที่สุดที่มีให้ agent ผู้ให้บริการจดจำผลการประมวลผลของ
prefix (ส่วนนำหน้า คือช่วงต้นของคำขอที่นับจากตัวอักษรแรก) ของคำขอไว้ ถ้าคำขอถัดไปมี
prefix ตรงกันทุกตัวอักษร ส่วนนั้นจะถูกคิดราคาถูกลงมาก และประมวลผลเร็วขึ้นด้วย
...
เพราะ cache จับคู่จาก prefix ตัวอักษรเดียวที่ต่างกันที่ตำแหน่งต้นๆ ทำให้ทุกอย่างหลัง
จากนั้นใช้ cache ไม่ได้เลย ถ้าคุณใส่เวลาปัจจุบัน ชื่อผู้ใช้ หรือ session id ไว้ต้น
system prompt cache จะพังทุกคำขอ ตลอดไป การสลับลำดับ tool definition ให้ผลเดียวกัน
ถ้า tool ของคุณเก็บใน dict ที่ลำดับไม่แน่นอน หรือคุณ generate schema ใหม่ทุกรอบด้วย
ลำดับที่ต่างกัน prefix จะไม่ตรงและ cache จะไม่เคยทำงาน
...
ไม่มีวันรู้ถ้าไม่ไปดูตัวเลข cache ที่ผู้ให้บริการรายงานกลับมา นี่คือเหตุผลที่ `Usage`
เก็บ `per_call` ไว้ทั้งก้อน แทนที่จะเก็บแค่ยอดรวมครับ

## 6. ตัวนับ token ของแต่ละเจ้าไม่เท่ากัน

หัวข้อนี้ต่อจากบทที่ 1 แต่ผลของมันตกอยู่ที่บทนี้โดยตรง การตัดสินใจว่าจะตัดหรือไม่ตัด
ต้องอาศัยตัวเลขว่าตอนนี้ใช้ไปเท่าไหร่แล้ว ถ้าตัวเลขนั้นมาจาก tokenizer ของผู้ให้บริการ
คนละรายกับที่คุณกำลังคุยอยู่ การตัดสินใจทั้งหมดตั้งอยู่บนตัวเลขที่ผิด ภาคพื้นฐาน
บทที่ 2 แสดงว่าทำไม tokenizer สองตัวถึงนับข้อความเดียวกันได้ต่างกันเกือบสามเท่า
...
1. ตัด output ของ tool ก่อนส่งกลับ ผลลัพธ์ยาวหนึ่งครั้งถูกจ่ายซ้ำทุกรอบที่เหลือของ
   บทสนทนา `truncate` ใน `files.py` มีอยู่เพราะเหตุนี้

2. อ่านไฟล์เฉพาะช่วงแทนทั้งไฟล์ ถ้า agent ต้องการฟังก์ชันเดียว การใส่ไฟล์สองพัน
   บรรทัดเข้าไปคือการจ่ายค่าอีกหนึ่งพันเก้าร้อยบรรทัดทุกรอบ
...
สี่ข้อแรกไม่ทำให้ agent ลืมอะไรเลย มีแต่ข้อสุดท้ายที่ทำ นั่นคือเหตุผลที่มันอยู่ท้ายสุด
ครับ
