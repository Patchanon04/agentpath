...
model แล้วรัน tool สลับกันไปจนได้คำตอบ) ไม่มีบรรทัดไหนขยับ และคุณจะรู้ว่า
ความต่างที่ต้องเขียนโค้ดรับมือมีอยู่สี่จุด ไม่ใช่สี่สิบจุด
...
คนที่ยังไม่เคยเขียน provider (ผู้ให้บริการ คือคลาสที่รู้วิธีคุยกับ API
เจ้าหนึ่ง) ด้วยมือ มักคิดว่าการรองรับสองเจ้าคือการเขียนโปรแกรมสองตัว
ความจริงคือมันคือการแปลสี่จุด

ในโปรเจกต์นี้ไฟล์ทั้งสองอยู่ที่ `src/agentpath/providers/openai_compat.py`
กับ `src/agentpath/providers/anthropic.py` ทั้งคู่ยาวไม่ถึงร้อยห้าสิบบรรทัด
และนี่คือสิ่งที่ต่างกันจริงๆ
...
เปลี่ยน signature ของ `stream` ซึ่งแปลว่าต้องแก้ทุกที่ที่เรียกมัน ทุกtest
...
`signature` อีกอันพังเงียบคือ `usage` ประเภทหลังคือประเภทที่ต้องมีtestยืนยัน
