...
บทนี้ตอบคำถามนั้นเป็นสี่ชิ้น retry (การลองใหม่ คือการยิงคำขอเดิมซ้ำหลังจากล้มเหลว) ที่รู้ว่าลองใหม่ตอนไหนช่วยและตอนไหนแค่ช้า
ลง เส้นสองเส้นที่ retry ข้ามไม่ได้ token ตัวเดียวที่ถูกอ่านสามจุด จึงทำให้การ
กดหยุดหนึ่งครั้งหยุดงานที่ยังไม่เริ่มได้ทุกชิ้น และไฟล์ session ที่รอด
...
จากคน หัวข้อที่ 1 ถึง 5 จึงแยกทั้งสามออกจากกันก่อน แล้วหัวข้อที่ 6 ค่อยว่าด้วยตัวที่สาม
ซึ่งต้องเห็นอีกสองตัวอยู่ในสายตาตลอด

หัวข้อที่ 3 เรื่อง jitter (การกระจายเวลาแบบสุ่ม) ข้ามได้ในรอบแรก ไม่มีหัวข้อไหนหลังจากนั้นที่ต้องใช้มัน
...
การถาม model ซึ่งถามซ้ำได้เสมอ การส่งใหม่จึงไม่ใช่การทำซ้ำที่มีผลข้างเคียง มันคือการ
...
ของมันก่อน แล้วค่อยถูกหยุดที่ประตูถัดไป ถ้าแต่ละที่มีสัญญาณของตัวเอง จะมีสักที่ที่ไม่ได้รับข่าว และนั่น
...
ตรวจใน loop อย่างเดียวไม่พอ loop ตรวจก่อนเรียก tool แต่ละตัวก็จริง แต่ระหว่าง
การตรวจนั้นกับ `Popen` ยังมีช่องว่าง และ tool ยังถูกเรียกจากที่อื่นที่ไม่ใช่ loop ได้
เช่น subagent กับ eval การตรวจที่ประตูของ shell เองคือสิ่งที่ปิดช่องทั้งสอง
...
อักษร ตัวเลข ขีด หรือจุดด้วยขีด แล้วตัดขีดกับจุดที่หัวท้ายทิ้ง ซึ่งขั้นหลังคือสิ่งที่
กัน `..` สิ่งที่เหลือเป็นชื่อไฟล์โดยโครงสร้าง ไม่ใช่โดยการผ่านการ
ตรวจ
