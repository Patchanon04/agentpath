...
มันรัน subprocess (โปรเซสลูก คือโปรแกรมที่โปรแกรมของเราสั่งให้เริ่มทำงาน) บน
...
บทนี้ตอบคำถามนั้นเป็นสี่ชิ้น retry ที่รู้ว่าลองใหม่ตอนไหนช่วยและตอนไหนแค่ช้า
ลง เส้นสองเส้นที่ retry ข้ามไม่ได้ token ตัวเดียวที่ทำให้การกดหยุดหนึ่งครั้ง
หยุดทั้ง stream ทั้ง subprocess และทั้ง loop พร้อมกัน และไฟล์ session ที่รอด
จากการถูกฆ่ากลางทาง ชิ้นที่หนักคือชิ้นที่สาม เพราะมันขอให้คุณถือสามเรื่องไว้
ในหัวพร้อมกัน คือ retry ที่อยากลองใหม่ timeout ที่อยากเลิกรอ และการกดหยุดที่มา
จากคน หัวข้อที่ 1 ถึง 5 จึงแยกทั้งสามออกจากกันก่อน แล้วหัวข้อที่ 6 ค่อยเอามาชนกัน

หัวข้อที่ 3 เรื่อง jitter ข้ามได้ในรอบแรก ไม่มีหัวข้อไหนหลังจากนั้นที่ต้องใช้มัน

## 1. retry เฉพาะสิ่งที่ retry แล้วช่วย

retry (การลองใหม่ คือการยิงคำขอเดิมซ้ำหลังจากล้มเหลว) ที่ใส่ผิดที่ไม่ได้ทำให้
...
500 กับ 503 แปลว่าฝั่งนั้นมีปัญหาชั่วคราว 408 แปลว่าหมดเวลา ทั้งหมดนี้คือ
สถานะที่รอแล้วต่างออกไปได้ จึงคุ้มที่จะรอ
...
transport error นับเป็นพวกที่ retry ได้ด้วยเหตุผลที่ตรงไปตรงมา การเชื่อมต่อ
ที่ขาดก่อนอะไรจะไปถึง แปลว่า server ยังไม่เคยเห็นคำขอนั้นเลย การส่งใหม่จึง
ไม่ใช่การทำซ้ำ มันคือความพยายามครั้งแรกที่สำเร็จ
...
ทำไมไม่สุ่มเต็มช่วงตั้งแต่ศูนย์ ทำได้ และบางระบบทำ แต่การรอใกล้ศูนย์แปลว่า
บางตัวยิงกลับมาแทบจะทันที ซึ่งทิ้งประโยชน์ของ backoff (การถอยห่าง คือการยืด
เวลารอออกไปเรื่อยๆ เมื่อล้มเหลวซ้ำ) ไปเปล่าๆ ช่วงครึ่งหนึ่งถึงเต็มรักษาการ
ถอยห่างไว้ พร้อมกับกระจายเวลาพอที่จะไม่กระจุก
...
เส้นแบ่งคือ idempotency (ความไม่เปลี่ยนแปลงเมื่อทำซ้ำ คือคุณสมบัติที่การทำ
สองครั้งให้ผลเท่ากับทำครั้งเดียว)
...
การรัน tool ที่มี side effect (ผลข้างเคียง คือการเปลี่ยนแปลงที่เกิดขึ้นนอก
โปรแกรม) ไม่ใช่แบบนั้น retry คำสั่งที่ส่งอีเมลแปลว่าอีเมลสองฉบับ retry คำสั่ง
ที่ตัดเงินแปลว่าตัดสองครั้ง และเรื่องที่ทำให้มันร้ายคือกรณีที่อยาก retry ที่สุด
คือกรณีที่คุณไม่รู้ว่าเกิดอะไรขึ้น การเชื่อมต่อขาดหลังส่งคำขอไปแล้วแต่ก่อนได้
คำตอบ ตรงนั้นการกระทำอาจสำเร็จไปแล้ว และคุณไม่มีทางรู้

ทางแก้ที่ถูกคือ idempotency key (กุญแจกันทำซ้ำ คือรหัสที่ผู้เรียกสร้างขึ้น
เพื่อให้ผู้รับรู้ว่าคำขอสองครั้งคือคำขอเดียวกัน) ผู้เรียกสร้างรหัสไม่ซ้ำหนึ่ง
ตัวต่อหนึ่งเจตนา แล้วส่งไปกับทุกความพยายาม ฝั่งรับเก็บรหัสที่เคยทำแล้วไว้
...
คำตอบที่สองจะถูกต่อท้ายคำตอบแรกที่ค้างอยู่ ได้ข้อความที่ไม่มีใครเคยเขียน
...
การหยุดต้องเกิดขึ้นหลายที่พร้อมกัน ต้องปิด stream ที่ค้างอยู่ ต้องฆ่า
subprocess ที่ spawn ไว้ ต้องปลดการรอคำตอบ permission และต้องหยุด loop ไม่ให้
เริ่มรอบใหม่ ถ้าแต่ละที่มีสัญญาณของตัวเอง จะมีสักที่ที่ไม่ได้รับข่าว และนั่น
...
ตรวจใน loop อย่างเดียวไม่พอ เพราะ loop ตรวจระหว่างรอบ ถ้าคนกดหยุดตอนที่ loop
กำลังไล่รัน tool ตัวที่สองจากสามตัว การตรวจครั้งถัดไปของ loop จะเกิดขึ้นหลัง
จากทั้งสามตัวรันจบแล้ว การตรวจที่ประตูของ tool เองคือสิ่งที่ทำให้ตัวที่สาม
ไม่ได้เริ่ม
...
`except Exception` ที่กว้างมีที่ทางของมัน แต่มันต้องมาพร้อมรายการสั้นๆ ของ
สิ่งที่ต้องปล่อยผ่าน สัญญาณหยุดที่มาจากคนอยู่ข้อแรกในรายการนั้นเสมอครับ
...
มันไม่ได้ใช้ `resolve_inside` เพราะสิ่งที่เข้ามาไม่ใช่พาธ มันคือชื่อ วิธีที่
ถูกจึงไม่ใช่การตรวจว่าพาธที่ประกอบแล้วอยู่ในโฟลเดอร์ไหม แต่คือการทิ้งทุกอย่าง
ที่ไม่ใช่ชื่อ เก็บเฉพาะส่วนหลังตัวคั่นตัวสุดท้าย แล้วแทนอักขระที่ไม่ใช่ตัว
อักษรตัวเลขด้วยขีด สิ่งที่เหลือเป็นชื่อไฟล์โดยโครงสร้าง ไม่ใช่โดยการผ่านการ
ตรวจ
