...
tool (เครื่องมือ คือฟังก์ชันที่ model ขอให้เรารันได้) นั่งอยู่ในไฟล์เดียวกัน เขียนโดยคนเดียว
...
กฎว่า agent (เอเจนต์ คือโปรแกรมที่ให้ model ตัดสินใจแล้วรัน tool ให้) แตะได้เฉพาะใน
...
โปรเจกต์นี้ยังไม่ได้ทำสิ่งนั้น เขียนไว้ตรงนี้เพราะมันคือขอบเขตที่คุณต้องรู้ก่อนเอาโค้ดนี้ไป
ใช้ ไม่ใช่ข้อบกพร่องที่ควรเงียบไว้ ถ้าคุณจะรัน agent ตัวนี้บนเครื่องที่มีของที่เสียไม่ได้ ให้
ปิด shell tool ทิ้ง หรือรันทั้ง process ในกล่องที่ระบบปฏิบัติการกักไว้ให้แล้ว **การรัน
`--yes` บนเครื่องส่วนตัวคือการยอมให้ model รันคำสั่งอะไรก็ได้ในสิทธิ์ของคุณ** และนั่นเป็น
ข้อความที่ควรอ่านแล้วรู้สึกไม่สบายใจครับ
...
`read_file` ปฏิเสธมันถูกต้อง ชั้นที่สองแย่กว่า คือ `rglob` เดินตาม symlink (ลิงก์เชิง
สัญลักษณ์ คือไฟล์ที่ชี้ไปหาไฟล์อื่น) และ junction ของ Windows ด้วย ลิงก์ที่ถูกวางไว้ใน
โฟลเดอร์งานจึงเปิดทางให้อ่านไฟล์ที่ไหนก็ได้บนเครื่อง

การกรองด้วยชื่อไม่มีทางเห็นเรื่องนี้เลย ชื่อของลิงก์คือชื่อที่คนวางลิงก์ตั้ง มันจะชื่อ
`notes.txt` ก็ได้ สิ่งเดียวที่รู้ความจริงคือการ resolve พาธนั้น ซึ่งเป็นสิ่งที่ประตูทำอยู่แล้ว
...
ลองนึกภาพว่าเจอสองที่แล้วแทนที่ให้ model อยากเปลี่ยนค่าเริ่มต้นของ timeout ในฟังก์ชัน
หนึ่ง แล้วส่ง `old` มาเป็น `timeout=30` ถ้าไฟล์นั้นมีคำว่า `timeout=30` อยู่สี่ที่
การแทนที่ทุกจุดจะสำเร็จ ไม่มี error ไม่มีคำเตือน tool คืนคำว่าแก้แล้ว และ model รายงานกับผู้ใช้ว่า
ทำเสร็จแล้ว ความเสียหายอยู่ในอีกสามที่ที่ไม่มีใครขอให้แตะ
...
error ของ tool คือ prompt (พรอมป์ต คือข้อความที่เราส่งไปให้ model อ่าน) ไม่ใช่ log ผู้อ่าน
...
หลังฆ่าแล้วยังต้องเก็บของ เพราะผลลัพธ์บางส่วนที่คำสั่งพิมพ์ไปแล้วมักเป็นสิ่งที่บอกว่ามันไป
ติดตรงไหน
...
การอ่านผลของคำสั่งด้วยการสมมติว่าเป็น utf-8 นั้นผิด เครื่อง Windows เครื่องเดียวมี
โปรแกรมสองพวก พวกใหม่เขียน utf-8 พวกที่ติดมากับระบบเขียน codepage (โค้ดเพจ คือตาราง
รหัสอักขระแบบเก่าของคอนโซล) เดิม การถอดรหัสพวกหลังด้วย utf-8 จะเปลี่ยนตัวอักษรที่ไม่ใช่
ASCII ทุกตัวเป็นเครื่องหมายแทนที่ และถ้าคุณใส่ `errors="replace"` ไว้ มันจะเกิดขึ้นเงียบ
สนิท
...
วิธีที่ดูเหมือนจะแก้ได้แต่แก้ไม่ได้ ก็คุ้มที่จะรู้ คือการเติม `chcp` ไว้หน้าคำสั่ง คำสั่งในตัว
shell อย่าง `dir` อ่านค่า codepage ตอน shell เริ่มทำงาน ซึ่งเกิดก่อน `chcp` ในบรรทัด
เดียวกันจะได้ทำงาน ผลคือคำสั่งแรกของ session ยังเสียชื่อไฟล์อยู่ ส่วนคำสั่งที่สองเป็นต้นไป
ปกติหมด ซึ่งเป็นอาการที่ตามหาต้นตอยากมากครับ
...
(ฐานข้อมูลเวกเตอร์ คือระบบที่ค้นด้วยความใกล้เคียงเชิงความหมาย) มันต้องการ tool สองตัว
...
ข้อได้เปรียบที่ตามมามีสามข้อ ไม่มีดัชนีให้ล้าสมัย ไฟล์ที่เพิ่งถูกแก้เมื่อวินาทีที่แล้วก็ค้นเจอ
ทันที ผลลัพธ์ตรวจสอบได้ด้วยตา เพราะมันคือเลขบรรทัดกับข้อความ ไม่ใช่คะแนนความใกล้เคียง
...
การให้ model เขียน regular expression (นิพจน์ปกติ คือรูปแบบสำหรับค้นหาข้อความ หรือ
...
คือตัวทำซ้ำซ้อนตัวทำซ้ำ และการเช็คมันไว้ก่อนคือของถูก
...
ถึงอย่างนั้นก็ยังต้องมีเส้นตาย และนี่คือส่วนที่น่าสนใจที่สุดของหัวข้อนี้ เพราะวิธีที่คนคิดออก
สองวิธีแรกไม่ทำงาน
...
thread จับเวลา มันก็ไม่ทำงาน เพราะการแมตช์ไม่ปล่อย GIL (Global Interpreter Lock คือ
กุญแจที่ยอมให้ thread เดียวรัน Python ได้ในหนึ่งขณะ) thread ที่รอเส้นตายจึงไม่ได้รันจน
กว่าสิ่งที่มันรออยู่จะจบไปแล้ว
...
import path ไฟล์ชื่อ `json.py` ที่ agent เพิ่งเขียนจะถูกรันโดย process ลูกก่อน
การค้นจะเริ่ม โดยไม่ผ่านการขอ permission เลย เพราะการค้นเป็น tool ที่ปลอดภัย
การเขียนราคาไว้ในคอมเมนต์ทำให้คนที่มาอ่านทีหลังตัดสินใจได้เอง แทนที่จะ
...
ของกลไกยกเลิกแบบร่วมมือ และการรู้ขอบเขตของเครื่องมือคือสิ่งที่ทำให้คุณรู้ว่าเมื่อไหร่ต้อง
ใช้อย่างอื่น
