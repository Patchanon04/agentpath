...
นี่คือประโยคเดียวที่ต้องจำ language model (ฟังก์ชันที่ดูข้อความที่มาก่อน แล้วเดาว่าอะไรน่
าจะมาต่อ) ไม่ได้เข้าใจ ไม่ได้คิด ไม่ได้จำ มันรับสิ่งที่มาก่อน แล้วตอบว่าคำถัดไปน่าจะเป็นอ
ะไรบ้าง มากน้อยเท่าไหร่
...
model จริงไม่ได้เก็บเป็นตาราง เพราะตารางจะใหญ่เกินจักรวาลถ้า context
ยาวกว่าสองสามคำ มันเก็บเป็นตัวเลขนับพันล้านตัว ที่ถูกปรับให้ให้คำตอบใกล้เคียงกับ
ตารางนี้ โดยไม่ต้องมีตาราง แต่คำถามที่มันตอบคือคำถามเดียวกัน อะไรตามหลังสิ่ง
นี้ กี่ครั้งจากทั้งหมดกี่ครั้ง

ลองคิดเป็นตัวเลข vocabulary แสนคำกับ context สามคำ ต้องมีช่องเท่ากับแสนยกกำลังสาม
คือพันล้านล้านช่อง และเกือบทุกช่องจะว่างเพราะไม่มีใครเคยพูดลำดับนั้น นั่นคือ
context แค่สามคำ ไม่ใช่แสน token แบบ model ที่คุณเรียก
...
วิธีเลือกมีสองแบบ เอาคำที่นับได้มากที่สุดเสมอ หรือสุ่มโดยให้คำที่นับได้มากมีโอกา
สมาก แบบแรกคือ temperature (ค่าที่คุมว่าเลือก token แบบสุ่มมากแค่ไหน)
ที่ค่าศูนย์ และมันคือสาเหตุของ loop
ตอนเปิดบท เพราะเมื่อทุกการเลือกคือคำที่น่าจะเป็นที่สุด เส้นทางที่เดินจะเป็นเส้น
ทางเดียวเสมอ และเส้นทางเดียวที่วนกลับมาที่เดิมจะวนไปตลอดกาล
...
เวลาเลือกค่าจริงจึงเลือกจากงาน งานที่ต้องได้ผลเดิมทุกครั้ง เช่น แปลง log เป็น JSON
ตามรูปแบบที่กำหนดไว้ ตั้งค่าต่ำ งานที่อยากได้ตัวเลือกหลายแบบมาเทียบ เช่น ขอชื่อ
ฟังก์ชันสิบแบบให้เลือก ตั้งค่าสูงขึ้น
...
top k เก็บแค่ k คำที่น่าจะเป็นที่สุดแล้วสุ่มในนั้น หางยาวของคำที่ไม่น่าจะเป็นถูก
ตัดทิ้ง ไม่ว่าลูกเต๋าจะออกยังไงก็ไม่มีวันได้คำนอกรายการ

top p ใช้แนวคิดเดียวกันแต่ตัดคนละที่ คือเก็บคำที่น่าจะเป็นที่สุดไปเรื่อยๆ จนความน่าจะเป็น
รวมกันถึง p แล้วสุ่มในกลุ่มนั้น กลุ่มนั้นมีชื่อว่า nucleus (กลุ่มคำน่าจะเป็นที่สุด ที่รวม
กันแล้วได้ส่วนแบ่งตามที่ตั้งไว้)
...
ทั้งสามปุ่มคือคำถามเดียวกัน จะเชื่อการแจกแจงมากแค่ไหน และการเลือกคำที่นับได้มากที่สุดเสม
อ ที่หัวข้อนี้เรียกว่า temperature ศูนย์ มีชื่อทางการว่า greedy decoding (หยิบคำที่น่าจะ
เป็นที่สุดทุกก้าว ไม่มองไกลกว่านั้น)

เรื่องนี้มีผลตอน debug ถ้าคุณเรียก model สองครั้งด้วย prompt เดียวกันแล้วได้คนละ
คำตอบ สิ่งที่ต่างไม่ใช่ตัว model มันคือลูกเต๋า ตั้ง temperature เป็นศูนย์แล้วเรียก
ใหม่ ถ้ายังได้คนละคำตอบ แปลว่ามีอย่างอื่นในระบบที่เปลี่ยนไป เช่น prompt ที่ประกอบ
จากเวลาปัจจุบัน
...
loop นี้เลือกทีละคำแล้วไม่ย้อนกลับ คำที่เลือกตอนก้าวที่สามอาจพาไปเจอทางตันในก้าวที่สิบ แล
ะไม่มีใครรู้จนกว่าจะถึงตรงนั้น มีวิธีที่มองไกลกว่านั้น ชื่อ beam search (เก็บคำตอบที่เป็น
ไปได้ไว้หลายเส้นพร้อมกัน แล้วต่อทุกเส้นทีละก้าว)
...
นี่คือเหตุผลที่แชทที่คุณใช้ทุกวันส่งบทสนทนาทั้งก้อนขึ้นไปใหม่ทุกครั้ง ตอนคุณพิมพ์
ว่า แล้วอันที่สองล่ะ คำว่าอันที่สองไม่มีความหมายเลยถ้าไม่มีข้อความก่อนหน้าแนบไป
ด้วย และไม่มีใครฝั่ง model จำมันไว้ให้
...
ประโยคที่คุณเพิ่งอ่านในย่อหน้าก่อนน่าจะไม่เคยถูกเขียนมาก่อนบนโลก แต่ทุกคำในนั้นคุณ
รู้จักหมดและเข้าใจมันได้ในครั้งเดียว ช่องว่างระหว่างสองอย่างนี้คือสิ่งที่ตารางนับ
ข้ามไม่ได้

model ยุคที่สามแก้เรื่องนี้ด้วยการเลิกเก็บตาราง แล้วเก็บตัวเลขชุดหนึ่ง ที่ถูกปรับให้
...
ทั้ง model อยู่ใน [foundations/03-next-token/ngram.py](../foundations/03-next-token/ngram.py) รัน `python ngram.py` เพื่อดูตาราง การสุ่มที่สาม temperature และผลของ context สองขนาด แล้วรัน `python check.py` เพื่อยืนยันทุกข้ออ้างในบทนี้ ลองแทน `CORPUS` ด้วยข้อความของคุณเองสักหน้าหนึ่ง แล้วดูว่ามันเขียนอะไรออกมา
