...
ของคำ ที่นี่มันยาวยี่สิบห้าตัวเลข เพราะข้อความมีคำต่างกันยี่สิบห้าคำ embedding (เวกเตอร์ฝัง
ตัว คือ vector ของคำหรือประโยคที่ model เรียนรู้มา) จริงยาวหลายร้อยถึงหลายพัน และไม่ได้มาจากการนับตรงๆ แต่มาจากการเรียนรู้แบบบทที่ 4
...
เฉพาะที่ถูกเพิ่มค่าขึ้น วิธีหนึ่งชื่อ TF-IDF ซึ่งหัวข้อที่ 6 จะสร้าง และบทที่ 16 ของหนังสือใช้แนวคิดเดียวกัน
...
เริ่มต้น แบบที่ใช้ที่นี่ชื่อ skip-gram (ทายเพื่อนบ้านจากคำกลาง) อีกแบบชื่อ CBOW ทายกลับ
ด้าน
...
(การส่งย้อนกลับ คือการไล่ gradient ผ่านชั้นที่ซ้อนกันจากหลังไปหน้า) ในรูปที่เล็กที่สุดเท่าที่
...
embedding model ที่บทที่ 16 ของหนังสือพูดถึงสืบเชื้อสายจากแนวคิดนี้ ฝึกบนข้อความขนาด
...
ชื่อมันซื่อสัตย์ bag of words (ถุงคำ คือการแทนเอกสารด้วยจำนวนนับของแต่ละคำ โดยไม่
สนลำดับ) เทคำลงถุงแล้วเขย่า แมวกินปลา กับ ปลากินแมว คือถุงเดียวกัน นี่คือขีดจำกัด
ของแนวคิด และเป็นเหตุผลที่บทถัดไปต้องมี attention
...
sparse (เบาบาง คือ vector ที่ตำแหน่งส่วนใหญ่เป็นศูนย์) และการเก็บเฉพาะตำแหน่งที่ไม่ใช่
ศูนย์คือสิ่งที่ทำให้ค้นเอกสารล้านฉบับด้วยคำมีราคาที่จ่ายไหว vector ของคำในหัวข้อ 1
ถึง 3 และ embedding ของ model จริงเป็นอีกแบบ คือ dense (หนาแน่น คือ vector สั้นกว่า
มากและแทบไม่มีศูนย์) สองแบบนี้คือสองตระกูลของการค้นหา และบทที่ 16 ของหนังสือเลือก
ระหว่างมัน

ทีนี้ปัญหาของคำว่า the จากหัวข้อ 4 กลับมาในรูปเอกสาร ถ้าค้นด้วยการนับเฉยๆ เอกสารที่
ยาวที่สุดชนะเสมอ และคำที่อยู่ในทุกเอกสารช่วยตัดสินไม่ได้เลย วิธีแก้มีชื่อว่า TF-IDF
(ความถี่คำคูณความถี่เอกสารผกผัน คือคะแนนที่สูงเมื่อคำนั้นเป็นสัดส่วนมากของเอกสารนี้
และหายากในเอกสารอื่น)
...
บทที่ 16 ของหนังสือใช้ความคิดนี้ในชื่อ rarity ตอน `search_notes` ให้คะแนนย่อหน้า และ
...
และการเรียนรู้อยู่ใน `skipgram.py` ข้างกัน รัน `python vectors.py` เพื่อดูเพื่อนบ้านของ cat และ agent กับการเทียบ cosine กับ
...
- Reimers และ Gurevych ปี 2019 บทความ Sentence-BERT คือก้าวจาก vector ของคำเป็น vector ของประโยค ซึ่งคือสิ่งที่บทที่ 16 ของหนังสือเรียกใช้
