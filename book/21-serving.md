...
หกสิบหก token ต่อวินาที ตัวเลขนี้ไม่ได้มาจากการวัด มันมาจากการเอา bandwidth ของการ์ด
หารด้วยขนาดของ weight และไม่มีอย่างอื่น แล้วมันใกล้กับสิ่งที่การ์ดทำได้จริงจนน่าตกใจ
เพราะการสร้างหนึ่ง token คือการอ่านทุก weight หนึ่งรอบ และเลขคณิตของชิปยังห่างจากขีด
จำกัดมาก
...
model เจ็ดสิบพันล้าน parameter ที่สิบหกบิตไม่พอดีการ์ดใบไหนที่เช่าได้ ที่สี่บิตพอดีใบเดียว
และนั่นคือเหตุผลทั้งหมดที่บทที่ 19 มีอยู่ ไม่ใช่เพื่อประหยัด แต่เพื่อให้เป็นไปได้
...
นี่คือตัวเลขจริง key หนึ่งกับ value หนึ่งต่อชั้น แต่ละอันคือจำนวนหัวคูณขนาดหัว สำหรับ model
เจ็ดพันล้านคือห้าสิบหกกิโลไบต์ต่อ token ต่อบทสนทนา
...
หน่วยความจำของการ์ดลบ weight คือที่ว่าง หารด้วย cache ต่อบทสนทนาที่ความยาวหนึ่ง คือจำนวน
บทสนทนาที่รับได้พร้อมกัน การ์ดยี่สิบสี่กิกะไบต์รับ model เจ็ดพันล้านที่สิบหกบิตได้ยี่สิบสองบท
สนทนายาวแปดพัน token ที่สามหมื่นสอง token เหลือห้า และถ้า weight ไม่พอดี คำตอบคือศูนย์
...
ทุก weight จากหน่วยความจำหนึ่งรอบ การอ่านสิบสี่กิกะไบต์บนการ์ดที่อ่านได้หนึ่งเทราไบต์ต่อ
วินาทีใช้เวลาสิบสี่มิลลิวินาที หกสิบหก token ต่อวินาที ชิปคูณเสร็จเร็วกว่านั้นหลายสิบเท่า
และนั่งรอหน่วยความจำ

decode (การถอดรหัส คือขั้นสร้าง token ทีละตัวหลังจากอ่าน prompt แล้ว) จึงถูกจำกัดด้วย
bandwidth (แบนด์วิดท์ คือจำนวนไบต์ต่อวินาทีที่อ่านจากหน่วยความจำได้) ไม่ใช่ด้วยการคำนวณ
และข้อเท็จจริงข้อเดียวนี้อธิบายสามอย่างที่คุณจ่ายเงิน
...
อย่างแรก quantize เป็นสี่บิตทำให้หนึ่งคำขอเร็วขึ้นสี่เท่า เพราะอ่านน้อยลงสี่เท่า อย่างที่สอง
การรับหลายคำขอพร้อมกันเกือบฟรี weight ถูกอ่านหนึ่งรอบต่อก้าวอยู่แล้ว จะคูณกับ vector ของ
หนึ่ง token หรือสี่สิบหก token ก็อ่านเท่ากัน ผู้ให้บริการจึงรวมคำขอเป็นชุด ที่เรียกว่า
batching (การรวมชุด คือการประมวลผลหลายคำขอในการอ่าน weight รอบเดียว) และมันคือเหตุผลที่
ราคาต่อ token ของเขาต่ำกว่าที่คุณเช่าการ์ดเองได้ อย่างที่สาม token ขาออกแพงกว่าขาเข้า เพราะ
prompt ทั้งก้อนถูกอ่านในรอบเดียวที่ชิปทำงานเต็ม ส่วนคำตอบคือ loop นี้ ทีละ token รออ่าน
หน่วยความจำทุกตัว
...
vLLM (วีแอลแอลเอ็ม คือ server ให้บริการ model แบบเปิดที่ทำ batching และจัดการ cache ให้)
เปิด endpoint รูปเดียวกับ OpenAI ซึ่งคือรูปเดียวกับที่บทเรียนที่ 01 เรียก และ mock server ของ
โปรเจกต์นี้เลียนแบบ บทที่ 12 ของหนังสือบอกว่าทั้งคอร์สถูกเขียนกับรูปร่างของ endpoint ไม่ใช่
ผู้ให้บริการ และตรงนี้คือที่ที่มันคุ้ม เปลี่ยนตัวแปรสภาพแวดล้อมตัวเดียว แล้วทุกบทเรียนตั้งแต่
01 ถึง 23 รันกับ model ที่คุณล้างข้อมูล ฝึก ปัดเศษ และปรับตามความชอบเองในสี่บทที่ผ่านมา
...
- weight คือ parameter คูณไบต์ต่อตัวเลข และสี่บิตคือสิ่งที่ทำให้ model ใหญ่พอดีการ์ดใบเดียว
