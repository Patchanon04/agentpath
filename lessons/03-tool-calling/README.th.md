...
```mermaid
flowchart LR
    S["SCHEMAS<br/>name description parameters"] --> Q["request หนึ่งก้อน<br/>messages กับ tools"]
    Q --> M["model ที่ปลายสาย"]
    M --> J["ข้อความมีโครงสร้าง<br/>name กับ arguments"]
    J --> D["dispatcher ของคุณ<br/>ยังไม่มีอะไรเกิดขึ้น"]
    D --> R["รันมัน<br/>หรือปฏิเสธ<br/>หรือถามคนก่อน"]
```
...
เดโมที่สร้างอีกแบบหนึ่งอธิบายเรื่องนี้เร็วกว่าการเถียงกัน มีคนต่อ model เข้ากับ
`exec` ตรง ๆ แล้วสั่งให้มันเก็บกวาดโฟลเดอร์ build มันเขียนออกมาสี่บรรทัดที่ถูกต้อง
กับอีกหนึ่งบรรทัดที่ตีความ `..` สูงขึ้นไปหนึ่งชั้นจากที่เจ้าของตั้งใจ งานที่ยังไม่ได้
commit สิบเอ็ดนาทีหายไปพร้อมกัน ไม่มีอะไรในโปรแกรมนั้นอยู่ในตำแหน่งที่จะดูข้อความ
ก่อนได้เลย เพราะพอข้อความนั้นมีตัวตนขึ้นมา มันก็กำลังรันอยู่แล้ว
...
ช่องว่างนี้เล็กจนกว่าปริมาณงานจะใหญ่ มีทีมหนึ่งจัดหมวด ticket ซัพพอร์ต 5000 ใบ
ผ่าน tool ที่ไม่เคยรัน แล้วได้คำตอบ 41 ใบที่ label หลุดออกนอก enum ส่วนใหญ่เป็น
`mixed` กับ `Negative` ที่ขึ้นต้นด้วยตัวใหญ่ บวกอีก 6 ใบที่ JSON แกะไม่ออกเลย
ทุกใบในนั้นคือการเรียกครั้งที่สองและบิลอีกก้อน 5000 ใบเดิมที่ผ่าน constrained mode
ของ provider ไม่ต้องเรียกซ้ำสักใบ
...
ควรเห็นขนาดของผลด้วย agent ซัพพอร์ตตัวหนึ่งที่มี tool ชื่อ `search_orders`
ตอบจากความจำแทนที่จะเรียก tool ไป 18 จาก 20 คำถามทดสอบ และคนเขียนกำลังจะเปลี่ยน
model อยู่แล้วก่อนจะลองของถูกก่อน เขาเติมประโยคเดียวว่าให้ใช้ tool นี้ทุกครั้ง
ที่ผู้ใช้ถามถึงสถานะหรือประวัติของออร์เดอร์ แล้ว model ตัวเดิมกับ 20 คำถามเดิม
เรียก tool 19 ครั้ง การแก้ครั้งนั้นใช้เวลานาทีเดียว
...
คุณดูความต่างได้ในเทิร์นเดียว agent ตัวหนึ่งถูกสั่งให้สรุป `notes/todo.md`
มันเรียก `read_file` ด้วยพาธ `todo.md` ได้ string `Error: FileNotFoundError: todo.md`
กลับไป แล้วมันเรียก `list_files` เองโดยไม่มีใครสั่ง เห็นโฟลเดอร์ `notes` และอ่าน
พาธที่ถูกต้องในเทิร์นที่สาม ถ้าเปลี่ยนเป็น raise exception แทนการคืน string
รอบนั้นจบตั้งแต่เทิร์นแรกด้วย traceback และผู้ใช้ที่ต้องไปหาพาธเอาเอง
...
ตัวเลขคือสิ่งที่ทำให้คนเชื่อ ทีมหนึ่งมีการทดสอบ 40 ตัวที่ยิงไปที่ model จริง
และเห็นสามตัวล้มเหลวในวันที่ทุกอย่างปกติโดยไม่มีใครทำซ้ำได้ build จึงแดงราว
สองวันครั้ง และทุกคนก็เรียนรู้ที่จะกดรันใหม่โดยไม่อ่านมัน พอย้าย 37 จาก 40 ตัว
ไปอยู่หลังของปลอม ชุดทดสอบลดจาก 90 วินาทีกับบิลหนึ่งก้อน เหลือ 1.2 วินาที
กับศูนย์บาท และ build ที่ยังแดงอยู่ก็กลับมามีความหมายอีกครั้ง
