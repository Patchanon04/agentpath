...
และไม่พึงปรารถนา เป็นไปไม่ได้เพราะ model รันอยู่บนฮาร์ดแวร์ของคนอื่นหลัง HTTP endpoint
(ปลายทางที่เป็น URL สำหรับยิง request ไปหา) และไม่มีเส้นทางมาถึงดิสก์ของคุณ
ไม่พึงปรารถนาเพราะคุณกำลังจะยกสิทธิ์ รันอะไรก็ได้ให้ระบบที่ทำงานด้วยการเดา
...
ประกาศไว้เท่านั้น ถ้ามันคิดชื่อขึ้นมาเองที่คุณไม่เคยลงทะเบียนไว้ dispatcher (ตัวจ่ายงาน
คือโค้ดที่รับชื่อ tool แล้วส่งต่อไปยังฟังก์ชันจริง) ของคุณจะคืน error string
...
ทำไมต้องใส่ข้อจำกัดไว้ใน schema แทนที่จะใส่เป็นประโยคใน prompt เพราะ schema คือ ส่วนที่
provider ตรวจสอบ และเป็นส่วนที่ constrained decoder (ตัวถอดรหัสที่บังคับให้ output ของ
model ตรงตามรูปแบบ) บังคับได้ ขณะที่ ประโยคใน prompt เป็นเพียงคำขอที่สุภาพ
...
file system ไม่มี permission (สิทธิ์ในการเข้าถึง) ไม่มี rate limit ไม่มี API key
