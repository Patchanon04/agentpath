...
ดังนั้น model จึงเป็นเครื่องผลิตข้อความที่เก่งมาก แต่ถูกขังอยู่ในกล่องที่ไม่มีมือ
...
และไม่พึงปรารถนา เป็นไปไม่ได้เพราะ model รันอยู่บนฮาร์ดแวร์ของคนอื่นหลัง HTTP
endpoint และไม่มีเส้นทางมาถึงดิสก์ของคุณ ไม่พึงปรารถนาเพราะคุณกำลังจะยกสิทธิ์
รันอะไรก็ได้ให้ระบบที่ทำงานด้วยการเดา สิ่งที่เราทำแทนคือหัวข้อของส่วนถัดไป
...
ของคุณจะคืน error string ออกมาและโลกก็ไม่เปลี่ยนแปลง คุณจะเห็นการ์ดนั้นตรง ๆ
ใน `tools.py` ด้านล่าง
...
ทันทีที่การรันคือการเรียกฟังก์ชันในโปรแกรมของคุณเอง ภาค 2 ของคอร์สนี้จะเพิ่ม
...
นั่นคือเหตุผลที่ถ้อยคำในคำอธิบายเหล่านั้นสำคัญมหาศาล ซึ่งเป็นเนื้อหาของส่วนที่ 4
...
**ฟิลด์ `type` ชั้นนอก** ค่าของมันคือ string ตรงตัวว่า `function` ทุกวันนี้
...
และ underscore string นี้ยังถูก model อ่านเป็นคำใบ้ด้วย ดังนั้น `add` ดีกว่า `f1`
และ `read_file` ดีกว่า `rf`

**`function.description`** คือภาษาอังกฤษง่าย ๆ หนึ่งหรือสองประโยคที่อธิบายว่า
tool ทำอะไร ส่วนที่ 4 พูดถึงฟิลด์นี้ทั้งส่วน เพราะมันทำงานหนักกว่าส่วนอื่น
ใด ๆ ของ schema
...
ที่ส่งเข้าฟังก์ชัน Python ของคุณ ดังนั้น `a` และ `b` ใน schema ต้องตรงกับ
`def add(a, b)` ในโค้ด ถ้ามันเพี้ยนไปคนละทาง คุณจะได้ `TypeError` ตอนเรียก
ซึ่ง `tools.run` จะดักไว้แล้วแปลงเป็น error string

**`type` ที่อยู่ในแต่ละ property** ตรงนี้คือที่ที่คุณบอกว่าอนุญาตค่าชนิดไหน
...
**`description` ที่อยู่ในแต่ละ property** คือเอกสารประกอบรายตัว argument
...
## 4. ทำไมคำอธิบายจึงสำคัญกว่าตัวโค้ด
...
จะถูกตำหนิถ้าใช้ฟังก์ชันผิด ถ้าย่อหน้าของคุณทำให้เพื่อนร่วมงานคนนั้นต้องเดา
มันก็จะทำให้ model ต้องเดาเช่นกัน
...
## 5. ทำไม tool ในบทนี้ถึงเป็นของเล่น
...
สองบวกสามได้ห้า มันได้ห้าบนเครื่องคุณ บนตัวรัน continuous integration บน Windows
บน Linux ทั้งวันนี้และปีหน้า ลูกเต๋าหกหน้าคืนค่าตั้งแต่หนึ่งถึงหกเสมอ ไม่มี network
ไม่มี file system ไม่มี permission ไม่มี rate limit ไม่มี API key ไม่มีค่าใช้จ่าย
และไม่มีทางที่ตัว tool เองจะเป็นบั๊ก
...
tool ของจริงจะมาถึงในภาค 2 ที่นั่นคุณจะสร้างตัวอ่านไฟล์ ตัวเขียนไฟล์ และตัวรัน
...
## 6. เขียน tools.py และ llm.py ทีละบรรทัด
...
dispatcher มาท้ายสุดเพราะมันเชื่อมสองอย่างเข้าด้วยกัน
...
การค้นด้วย `FUNCTIONS.get(name)` พร้อมการเช็ค `None` อย่างชัดเจนคือการ์ดที่กัน
...
**การเปลี่ยนแปลงที่หนึ่ง signature และค่าที่คืนกลับ** ในบทที่ 02 `complete`
...
**การเปลี่ยนแปลงที่สอง payload** key `tools` จะถูกเพิ่มก็ต่อเมื่อมีการส่ง tools
เข้ามา การส่ง `"tools": []` หรือ `"tools": null` ทำให้ provider บางเจ้าไม่พอใจ
การ์ด `if tools` จึงทำให้ request สะอาดและทำให้ฟังก์ชันนี้ยังใช้กับแชทธรรมดาได้

environment variables ตัว bearer header และการเรียก `raise_for_status`
ยังเหมือนเดิมจากบทที่ 02 ถ้า `raise_for_status` เป็นของใหม่สำหรับคุณ มันเปลี่ยน
response 4xx หรือ 5xx ใด ๆ ให้เป็น exception แทนที่จะปล่อยให้ JSON parse error
มาทำให้คุณงงในอีกสามบรรทัดถัดไป
...
## 7. รัน check.py และอ่าน tool call
...
มันจืดชืดโดยตั้งใจ ดังนั้นนี่คือสิ่งที่เดินทางไปมาจริง ๆ body ของ response
...
จะกระจายมันเป็น `add(a=2, b=3)` ซึ่งคืนค่า `5` ซึ่ง `str()` เปลี่ยนเป็น `"5"`
...
## 9. การแก้ปัญหาเมื่อ model ไม่ยอมเรียก tool
...
**วิธีที่หนึ่ง ใช้ model ที่ใหญ่กว่า** นี่คือการเปลี่ยนแปลงที่น่าเชื่อถือที่สุด
...
**วิธีที่สอง ทำให้คำอธิบายชัดเจนและเจาะจงขึ้น** ส่วนที่ 4 บอกว่าคำอธิบายคือ
...
**วิธีที่สาม ย้ายไปใช้ model แบบ hosted บน free tier** provider หลายเจ้าเสนอ
endpoint แบบ OpenAI compatible พร้อม free tier ที่เหลือเฟือสำหรับคอร์สนี้
เพราะทุกบทส่ง request เล็ก ๆ แค่ไม่กี่อัน เนื่องจาก `llm.py` อ่าน endpoint
...
ถูกต้องและแยกปัญหาไปที่ความสามารถของ model ซึ่งเป็นการแยกแบบที่ส่วนที่ 5
พยายามบอกไว้พอดี

มีปัญหาเล็ก ๆ อีกสองอย่างที่ควรรู้

ถ้าคุณเห็น `TypeError: argument after ** must be a mapping, not str` แสดงว่าคุณ
ทำการเรียก `json.loads` ตกไป กลับไปดูส่วนที่ 6
