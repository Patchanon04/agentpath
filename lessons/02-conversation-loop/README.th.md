...
ดังนั้นตอนที่คุณเรียก `ask("What is my cat called?")` จักรวาลทั้งหมดที่ model มีคือหกคำนั้น มันตอบได้ดีเท่าที่ใครก็ตามจะตอบได้
...
คุณจะยังไม่ได้ใช้มันจนกว่าจะถึงบทที่ 03 และ `chat.py` ก็ไม่เคยสร้างมันขึ้นมาเลย แต่ควรรู้ไว้ตอนนี้ด้วยเหตุผลสองข้อ ข้อแรก มันบอกคุณว่าคอร์สนี้กำลังมุ่งไปทางไหน ในเชิงกลไกแล้ว agent ก็คือ chat loop ที่มี tool message อยู่ข้างใน ข้อสอง `check.py` ในโฟลเดอร์นี้ใช้มันอยู่แล้วเพื่อพิสูจน์ประเด็นหนึ่ง และผมอยากให้คุณอ่านไฟล์นั้นได้ หน้าตาของมันเป็นแบบนี้
...
อันแรกแปลว่า model ไม่ได้คืนอะไรกลับมาเลย ซึ่งมักเกิดจากชื่อ model ที่ไม่มีอยู่จริงที่ base URL นั้น อันที่สองแปลว่า model ไม่ได้เห็น tool message ปลอมที่บรรจุ `42` ซึ่งชี้ไปที่ proxy หรือ gateway ระหว่างคุณกับ model ที่กำลังทิ้งหรือเขียนประวัติสนทนาใหม่
...
**context window** ทุก model อ่านข้อความได้จำนวนจำกัดในหนึ่ง request วัดเป็น token หนึ่ง token ประมาณสามในสี่ของหนึ่งคำในภาษาอังกฤษ ขึ้นอยู่กับ model ขีดจำกัดอาจเป็นแปดพัน token หรือสองแสน token แต่ยังไงก็มีขีดจำกัดเสมอ ข้ามเส้นไปแล้วคุณจะไม่ได้การเสื่อมสมรรถนะแบบนุ่มนวล คุณจะได้ error
...
**ค่าใช้จ่าย** นานก่อนที่คุณจะชนกำแพง คุณกำลังจ่ายค่า message ยุคแรก ๆ เดิม ๆ ซ้ำแล้วซ้ำอีก บทสนทนาร้อยรอบส่งรอบที่หนึ่งซ้ำร้อยครั้ง ในซองของ response ข้างบน ลองดู `prompt_tokens` ไต่ขึ้นในขณะที่ `completion_tokens` แทบจะคงที่ ช่องว่างนั้นคือเงินของคุณ
