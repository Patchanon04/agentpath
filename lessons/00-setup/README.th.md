...
- **virtual environment สำหรับคอร์สนี้** โฟลเดอร์ส่วนตัวที่เก็บ package
  ของโปรเจกต์นี้โดยเฉพาะ ดังนั้นการติดตั้งอะไรลงตรงนี้จะไม่มีทางทำให้โปรเจกต์อื่น
  ในเครื่องคุณพัง
- **package ที่ต้องติดตั้งแค่ตัวเดียวคือ `httpx`** นั่นคือ dependency ทั้งหมด
  ...
  language model ก็คือ HTTP endpoint ตัวหนึ่ง ดังนั้น HTTP client จึงเป็นทุกอย่าง
  ที่คุณต้องการจริง ๆ
...
ดังนั้นการแก้ไข source ของคุณจะมีผลทันทีโดยไม่ต้องติดตั้งใหม่
...
Ollama เปิด endpoint ที่เข้ากันได้กับ OpenAI ดังนั้น base url ที่คุณจะใช้คือ
ที่อยู่ local ที่แสดงไว้ในหัวข้อ 6

**ข้อดีของมัน** มันฟรีตลอดไป ไม่ต้องสมัคร ไม่ต้องใช้บัตร และไม่มี rate limit
...
**ข้อเสียของมัน** มันช้ากว่า และบ่อยครั้งช้ากว่ามาก บนแล็ปท็อปที่ไม่มี GPU แยก
...
**ข้อดีของมัน** มันเร็ว บ่อยครั้งเร็วกว่าฮาร์ดแวร์ในเครื่องอย่างมโหฬาร และไม่เสียเงิน
...
**ข้อเสียของมัน** คุณต้องมีบัญชี ซึ่งมักหมายถึงการให้อีเมล และบางครั้งก็เบอร์โทรศัพท์
free tier มี rate limit ทั้งต่อนาทีและต่อวัน ดังนั้น agent loop ที่หลุดควบคุม
ในบทที่ 04 อาจเผาโควตารายวันของคุณหมดในไม่กี่นาที และทำให้คุณใช้ไม่ได้จนถึงพรุ่งนี้
model ตัวไหนที่ฟรีก็เปลี่ยนไปตามเวลา ดังนั้น model id ที่คุณใช้อาจต้องอัปเดต
...
**ข้อดีของมัน** พวกนี้คือ model ที่แข็งแรงที่สุดที่หาได้ และเชื่อถือได้ที่สุด
...
**ข้อเสียของมัน** มันเสียเงิน สำหรับคอร์สนี้จำนวนเงินไม่มาก เพราะบทเรียนส่ง prompt สั้น ๆ
...
ที่เข้ากันได้กับ OpenAI ให้ใช้ ดังนั้น key ของ Anthropic ใช้ได้ในบทที่ 00
...
ตั้ง `AGENTPATH_MODEL` เป็น `mock` แล้วรัน `check.py` ของทุกบทโดยไม่แก้อะไรเลย
...
มีผลกับหน้าต่างใหม่เท่านั้น ดังนั้นหน้าต่างที่คุณพิมพ์คำสั่งลงไปจะยังไม่เห็นค่านั้น
...
Ollama บนเครื่องคุณเอง ไม่ต้องใช้ key ดังนั้นปล่อย `AGENTPATH_API_KEY` ไว้ว่าง
...
เริ่มต้น ดังนั้นตัวมันเองไม่ควรมี `/chat/completions` อยู่ ข้อสอง วาง key ของคุณ
...
- `OK Python 3.11` แปลว่า interpreter ของคุณใหม่พอ ตัวเลขคือเวอร์ชันของคุณ
  ดังนั้น `3.12` หรือ `3.13` ตรงนี้ก็ดีเท่ากัน
...
script จะหยุดที่ปัญหาแรก ดังนั้นแก้ทีละอย่างจากบนลงล่าง
...
ไม่เคยไปถึง server เลย ดังนั้นนี่ไม่ใช่ปัญหาเรื่อง key และไม่ใช่ปัญหาเรื่อง model แน่นอน
...
อะไรก็ตามที่คุณให้มา ดังนั้นด้วย url ข้างบนมันจึงขอ
`http://127.0.0.1:11434/chat/completions` ซึ่งไม่มีอยู่จริง endpoint
ที่เข้ากันได้ของ Ollama อยู่ใต้ `/v1` ดังนั้น url ควรลงท้ายด้วย `/v1`
...
ที่บัญชีของคุณใช้ไม่ได้ ดังนั้นให้ถือว่า error ใดก็ตามที่ระบุชื่อ model ของคุณ
...
มักเริ่ม terminal ก่อนที่คุณจะตั้งค่าอะไร ดังนั้นการเปิด terminal ใหม่คือสิ่งแรกที่ควรลอง
