...
repository นี้มี 31 directory และไฟล์ Python 78 ไฟล์ เมื่อไม่นับ virtual
...
การวิ่งไปกลับหา model 31 รอบก่อนที่มันจะได้อ่านโค้ดแม้แต่บรรทัดเดียว และทุกรอบคือ
...
คือ `MAX_OUTPUT` ที่คุณเขียนไว้ในบทที่ 07 ไฟล์ 78 ไฟล์นั้นมี source รวมกัน
227,786 ตัวอักษร ซึ่งประมาณ 57,000 token ถ้า agent อ่านทั้งหมดเพื่อหา function
เดียว มันได้เทโทเคน 57,000 token ลงในบทสนทนาไปแล้ว และเพราะบทสนทนาถูกส่งซ้ำ
...
`WORKSPACE` และ `SKIP_DIRECTORIES` มาจากบทที่ 07 ทั้งคู่ และถูกนำมาใช้ซ้ำโดยไม่
เปลี่ยน `rglob("*")` เดินทั้งต้นไม้แบบ recursive และให้ทั้งไฟล์และ directory ออก
มา จึงเป็นเหตุผลที่มีการเช็ค `is_file()` อยู่ตรงนั้น ขีดล่างนำหน้าชื่อคือธรรมเนียม
ของ Python ที่แปลว่า "อันนี้ใช้ภายใน ไม่ใช่หนึ่งใน tool"
...
### สิ่งหนึ่งที่ grep_files ไม่ได้สืบทอดมา

ควรพูดตรง ๆ เพราะมันเป็นช่องโหว่จริง และคุณควรรู้ว่ามันอยู่ตรงนั้น
...
`resolve_inside` มันเดินต้นไม้เองและเรียก `read_text` ตรง ๆ การปฏิเสธนั้นจึงไม่มี
ผลกับมัน

คุณสาธิตให้เห็นได้ในราวสิบวินาที
...
ประตูหน้าล็อกอยู่ แต่หน้าต่างเปิด นี่ไม่ใช่บั๊กที่แนบเนียน มันคือการตรวจสอบที่
หายไป และมันคือช่องโหว่แบบที่เกิดขึ้นพอดีเมื่อกฎความปลอดภัยอยู่ใน function เดียว
แทนที่จะอยู่ในการเดินต้นไม้ที่ tool ทุกตัวใช้ร่วมกัน วิธีแก้คือหนึ่งบรรทัดใน
`_walk` โดยนำ helper ชื่อ `looks_like_a_secret` ที่คุณเขียนไว้แล้วมาใช้ซ้ำ
...
เพิ่มมันเข้าไป มันคือแบบฝึกหัดข้อแรกท้ายบทนี้ และบทเรียนทั่วไปมีค่ามากกว่าตัว
บรรทัดนั้น กฎที่บังคับใช้ที่จุดเข้าจุดเดียวเท่ากับไม่ได้บังคับใช้ มันต้องอยู่ใน
ที่ที่ทุกเส้นทางผ่าน และภาคสามจะสร้าง tool ใหม่รอบแนวคิดนี้พอดี
...
`path.parts` แยก path ออกเป็นส่วนประกอบ ดังนั้นบรรทัดนี้จึงข้ามไฟล์ถ้ามี directory
...
ดังนั้นการเดินที่ไม่ข้ามอะไรเลยจะเยี่ยมชม 1315 ไฟล์ แทนที่จะเป็น 78 ไฟล์ งานมาก
กว่าสิบหกเท่าเพื่อหาคำตอบเดิม บนโปรเจกต์ที่มี dependency สองตัว แอปพลิเคชันเว็บ
...
สัญญาณรบกวนมากกว่าสัญญาณจริงยี่สิบเท่า และคิดเป็นราว 140,000 token นั่นใหญ่กว่า
...
มีข้อควรระวังหนึ่งข้อที่ควรพูดตรง ๆ เพราะการตรวจสอบดูทุกส่วนประกอบของ absolute
path มันจึงดู directory ที่อยู่เหนือ workspace ของคุณด้วย ถ้าคุณบังเอิญวางโปรเจกต์
ไว้ในโฟลเดอร์ชื่อ `node_modules` ทุกไฟล์จะถูกข้าม และ tool ทั้งสองจะคืนค่าว่าง
เปล่าตลอด นั่นเป็นเรื่องที่ไม่น่าเกิดพอที่คอร์สนี้ยอมรับมัน และวิธีแก้คือเทียบกับ
ส่วนประกอบของ relative path แทน มันคือแบบฝึกหัดข้อที่สอง
...
ถ้าบรรทัดที่สองล้มเหลว แสดงว่า `_walk` ของคุณไม่ได้เช็ค `path.parts` ถ้าบรรทัดที่
...
**ข้อหนึ่ง** ปิดช่องโหว่จากหัวข้อ 5 เพิ่มการเช็ค `looks_like_a_secret` ลงใน
`_walk` เพื่อให้เข้าถึงไฟล์ credential ผ่านการค้นหาไม่ได้เช่นกัน จากนั้นเขียน
check ที่สร้าง `.env` ใน workspace ชั่วคราวและ assert ว่า `grep_files` มองไม่เห็น
เนื้อหาของมัน ข้อนี้มีค่าที่สุดในสามข้อ เพราะมันเป็นรูจริง ไม่ใช่แบบฝึกหัดที่
คิดขึ้นมา

**ข้อสอง** แก้ปัญหา absolute path จากหัวข้อ 6 เปลี่ยนการทดสอบการข้ามให้ดูส่วน
ประกอบของ path ที่สัมพันธ์กับ workspace แทน absolute path จากนั้นตรวจสอบด้วยการ
สร้าง workspace ไว้ใน directory ชื่อ `node_modules` แล้วยืนยันว่า tool ยังหาไฟล์
เจอ
