...
schema (บอกรูปร่างของข้อมูลด้วย JSON) บวกกับ function ใน dictionary เท่านั้น และไม่มีส่วนอื่นของโปรแกรมที่ต้อง
รู้เรื่องนี้

`grep_worker.py` คือไฟล์ที่น่าแปลกใจในรายการนั้น และหัวข้อ 5 คือที่ที่มันถูก
อธิบายอย่างละเอียด ฉบับย่อคือ regular expression (รูปแบบที่ใช้ค้นข้อความ หรือ regex) ที่ model เขียนอาจรันนานกว่าอายุ
...
environment กับ git store agent เรียก `list_files` ที่ root เห็น `src/`
...
อย่างแรกคือคุณหาไฟล์จากชื่อของมัน คุณรู้ว่ามันชื่อประมาณ `settings` หรือรู้ว่ามันเป็น
...
อย่างที่สองคือคุณหาข้อความข้างในไฟล์ คุณรู้สตริงที่ปรากฏในโค้ด ชื่อ function ข้อความ error
...
error ได้มาสามไฟล์ glob (หาไฟล์จากรูปแบบของชื่อ) หา test ที่อยู่ใกล้ไฟล์หนึ่งในนั้น อ่าน test grep หา
...
ด้วยความหมาย มันคือโครงสร้างที่ถูก index (จัดของไว้ล่วงหน้าให้ค้นเจอเร็ว) ด้วยมือไปแล้ว โดยนักพัฒนาทุกคนที่เลือก
...
เมื่อดีไซน์ตรงนี้ดูเรียบง่ายเกินไป นั่นไม่ใช่การประนีประนอมเพื่อทำ
...
ขั้นแรกคือ chunking คุณตัดทุกเอกสารเป็นชิ้นเล็กพอที่จะใส่ใน prompt ได้ อาจจะราวห้าร้อย
คำต่อชิ้น มักมีส่วนซ้อนทับเล็กน้อยระหว่างชิ้นที่ติดกัน เพื่อไม่ให้ประโยคที่คร่อม
เส้นแบ่งหายไป

ขั้นที่สองคือ embedding คุณส่งแต่ละ chunk ไปยัง model ที่มีหน้าที่ไม่ใช่เขียนข้อความ แต่
...
ขั้นที่สามคือตัว query เอง เมื่อคำถามมาถึง คุณ embed คำถามด้วยวิธีเดียวกัน ถาม database
...
ลองลากหน้าต่างนั้นผ่านโปรเจกต์นี้แล้วดูว่าอะไรหล่นออกมา `providers.py` จาก
บทที่ 06 ยาวราวสองร้อยบรรทัด หน้าต่างห้าร้อยคำจึงตัดครั้งที่สองลงกลาง `stream`
พอดี chunk ที่ได้มีแค่ loop ที่ไล่บรรทัดขึ้นต้นด้วย `data: ` มี `if` บน key
ชื่อ `delta` และไม่มีชื่อ class ไม่มี path ของไฟล์ ไม่มี import
ไฟล์นั้นมี class สองตัวที่มี method ชื่อนี้และมี loop ทรงนี้เหมือนกัน
ไม่มีอะไรใน chunk บอกว่ามันมาจากตัวไหน
...
ตัว index เก่าทันทีที่ไฟล์เปลี่ยน ข้อนี้ร้ายแรงในแบบที่คนมักประเมินต่ำเกินไป
...
ให้ทันว่า chunk ไหนมาจากไฟล์เวอร์ชันไหน ซึ่งคือปัญหา cache invalidation (รู้ว่าของที่เก็บไว้อันไหนเก่าไปแล้ว) และนั่นก็คือ
...
ลองเอา session จริงมาทาบดู agent ที่ไล่ bug อยู่ยี่สิบนาทีจะแตะไฟล์หกไฟล์
repository ที่มีสามพัน chunk จะเสีย chunk ไปราวสิบห้าอันต่อการแก้หนึ่งครั้ง
แปลว่ามีคนต้องตัดสินเก้าสิบครั้งว่าสิบห้าอันไหน สร้าง index ใหม่ทั้งก้อนแทน
ก็จ่ายค่า embedding สามพันครั้งกับเวลาอีกสองสามนาที และพอมันทันสมัยแล้ว
agent ก็แก้ไฟล์ไปอีกสามไฟล์ index ไล่ตามเป้าที่ขยับ เพราะ agent คือตัวที่ขยับมัน
...
backslash แปลว่า relative path จะออกมาเป็น `src\main.py` และ model ที่อ่าน glob
pattern มาเป็นล้านครั้งจะส่ง `src/*.py` ด้วย forward slash แล้วไม่มีวันจับคู่ได้
`as_posix()` บังคับให้เป็น forward slash pattern เดียวกันจึงทำงานเหมือน
...
ลองนึกภาพรุ่นที่ไม่มีมัน ผู้อ่านบน Windows ขอให้ agent ดู source
model ส่ง `src/*.py` มา แล้ว walk เอาไปเทียบกับ `src\main.py`
backslash ไม่ใช่ slash จึงไม่มีอะไรจับคู่ได้ และ tool ตอบว่า
`no files match src/*.py` model ไม่มีเหตุผลให้สงสัย tool
มันจึงสรุปว่าไม่มีไดเรกทอรี `src` แล้วเสนอจะสร้างให้
...
กลืน slash ได้สบาย แปลว่าการจับคู่กับ path เพียงอย่างเดียวก็รองรับกรณี recursive
...
แปลว่าการจับคู่กับ path รองรับ pattern ที่บรรยายตำแหน่ง การจับคู่กับชื่อรองรับ
...
ลองสัมผัสขนาดของสิ่งที่คุณพยายามจะขัดจังหวะดู `(a+)+$` ที่เจอตัวอักษร `a`
สามสิบตัวมีวิธีแบ่งช่วงนั้นระหว่างสอง repeat ราวพันล้านแบบ และ engine ก็ไล่ลองจริง
เครื่องที่ไล่ได้สิบล้านแบบต่อวินาทียังทำงานอยู่อีกหนึ่งชั่วโมงถัดมา
เติมอีกหนึ่งตัวอักษรลงในบรรทัดนั้น หนึ่งชั่วโมงก็กลายเป็นสอง
นี่ไม่ใช่การค้นที่ช้าแล้วจะเสร็จถ้าคุณใจเย็นพอ มันคือการค้นที่เสร็จหลังคุณเกษียณ
...
```mermaid
flowchart LR
    P["grep_files<br/>ใน process ของ agent"] -->|"ส่ง JSON เข้าไปหนึ่งก้อน"| W["grep_worker.py<br/>process ของตัวเอง"]
    W --> L["_walk ที่ import มาจาก tools.py"]
    L --> M["re.search ทีละบรรทัด"]
    M -->|"ส่ง JSON array ออกมาหนึ่งชุด"| P
    P -.->|"ครบ SEARCH_SECONDS<br/>ก็ฆ่าทั้งตัว"| W
```
...
มี argument สองตัวในการเรียกนั้นที่ไม่เกี่ยวกับประสิทธิภาพ และอ่านผ่านได้ง่าย
...
process เริ่มต้นก็ไปอยู่ตรงนั้นด้วย ของที่อยู่หน้าสุดชนะ แปลว่า `import json`
...
workspace แม้จะพลาดเรื่อง path ก็ยังไปตกอยู่ในที่ที่ agent เขียนไม่ได้
...
argument `glob` จึงทำตัวเหมือน argument `pattern` เป๊ะ ๆ ค่าเริ่มต้นคือ
...
บน แปลว่าการอ่านไฟล์จะไปถึงมัน จุดที่ตรงกันที่บรรทัด 900 บอก model ว่าไฟล์นี้
...
พอดี output ของ tool ตัวหนึ่งคือ argument ของตัวถัดไป โดยไม่ต้องแปลงอะไรคั่น
...
`MAX_LINE` จำกัดสิ่งที่เครื่องยนต์ถูกขอให้แมตช์ นั่นคือเรื่องเวลา ส่วน `[:200]`
จำกัดสิ่งที่คืนกลับมา นั่นคือเรื่อง context window (ข้อความมากสุดที่ model อ่านได้ในหนึ่งครั้ง) สองอันนี้คือเพดานที่สองกับที่สาม
...
`.parts` แยก path ออกเป็นส่วนประกอบ บรรทัดนี้จึงข้ามไฟล์ถ้ามี directory
...
แปลว่าการเดินที่ไม่ข้ามอะไรเลยจะเยี่ยมชม 1808 ไฟล์ แทนที่จะเป็น 251 ไฟล์ งานมาก
...
เขียนตายตัวไว้ แปลว่าโปรเจกต์ที่เก็บของหนักไว้ในที่ที่คอร์สนี้ไม่เคยได้ยินชื่อ
...
รายการละสองร้อยตัวอักษรจะเป็น 40,000 ตัวอักษร `truncate` จากบทที่ 07 จึง
...
ส่งอีกครั้งในทุก request ถัดไป และไม่มีทางเอาคืนได้ tool ทุกตัวจึงต้องมี
...
directory pattern ที่มีส่วนประกอบเป็น directory จึงมีอะไรให้หา `notes.md`
เป็นนามสกุลอื่น ตัวกรอง glob จึงมีอะไรให้คัดออก และ `.venv/junk.py` จงใจมี
ข้อความเดียวกับ `src/main.py` แปลว่าการเดินที่ล้มเหลวในการข้าม virtual
...
argument และการจัดการ error ไปพร้อมกับตัวการค้นหาเอง
...
`MAX_RESULTS` แล้วไม่พูดอะไรเลย แปลว่าการค้นหาที่เจอสี่พันแมตช์กับการค้นหาที่เจอ
...
ถูกเขียนตายตัว `vendor` `target` `.next` และ `dist` จึงถูกเดินเต็ม ๆ บน
