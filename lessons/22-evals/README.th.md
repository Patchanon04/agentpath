...
คุณเอื้อมไปหา judge เมื่อโลกไม่มีอะไรให้ตรวจ ไม่ใช่เมื่อการตรวจโลกมันไม่สะดวก
...
ตารางข้างบนบอกว่า judge ผิดได้ ควรพูดให้เจาะจง เพราะวิธีที่มันผิดเป็นที่รู้กัน และสองใน
นั้นป้องกันได้ถูก ๆ
...
ข้างนอกจะให้ ซึ่งเป็นเหตุผลที่ provider เป็นพารามิเตอร์ และ judge มี position bias ให้ดู
สองคำตอบแล้วถามว่าอันไหนดีกว่า มันเลือกอันแรกบ่อยกว่าที่อันแรกสมควรได้ ด้วยปริมาณที่ขึ้น
กับ model และถ้อยคำ และคุณรู้ล่วงหน้าไม่ได้

สองข้อแรกจัดการด้วยวิธีใช้ เขียนเกณฑ์ที่บอกว่าคำตอบที่ดีมีอะไร ไม่ใช่มีมากแค่ไหน และให้
เกรดด้วย model ที่ไม่ได้เป็นคนผลิตคำตอบ ข้อที่สามจัดการด้วยโค้ด
...
เรื่องนี้มีราคาสองครั้งแทนที่จะเป็นหนึ่ง ซึ่งเป็นราคาปกติของการไม่ถูกหลอก และ `check.py`
พิสูจน์มันด้วยผู้ให้เกรดปลอมสองตัว ตัวหนึ่งเลือกคำตอบแรกเสมอ อีกตัวมีความเห็นจริง การ
สลับเปลี่ยนตัวแรกให้เป็นเสมอ และปล่อยตัวที่สองไว้ตามเดิม

การเปรียบเทียบเป็นคู่ยังเป็นรูปแบบที่การเลือก model ในหัวข้อ 9 ต้องการอยู่เงียบ ๆ ให้เกรด
สอง model เทียบกับมาตรฐานได้ pass rate สองค่า เปรียบเทียบคำตอบของทั้งคู่ต่อคำถาม
...
**ทำไมต้องคำแรกและไม่ใช่การค้นหา** เพราะการค้นหากำกวมพอดีในกรณีที่สำคัญ
...
**ทำไมอะไรที่ไม่ใช่การผ่านอย่างชัดเจนถึงนับเป็นความล้มเหลว**
...
**ทำไมต้อง `collected.get` พร้อมค่าสำรอง** เพราะ task ที่ไม่ได้ผลิต `Result`
...
**มีกี่ task ที่ผ่าน** ตรงมาจาก report
...
**มันมีค่าใช้จ่ายเท่าไร** `Result.usage` พก `prompt_tokens`, `completion_tokens`
...
**มันใช้เวลานานแค่ไหน** ข้อนี้คุณต้องเพิ่มเอง และก็ควรบอกกันตรง ๆ
