...
ดังนั้นเมื่อ model ต้องการข้อเท็จจริงที่มันไม่มี ทางเดียวที่ทำได้คือหยุดสร้างข้อความแล้วถาม
...
คืนกลับมา และ tool ก็ยังไม่ได้รัน ดังนั้นนี่ต้องเป็น loop ไม่ใช่เพราะความชอบเชิงสไตล์
แต่เพราะความจำเป็น
...
มีขั้นตอนขออนุมัติจากมนุษย์ และมี sub agent ที่ทำงานขนาน ซึ่งเป็นเนื้อหาของ part 4
...
ตอบได้จากความรู้เดิม มันรันศูนย์รอบ เพราะขั้นตอนที่หนึ่งกลับมาพร้อมถ้อยคำตั้งแต่ครั้งแรก
และเราคืนค่าทันที
...
ของฟังก์ชันไหน ด้วยอาร์กิวเมนต์อะไร มีใครบวก หรือคูณ หรือทอยลูกเต๋าห้าหน้า
...
บทสนทนาที่บอกว่ามีการขออะไร ฟังก์ชันไหน ด้วยอาร์กิวเมนต์อะไร ลบมันทิ้งแล้วผลลัพธ์
...
จะตอบกลับด้วย HTTP 400 และ body หน้าตาแบบนี้
...
เนื่องจาก `llm.py` เรียก `response.raise_for_status()` สิ่งนั้นจะมาถึง terminal ของคุณ
...
### ทำไมเรื่องนี้จะย้อนกลับมากัดคนใน part 3
...
บทเรื่อง context management ใน part 3 และเมื่อคุณไปถึงตรงนั้น กฎการจับคู่ที่คุณ
...
ลองครั้งที่สามโดยสะกดอาร์กิวเมนต์ต่างออกไปนิดหน่อย แล้วก็วนแบบนี้ไปตลอดกาล
...
รอบที่ห้าสิบส่ง tool call และสตริง error ที่สะสมมาห้าสิบรอบ ดังนั้นค่าใช้จ่ายต่อเทิร์น
...
- **การตรวจจับ call ที่ซ้ำ** ทำ hash ของชื่อ tool รวมกับอาร์กิวเมนต์ของมัน ถ้า call
...
ทั้งหมดนั้นเป็นเรื่องของ part 3 ในบทว่าด้วยความน่าเชื่อถือและการควบคุม มันไม่ควรอยู่ใน
...
นี่คือไฟล์ทั้งหมด อ่านมันหนึ่งรอบจากต้นจนจบ แล้วค่อยอ่านคำอธิบาย
...
อาร์กิวเมนต์ถูกแปลงเป็น dict ของ Python จริง ๆ แล้วด้วย `json.loads`
...
model ตอบเป็นถ้อยคำแล้ว ดังนั้นถ้อยคำนั้นคือคำตอบ และเราส่งมันกลับไป
...
บทที่ 03 มีหัวข้อชื่อ "The JSON string that surprises everyone" นี่คือความประหลาดใจ
...
ดังนั้นเมื่อ `agent.py` เห็น call `call["arguments"]` จึงเป็น dict จริงอย่าง
...
แก้ไขอาร์กิวเมนต์ก่อนที่มันจะถูกบันทึก การลบความลับออกจาก tool call ที่ถูก log ไว้
...
เพราะมันแสดงอาร์กิวเมนต์ที่ model เลือกเป๊ะ ๆ และสตริงที่มันได้กลับมาเป๊ะ ๆ จง print
...
คำขอที่มีโครงสร้างซึ่งระบุชื่อฟังก์ชันและเติมอาร์กิวเมนต์ทั้งสองตัว จากนั้น Python ของเรา
...
เรียก `complete` ตรวจ `calls[0]` ตรวจสอบชื่อ ตรวจสอบอาร์กิวเมนต์ แล้วรัน tool ด้วย
...
03 หัวข้อ 8 และ model จริงจะไม่สนใจมัน
...
ไม่ได้ การจำแนกประเภทนั้นและการจัดตารางงานที่ตามมา เป็นเนื้อหาของ part 4 ซึ่งครอบคลุม
