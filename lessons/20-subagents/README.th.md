...
ไม่มีอะไรใน part 3 แก้เรื่องนี้ได้ และควรพูดให้ชัดว่าทำไม เพราะแต่ละชิ้นดูเหมือน
น่าจะแก้ได้
...
สามบรรทัดที่มีความหมาย สร้าง child เรียกมันด้วยสตริง คืนสตริงที่มันผลิตออกมา
...
แต่คือมันตกอยู่ฝั่งไหนของเส้นนั้น เพราะความสามารถที่แสดงเป็น tool ได้ทำให้คุณเสีย
ไฟล์หนึ่งไฟล์ ส่วนความสามารถที่ทำไม่ได้ทำให้คุณเสียพารามิเตอร์บนฟังก์ชันที่อันตราย
ที่สุดในโปรแกรม
...
เรียกที่ขอมาเป็นการมอบงานหรือไม่ งอกเส้นทาง dispatch แยกสำหรับ child
...
กับคุณฟรีอยู่แล้ว child ที่สร้างโดย `build_child` ได้ tool เท่าที่ตัว builder ลงทะเบียนให้
อย่าลงทะเบียน `run_subagent` ให้ child แล้ว child ก็ไม่มีทางสร้างหลานได้เลย
...
## 4. context isolation ซึ่งคือประเด็นทั้งหมด
...
จากนั้น parent ก็ไปทำอย่างอื่นโดยสิ้นเชิง
...
`made-by-child` ซึ่งเป็นชื่อไฟล์ที่ child เขียน และล้มเหลวถ้ามันปรากฏที่ไหนก็ตาม
...
child ไม่อยู่จริง ๆ แทนที่จะแค่บังเอิญไม่ถูกนับ

สี่กับสอง สองตัวเลขนั้นคือบทนี้

ทีนี้ขยายกลับขึ้นไปหาคำถามเรื่อง retry จากหัวข้อ 1 การอ่านไฟล์สิบห้าครั้งกับ
การค้นหาอีกไม่กี่ครั้งเกิดขึ้นข้างใน child ซึ่งแปลว่ามันไปตกอยู่ในลิสต์ `messages`
ของ child ซึ่งเป็นตัวแปรท้องถิ่นในการเรียกฟังก์ชันที่คืนค่าแล้วจบ เมื่อ `run_subagent`
คืนค่า ลิสต์นั้นก็ไม่มีอะไรอ้างถึงและ Python ก็คืนหน่วยความจำ สิ่งที่ไปถึง parent
คือสตริงหนึ่งอัน

พูดผลที่ตามมาให้ชัด **parent ได้ข้อสรุป ไม่ได้การสืบสวน** บทสนทนาของ parent
โตขึ้นด้วย assistant message หนึ่งอันที่บรรจุการเรียก tool กับผลลัพธ์ tool หนึ่งอัน
ที่บรรจุสองประโยค ถ้าไม่มี child มันจะโตขึ้นด้วยเนื้อหาไฟล์สิบห้าไฟล์และผลลัพธ์ grep
...
มีต้นทุนเท่าขนาดของมันหนึ่งครั้งไม่ว่าใครจะอ่าน ความต่างคือไฟล์ที่ parent อ่าน
จะถูกจ่ายซ้ำในทุก request ที่ตามมาไปตลอดการรัน ส่วนไฟล์ที่ child อ่านจะถูกจ่าย
เฉพาะในช่วงชีวิตสั้น ๆ ของ child เอง ในการรันสี่สิบ turn นั่นคือความต่างระหว่าง
การจ่ายครั้งเดียวกับการจ่ายสามสิบห้าครั้ง

และ child คมกว่าที่ parent จะเป็นในงานเดียวกัน ด้วยเหตุผลเดียวกัน context ของมัน
...
อ่านบรรทัดนั้นให้ดีเพราะมันทำสองอย่าง `build_child()` สร้าง child ตัวใหม่
แล้ว `(task)` ก็รันมัน ตัวใหม่ทุกครั้งไม่มียกเว้น

ทางเลือกอีกแบบสั้นกว่าหนึ่งบรรทัดและเป็นสิ่งที่นึกออกทันที ส่ง child เข้าไป เก็บมันไว้
เรียกมันเมื่อ parent มอบงาน
...
สืบสวนสามเรื่องให้ child ที่คงอยู่ตลอด แล้วบทสนทนาของมันก็บรรจุทั้งสามเรื่อง
การมอบงานครั้งที่สามถูกตอบโดย agent ที่แบกบทสนทนาของสองครั้งแรกไว้
และ context ของ child ก็เต็มขึ้นในแบบเดียวกับที่ของ parent เคยเป็น คุณย้ายความรก
...
จะเริ่มจากตรงที่ครั้งแรกบังเอิญหยุดไว้** child ที่เพิ่งใช้สี่ turn สรุปว่า retry delay
...
ไม่ได้ ซึ่งเป็นสิ่งที่ทำให้ผลลัพธ์เทียบกันได้ และทำให้การมอบงานที่ล้มเหลวปลอดภัย
พอที่จะลองใหม่ได้เฉย ๆ บทที่ 21 ต้องใช้คุณสมบัตินี้และเขียนไม่ได้เลยถ้าไม่มีมัน
เพราะการรัน child สี่ตัวพร้อมกันบน thread ต้องการว่าไม่มีสองตัวไหนใช้ state ร่วมกัน

สังเกตด้วยว่า factory ไม่ได้ตัดสินอะไรบ้าง มันไม่มีความเห็นว่า child ใช้ provider ไหน
...
closure ที่ครอบ `children` ไว้ เพื่อให้ check ตรวจสอบได้ว่า child ทำอะไรไปบ้าง
...
ที่ขอให้ child แสดงขั้นตอนทีละขั้น ซึ่งจะทำลายจุดประสงค์ด้วยการดึงการสืบสวน
กลับเข้ามาใน context ของ parent ผ่านค่าที่คืนกลับมา
...
ดังนั้น parent ที่ไม่ถูกบอกจะเขียนงานแบบที่มันจะเขียนให้ตัวเอง

```text
task = "Find out where that delay comes from and check the other one too"
```

ทุกคำในนั้นอ้างถึงสิ่งที่ child มองไม่เห็น delay อันไหน อีกอันคืออันไหน child ได้รับ
...
**parent ที่เขียนงานคลุมเครือจะได้คำตอบคลุมเครือกลับมา** และมันได้มาด้วยน้ำเสียง
มั่นใจของการสืบสวนที่เสร็จสมบูรณ์ ซึ่งแย่กว่า error เพราะมันดูเหมือนผลลัพธ์

เมื่อถูกบอกเรื่อง isolation แล้ว parent จะเขียนแบบนี้แทน
...
## 7. child ที่ล้มเหลวต้องไม่ลาก parent ล้มไปด้วย
...
**ทำไม exception handler ถึงกว้างโดยตั้งใจ** `except Exception` เปล่า ๆ ปกติเป็น
...
ของ parent ที่ลึกมาสี่สิบ turn แล้วและมีทางเลือกที่ดีอยู่ในมือ

ซึ่งเป็นครึ่งที่สองของข้อโต้แย้ง รัศมีความเสียหายของการจับมากเกินไปตรงนี้คือผลลัพธ์
tool ที่ไม่ดีหนึ่งอัน และ parent อ่านผลลัพธ์ tool เป็นอาชีพอยู่แล้ว รัศมีความเสียหาย
ของการจับน้อยเกินไปคือการรันของ parent ทั้งหมด

**ทำไม error ถึงกลับมาเป็นข้อความแทนที่จะถูกโยน** นี่คือการตัดสินใจที่น่าสนใจกว่า
...
โยน แล้ว parent ก็ตาย มันไม่มีโอกาสตอบสนอง session ของมันจบ และอะไรก็ตาม
...
เพื่อตอบสนองโดยเฉพาะ parent อ่านว่าการมอบงานล้มเหลว และมันทำสิ่งที่มนุษย์
...
`FileNotFoundError` บอกว่า child ถูกชี้ไปยังที่ที่ไม่มีอยู่ และการทำซ้ำจะล้มเหลว
...
child ที่จบด้วยข้อความสุดท้ายว่างเปล่าไม่ใช่ exception ไม่มีอะไรถูกโยน
แต่การคืนสตริงว่างให้ parent ทำให้เกิดผลลัพธ์ tool ที่ไม่มีเนื้อหา และ model
ที่ได้รับผลลัพธ์ tool ว่างเปล่าต้องเดาว่า tool สำเร็จอย่างเงียบ ๆ คืนอะไรที่ไม่มีความหมาย
หรือพังไปแล้ว ประโยคที่บอกชัดเจนว่า child จบโดยไม่ได้พูดอะไรเป็นข้อเท็จจริง
ที่ parent ทำอะไรต่อได้ และมันมีต้นทุนแค่หกคำ
...
สี่ขั้นตอน เขียนไฟล์ที่มีเนื้อหาที่รู้อยู่แล้ว parent อ่านมันและเนื้อหาไปอยู่ใน `parent_saw`
ซึ่งแทน context ของ parent มอบงานที่เขียนทับไฟล์เดียวกัน อ่านดิสก์อีกครั้ง
...
อ่านบรรทัดสุดท้ายช้า ๆ **parent ยังเชื่อว่า `original` ขณะที่ไฟล์ตอนนี้เขียนว่า `changed`**

ไม่มี error ไม่มีคำเตือน ผลลัพธ์ tool ของ parent ที่มาจาก child บอกว่าการมอบงาน
สำเร็จ ซึ่งเป็นเรื่องจริง ไม่มีที่ไหนเลยในบทสนทนาของ parent ที่บ่งชี้ว่าไฟล์ที่มันอ่าน
ไปก่อนหน้านี้ไม่ใช่สิ่งที่มันอ่านอีกต่อไปแล้ว

และทีนี้ลองคิดว่า parent จะทำอะไรต่อ เพราะนั่นคือจุดที่ความเสียหายอยู่ มันถือ
...
ทีนี้สมมติว่า parent เรียก `write_file` แทน โดยประกอบไฟล์ขึ้นใหม่จากสิ่งที่มันจำได้
พร้อมการปรับปรุงหนึ่งอย่าง อันนั้นจะสำเร็จ และมันทำลายทุกอย่างที่ child ทำไป
อย่างเงียบ ๆ parent ไม่ได้สับสนเรื่องการแก้ที่มันกำลังทำ มันสับสนเรื่องไฟล์ที่มันกำลังแก้

ทีนี้ส่วนที่ทำให้เรื่องนี้เป็นหัวข้อแทนที่จะเป็นเชิงอรรถ

**isolation ที่ทำให้ subagent มีประโยชน์คือสิ่งเดียวกับที่ทำให้เกิดเรื่องนี้**

มันไม่ใช่ข้อเท็จจริงสองข้อ มันคือข้อเท็จจริงข้อเดียวที่มองจากสองด้าน หัวข้อ 4
ฉลองที่การอ่านไฟล์สิบห้าครั้งของ child ไม่ไปถึง parent หัวข้อนี้บ่นว่าการเขียนไฟล์
หนึ่งครั้งของ child ก็ไม่ไปถึง parent เหมือนกัน ไม่มีเวอร์ชันไหนของการออกแบบที่
ข้อแรกจริงและข้อสองเท็จ เพราะกลไกคือกลไกเดียวกัน child มีลิสต์ `messages`
ของตัวเอง ไม่มีอะไรในลิสต์ `messages` ของ parent ที่ child เขียน นั่นคือฟีเจอร์
...
บทสนทนาเดียวจากหัวข้อ 1 ขึ้นมาใหม่ ให้ parent อ่านทุกไฟล์ใหม่หลังทุกการมอบงาน
แล้วคุณก็ดึงเนื้อหาไฟล์กลับเข้ามาใน context ของ parent ซึ่งคือต้นทุนเดียวกับที่คุณ
...
**ให้ child รายงานว่ามันเปลี่ยนอะไร** นี่คือประโยคเดียวในงานที่ parent เขียน
...
ตอนนี้คำตอบสุดท้ายของ child เอง ซึ่งเป็นสิ่งเดียวที่ข้ามเส้นแบ่งไปได้ ก็พก
ข้อมูลที่ parent ต้องการไปด้วย ช่องทางนี้มีอยู่แล้ว ค่าที่คืนกลับจะไปจบใน context
ของ parent อยู่แล้ว ดังนั้นการใส่รายการการเปลี่ยนแปลงลงไปจึงมีต้นทุนหนึ่งบรรทัด
แทนที่จะเป็นทั้ง subsystem นี่ยังเป็นประโยคที่คุณใส่ไว้ถาวรใน system prompt
ของ child ผ่าน `build_child` ได้ด้วย เพื่อให้ child ทุกตัวทำแบบนี้โดยที่ parent
ไม่ต้องจำ

**อ่านสิ่งที่ parent กำลังจะลงมือกับมันใหม่** ไม่ใช่ทุกอย่าง และไม่ใช่ตามตาราง
...
บทสนทนาที่สองทั้งชุด พร้อม system prompt ของตัวเองที่ถูกส่งในทุก turn ของ child
...
และทั้งหมดนั้นถูกจ่ายก่อนที่ child จะทำอะไรที่มีประโยชน์เลยสักอย่าง เพราะ
system prompt และ schema ออกไปตั้งแต่ request แรกสุดของ child
...
ฝั่ง parent เพื่อออกคำสั่งมอบงาน และ turn เพิ่มอีกหนึ่งเพื่ออ่านคำตอบ
...
ยังเพิ่ม latency เข้าไปอีก เพราะ parent นั่งถูกบล็อกอยู่ระหว่างที่ child วิ่งไปกลับ
ทีละรอบ

มีต้นทุนที่สองที่ไม่โผล่มาในรูปของ token ทุกอย่างที่ parent รู้ต้องถูกเขียนลงไปใน
...
และข้อที่สาม บทสนทนาของ child หายไปแล้ว ถ้าคำตอบของ parent ผิด การให้เหตุผล
ที่ผลิตส่วนที่ผิดอยู่ในลิสต์ `messages` ที่ถูกคืนหน่วยความจำไปตอนฟังก์ชันคืนค่า
`check.py` เห็นบทสนทนาของ child ได้ก็เพราะ `build_child` ถูกเขียนด้วย closure
...
ที่ผลิตสามประโยคไม่คุ้ม และอะไรก็ตามที่ parent ต้องใช้ผลลัพธ์ระหว่างทางเพื่อทำ
สิ่งถัดไปก็ไม่คุ้มเช่นกัน

มีอีกกรณีหนึ่งที่คำตอบคือไม่ ไม่ว่าขนาดเท่าไร อย่ามอบงานที่ parent ต้องใช้เหตุผล
กับขั้นตอนระหว่างทางของมัน ถ้า parent ต้องเปรียบเทียบไฟล์สองไฟล์ที่มันขอให้ child
ไปดู มันต้องการทั้งสองไฟล์ และ child ที่คืนบทสรุปของทั้งสองก็โยนสิ่งที่การเปรียบเทียบ
...
**หนึ่ง subagent เป็น tool ธรรมดาในสายตาของ parent** check อ่านชื่อออกจาก
...
**สอง child ทำงานจริงที่ไปถึงดิสก์** นี่คือนิสัยที่บทที่ 18 โต้แย้งไว้ยืดยาว
...
จึงผ่าน dispatch เดียวกับที่การรันจริงจะใช้ จากนั้นจึงตรวจดิสก์ child ที่บอกว่าเขียน
ไฟล์แล้วแต่ไม่ได้เขียนจะล้มเหลวตรงนี้ ซึ่งเป็นเหตุผลทั้งหมดที่ต้องยืนยันที่ side effect
แทนที่จะยืนยันที่ตัวหนังสือ

**สามกับสี่คือคู่จากหัวข้อ 4** สี่ข้อความใน child สองข้อความใน parent พร้อมการค้น
สตริงที่พิสูจน์ว่าไม่มีการรั่วไหล

**ห้า child ที่ระเบิดทิ้ง parent ให้ยังยืนอยู่ได้**
...
builder ที่คืน child ที่โยน exception ทันที ซึ่งเป็นไปได้ก็เพราะ builder ถูกฉีดเข้ามา
...
ผลลัพธ์ต้องขึ้นต้นด้วย `Error` และต้องยังพกข้อความของ child เองมาด้วย ครึ่งไหน
...
`Error` จะทำให้ parent แยกความล้มเหลวออกจากคำตอบไม่ได้ handler ที่จับ exception
แล้วโยนสิ่งที่มันพูดทิ้งจะทำให้ parent แยก timeout ที่ควรลองใหม่ ออกจาก error
...
ถ้าคำกล่าวอ้างข้อสามหรือข้อสี่ล้มเหลว แปลว่ามีอะไรกำลังแชร์ state ระหว่าง parent
กับ child และที่ที่ควรไปดูคือ `build_child` สร้าง agent ใหม่ต่อการเรียกหนึ่งครั้งจริงหรือไม่
...
HTTP request ที่บล็อก ดังนั้น parent ที่อยากได้การสืบสวนสามเรื่องจะออกการเรียก
...
วางไว้แล้วว่าการมอบงานทุกครั้งสร้าง agent สดขึ้นมา และไม่มี child สองตัวไหน
แชร์อะไรกัน นั่นคือเงื่อนไขเบื้องต้นของการรันพวกมันพร้อมกันเป๊ะ และมันถูกซื้อมา
ในบทนี้ด้วยเหตุผลที่ต่างกันโดยสิ้นเชิง

**บทที่ 21 รูปแบบ multi agent** orchestrator กับ worker ขนานบน thread และ queue
...
ล้าสมัยไม่ได้มีแค่ของ parent และสิ่งที่เปลี่ยนอยู่ใต้เท้าคุณอาจยังเปลี่ยนอยู่
