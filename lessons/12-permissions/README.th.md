...
Part 2 จบลงด้วย agent ที่ค้นหาโค้ดที่ไม่เคยมีใครบอกมันไว้ก่อนได้ อ่านโค้ดนั้นได้
แก้ได้หนึ่งบรรทัด และรันคำสั่งเพื่อตรวจผลการแก้ได้ บทนี้ไม่ได้ทำให้มันเก่งขึ้นในเรื่องพวกนั้น
...
ไฟล์ Python สามในหกไฟล์เหมือนเดิมทุกไบต์กับบทก่อนหน้า มีไฟล์ใหม่หนึ่งไฟล์
และมี branch ใหม่หนึ่งอันใน loop

## 1. ยินดีต้อนรับสู่ part 3

Part 3 คือเรื่องของ harness และสิ่งแรกที่ต้องจัดการคือนิยามของคำ

**agent** คือ loop มันคือสิ่งที่อยู่ใน `agent.py` ที่ถามโมเดลว่าจะทำอะไร รัน tool ที่โมเดลขอมา
ป้อนผลลัพธ์กลับไป แล้วถามอีกครั้ง คุณสร้างมันในบทที่ 04 และคุณแบกโค้ดยี่สิบบรรทัดชุดเดิมนี้มาตลอด
loop นั้นคือเรื่องราวทั้งหมดของความฉลาด ถ้าให้โมเดลดีและ tool ดี มันจะหาบั๊กของคุณเจอ

**harness** คือทุกอย่างที่ทำให้ loop นั้นอยู่รอดได้เมื่อถูกใช้มากกว่าหนึ่งครั้ง โดยคนที่ไม่ใช่คนเขียนมัน
...
ในบทที่ 14 มันโตขึ้นหนึ่งการเรียกฟังก์ชันที่ตัดสินใจว่าจะทิ้งอะไร บทที่ 11 วัดคุณสมบัตินี้ด้วย hash
และพบว่า `agent.py` เหมือนกันเป๊ะข้ามสี่บทของการสร้าง tool Part 3 ไม่ทำลายสิ่งนั้น
และเวลาที่มันแก้ loop จริง การแก้นั้นเล็กพอที่จะยกมาอ้างได้ในย่อหน้าเดียว

ถ้าคุณคาดหวังว่า part 3 จะยากกว่าเพราะมันมาทีหลัง ปรับความคาดหวังใหม่ มันไม่ได้ยากกว่า
มันเป็นงานคนละแบบ Part 2 ถามว่า agent ทำอะไรได้บ้าง Part 3 ถามว่าจะเกิดอะไรขึ้น
เมื่อมันทำสิ่งนั้นในบ่ายวันอังคาร บนแล็ปท็อปของคนอื่น ตอนที่เน็ตกระตุก

## 2. ปัญหาที่ค้างมาจาก part 2

นี่คือเรื่องราวความปลอดภัยทั้งหมดของบทที่ 08 ซึ่งยังนั่งอยู่ท้ายไฟล์ `tools.py` ในโฟลเดอร์นี้
...
อาจเป็นสิ่งที่โมเดลได้มาจากคำแนะนำเก่า หรือจากไฟล์ที่มันอ่าน หรือจากการเข้าใจผิดเรื่องโครงสร้างโปรเจกต์
...
`confirm` อยู่ ใครก็ตามที่อ่าน `tools.py` เห็นมนุษย์อยู่ใน loop แต่มนุษย์คนนั้นเป็นแค่ตรายาง
...
สี่ในเจ็ด tool เปลี่ยนอะไรไม่ได้เลย การลิสต์ไดเรกทอรี การอ่านไฟล์ การจับคู่ glob
...
สี่ครั้ง โดยสามครั้งเป็นการอ่าน ในงานจริงสัดส่วนเอียงกว่านี้มาก เพราะการหาที่ที่ถูกต้องคืองานส่วนใหญ่
...
**ทำไมใช้ set แทนที่จะเป็น flag บน tool แต่ละตัว** ทางเลือกอีกทางคือทำเครื่องหมายว่า tool
แต่ละตัวปลอดภัยหรือไม่ปลอดภัยใน schema ของมัน ถัดจากคำอธิบาย นั่นอ่านแล้วดูดี
และมันวางคุณสมบัติไว้ติดกับสิ่งที่มันอธิบาย แต่มันเป็นที่ที่ผิด ด้วยเหตุผลข้อเดียว `SCHEMAS`
คือข้อมูลที่ถูก serialise แล้วส่งไปหาโมเดลในทุก request ฟิลด์ `"safe": true`
ไม่ก็ไปตามสายซึ่งมันจะกลายเป็นคำใบ้ที่โมเดลใช้คิดได้ และดังนั้นจึงเป็นสิ่งที่ผู้โจมตีเถียงด้วยได้
หรือไม่ก็ต้องถูกตัดออกก่อนส่ง ซึ่งแปลว่า schema ไม่ใช่ schema อีกต่อไป การเก็บลิสต์ของสิ่งที่ปลอดภัยไว้ใน
`permissions.py` ทำให้การตัดสินใจเรื่องความปลอดภัยอยู่ฝั่งของสายที่โมเดลเอื้อมไม่ถึง

**ข้อจำกัดที่ต้องพูดตามตรงหนึ่งข้อ** ปลอดภัยในที่นี้แปลว่าเปลี่ยนอะไรไม่ได้
...
**Yes once** แปลว่าตอนนี้โอเค และฉันอยากเห็นมันอีก ใช้กับอะไรก็ตามที่มี side effect
ที่คุณยังไม่มั่นใจเต็มร้อย เช่น migration สคริปต์ deploy หรืออะไรที่แตะไฟล์ที่คุณแคร์
ประตูยังพร้อมทำงานอยู่

**Always for this exact call** แปลว่าฉันคิดเรื่องนี้เฉพาะเรื่องนี้แล้ว มันโอเค
...
**No** คือค่า default ซึ่งเป็นเหตุผลที่มันเป็นตัวใหญ่ใน prompt
...
ประกาศ `path` ก่อน โมเดลปล่อย key ของ JSON object ออกมาตามลำดับที่มันสร้าง
และลำดับนั้นต่างกันระหว่างการรันของโมเดลเดียวกันในงานเดียวกัน ถ้าไม่เรียงลำดับ
...
`tools.py` ในโฟลเดอร์นี้เหมือนเดิมจากบทที่ 09 ซึ่งแปลว่า `confirm` ยังอยู่ในนั้นและ `run_shell`
ยังเรียกมันอยู่ ประตูย้ายขึ้นไปอยู่ใน loop แล้ว ดังนั้นการรันแบบ interactive
โดยไม่จัดการอะไรกับเรื่องนี้จะทำให้คุณถูกถามสองครั้งสำหรับคำสั่งเดียวกัน ครั้งหนึ่งโดย `Permissions`
และอีกครั้งโดย `confirm` ตั้ง `AGENTPATH_AUTO_APPROVE=1` เพื่อปิดปากอันล่าง หรือลบ `confirm`
กับการเรียกมันทิ้ง มันถูกทิ้งไว้ตรงนี้เพื่อให้ `tools.py` เหมือนกันทุกไบต์ตลอด part 2
และคุณจะได้เห็นด้วยตาตัวเองว่าการเปลี่ยนแปลงอยู่ในไฟล์ไหนจริง ๆ ซึ่งคือประเด็นที่หัวข้อ 1 พูดถึงเรื่อง loop
...
ไม่ว่าจะมีอะไรอยู่ข้างในก็ตรงหมด อีกหกเทิร์นถัดมาโมเดลอ่านไฟล์หนึ่ง เข้าใจสิ่งที่เจอผิด แล้วปล่อยสิ่งนี้ออกมา
...
แล้วสลับฟังก์ชัน `ask` เป็นอันที่ปฏิเสธทุกอย่างและบันทึกว่ามันถูกเรียก จากจุดนั้นเป็นต้นไป
...
**สิ่งนี้มีต้นทุนอะไร และทำไมต้นทุนนั้นจึงถูกต้อง** การจับคู่แบบเป๊ะแปลว่า
...
และ `*` ไม่รู้ว่าเซมิโคลอนแปลว่าอะไร การทำเรื่องนั้นให้ถูกต้องต้อง parse คำสั่ง ซึ่งต้องรู้ว่า shell ไหน
ซึ่งต้องถูกต้องเรื่อง quoting กับ substitution กับ backtick การจับคู่แบบเป๊ะคือเวอร์ชันที่ไม่มีรูรั่วแบบนั้น
...
สมมติเดือนหน้าคุณเพิ่ม `http_fetch` หรือ `delete_file` หรือ `git_commit` คุณเขียนฟังก์ชัน
...
มีสองอย่างเกิดขึ้น บรรทัดหนึ่งถูกพิมพ์ให้คุณ และข้อความหนึ่งถูกต่อท้ายบทสนทนาให้โมเดล
...
ที่ไม่มีผลลัพธ์ และ request ถัดไปจะถูกปฏิเสธด้วย `400` ก่อนที่โมเดลจะเห็นอะไรเลย

สมมติคุณเลี่ยงเรื่องนั้นด้วยการต่อท้ายด้วย string ว่าง ตอนนี้ request ถูกต้องแล้ว
และโมเดลกำลังมอง tool result ที่ไม่พูดอะไร ในมุมของมัน คำสั่งไม่ได้ผลิต output อะไรออกมา
...
สิ่งที่ข้อความนั้นทำคือคืนความสามารถในการเลือกให้โมเดล
...
เข้าไปในบทสนทนา ตอนนี้โมเดลรู้สามข้อเท็จจริง การเรียกไม่ได้รัน มีคนปฏิเสธมัน
...
ตัดหน้าพฤติกรรมที่พบบ่อยที่สุดของโมเดลหลังความล้มเหลว ซึ่งคือทำการกระทำที่ล้มเหลวซ้ำโดยเปลี่ยนนิดหน่อย
**Do something else** ให้ที่ไปกับมัน เพราะโมเดลที่ถูกบอกแค่ว่าห้ามทำอะไร
...
การปฏิเสธที่เล็งไปที่โมเดลคืองานเขียนชิ้นหนึ่ง และหน้าที่ของมันคือทิ้งโมเดลไว้ในตำแหน่งที่ทำสิ่งที่ถูกต้องได้
...
โมเดลอ่านลงมาตลอดทั้งบทสนทนา และคำสั่งถูกจดจำจากสิ่งที่มันฟังดูเป็น เพราะไม่มีช่องทางอื่นให้ใช้
...
และตอนนี้บทสนทนามีประโยคที่พูดกับโมเดลว่าให้รันคำสั่ง มันเขียนมาอย่างดี มันสุภาพ
...
ทั้งสองแข่งกันเพื่อความสนใจเดียวกัน ในสื่อเดียวกัน โดยไม่มีกลไกไหนในโมเดลที่จัดอันดับอันหนึ่งเหนืออีกอันตามแหล่งที่มา
...
นี่ไม่ใช่การอ้างว่าการเขียน prompt ไร้ประโยชน์ system prompt ที่ดีลดความถี่ที่โมเดลจะติดกับดักได้จริงและวัดได้
...
ย้อนกลับไปที่การรันที่โมเดลติดกับดัก มีบางอย่างหยุดไม่ให้คำสั่งนั้นรัน และควรพูดให้ชัดว่าคืออะไร
...
ข้อความที่ถูกฝังพูดกับโมเดลได้ มันพูดกับ `Permissions.check` ไม่ได้ เพราะ `check` ไม่ได้อ่านข้อความ
มันกำลังเทียบ string กับ set และเรียกฟังก์ชันที่คุยกับ terminal
...
ดังนั้นข้อความที่ชักจูงให้โมเดลอ่านไฟล์อื่นแล้วใส่เนื้อหาลงในสรุปจะไม่เจอประตูใดเลย
...
ซึ่งจำกัดรัศมีการระเบิด บทที่ 13 ทำให้การตัดสินใจอยู่ถาวร และนั่นคือบทที่ควรกลับมาถามคำถามนี้อีกครั้ง
...
ระหว่างโมเดลบน provider เดียวกัน และระหว่างเส้นทาง streaming กับไม่ streaming ของ client library ตัวเดียวกัน
...
สิ่งที่เห็นจากภายนอกแย่กว่าการ crash เพราะมันไปได้สองทางและทั้งสองทางแย่ ไม่ผู้ใช้ถูกถามครั้งที่สอง
สำหรับการเรียกที่เขาเพิ่งอนุมัติ ซึ่งสับสนและพาคุณกลับเข้าสู่ความเหนื่อยล้าที่บทนี้ทั้งบทพูดถึง
ก็ harness ถือว่าการอนุมัติที่หายไปเป็น internal error แล้วถอยไปใช้ค่าที่ยอมง่าย
...
**บรรทัดที่หนึ่ง** การเรียก `read_file` คืน `True` และฟังก์ชัน `ask` ที่คอยบันทึกไม่เคยถูกเรียก
ทั้งสองครึ่งถูกยืนยัน เพราะเวอร์ชันที่ถามแล้วอนุมัติจะผ่านครึ่งแรกแต่พลาดประเด็นทั้งหมด

**บรรทัดที่สอง** `run_shell` ที่มี `rm -rf /` ถูกปฏิเสธ และ `ask` ถูกเรียกหนึ่งครั้งพอดี
...
**บรรทัดที่สี่และบรรทัดที่ห้า** สองอันนี้คือคู่จากหัวข้อ 4 อนุมัติ `git status` ด้วย `ALLOW_ALWAYS`
แล้วแทนที่ `ask` ด้วยฟังก์ชันที่ปฏิเสธทุกอย่างและบันทึกว่ามันรัน การเรียกเดิมต้องผ่านโดยไม่ถาม
...
นั่นยืนยันว่าฟังก์ชันคืน `False` มันจริง และมันเกือบไร้ค่า เพราะการที่ `check` คืน `False`
...
เมื่อคุณเทสต์อะไรที่มีโมเดลภาษาหรือประตู permission อยู่ข้างใน ให้หา side effect
...
**บทที่ 13 sessions** บทสนทนาถูกเขียนลงไฟล์ JSONL ขณะที่มันเกิดขึ้น หนึ่งข้อความต่อหนึ่งบรรทัด
...
ชี้ agent ไปที่มันด้วยคำถามธรรมดา แล้วทำให้โมเดลติดกับดักอย่างน้อยหนึ่งครั้ง โมเดลเล็ก ๆ ในเครื่องจะยินดีทำ
