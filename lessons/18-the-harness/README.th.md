...
ก็จะทำงานจนจบ เพราะ `subprocess.run` บล็อกจนกว่ากระบวนการจะจบ
...
ลองไล่ดูว่า callback ของคุณต้องข้ามอะไรบ้างกว่าจะถึงคน `open_stream` ถูกเรียกโดย `stream`
ซึ่งถูกเรียกโดย `run` ซึ่งถูกเรียกโดย `main` ทุกชั้นในนั้นต้องแบก callback ที่มันไม่ได้ใช้
...
ดังนั้นจึงไม่มีที่ข้างในมันให้ตรวจธง คุณจะต้องใช้ `subprocess.Popen` ลูปที่ poll ด้วย timeout
สั้น ๆ ขณะที่ตรวจ `CANCELLATION` และ `terminate` ตามด้วย `kill`
เมื่อคำขออย่างสุภาพถูกเพิกเฉย
