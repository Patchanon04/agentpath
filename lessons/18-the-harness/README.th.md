...
# บทที่ 18 หมุดหมาย The harness (โค้ดรอบ model ที่ทำทุกอย่างที่ model ทำเองไม่ได้)
...
สิบสองไฟล์จากสิบสี่ไฟล์ Python เหมือนเดิมทุกไบต์กับที่เคยอยู่ในบทก่อนหน้า นั่นไม่ใช่คำกล่าวอ้าง
...
ลองใส่ตัวเลขให้ข้อสุดท้ายในห้าข้อนั้น เพื่อนร่วมงานคนหนึ่งรัน agent ของภาค 2 กับชุด test
ที่ fail อยู่ นั่งดูมันอ่านสี่ไฟล์ แก้สองไฟล์ และรันชุด test สองรอบ แล้วกดปุ่ม interrupt
ที่นาทีที่สิบเก้าเพราะมันกำลังแก้ module ผิดตัว สิ่งที่ได้กลับมาคือ `KeyboardInterrupt`
ออกมาจากกลางตัว parse stream บรรทัดที่พิมพ์ค้างครึ่งเดียวบนจอ
และไฟล์หนึ่งไฟล์ที่ถูกแก้ไปแล้วบนดิสก์ โดยไม่มีบันทึกที่ไหนเลยว่าไฟล์ไหน agent
ทำงานได้ตลอดสิบเก้านาทีนั้น สิ่งที่ยังไม่พร้อมคือทุกอย่างรอบตัวมัน ในวันที่มีคนอยู่ในห้องด้วย
...
ทุกขอบเขตคือการตัดสินใจที่อาจจะไปทางตรงกันข้ามก็ได้ หนึ่งเทิร์นวิ่งผ่านทุกตัวตามลำดับที่ตายตัว

```mermaid
flowchart TD
  A[main.py ประกอบชิ้นส่วน] --> B[agent.py หนึ่งเทิร์น]
  B --> C{permissions.check}
  C -- ไม่ --> D[ข้อความปฏิเสธกลับไปหา model]
  C -- ใช่ --> E[tools.run]
  E --> F[session.append]
  D --> F
  F --> G[fit_to_budget เลือกสิ่งที่ส่ง]
  G --> H[with_retries ห่อการเรียก]
  H --> I[provider.stream]
  I --> J[usage.add นับสิ่งที่กลับมา]
  J --> B
```
...
เมื่อบทที่ 22 รัน agent จาก CI โดยไม่มีใครนั่งอยู่หน้าคีย์บอร์ด tool ที่ถามคำถามของ
...
การต่อสายไฟทำเสร็จไปแล้ว และควรดูว่ามันอยู่ตรงไหน `providers.py` เรียก `with_retries`
ในฟังก์ชันเดียวเท่านั้น และ provider ทั้งสองตัวผ่านฟังก์ชันนั้น
...
ยังคงไม่รู้เรื่องสายส่งคือคุณสมบัติที่บทที่ 06 ซื้อมาและบทที่ 11 วัดผลไว้

สังเกตขอบเขตให้ดี สิ่งที่ถูกห่อคือการเปิด request ไม่ใช่การอ่าน stream
เมื่อไบต์แรกไปถึง `on_text` แล้ว ผู้เรียกได้เห็นคำตอบไปบางส่วนแล้ว
และการเล่น request ซ้ำจะพิมพ์คำตอบที่สองทับคำตอบแรก
ดังนั้น rate limit ที่เกิดก่อน response จะเริ่มจึงมองไม่เห็นจากฝั่งคุณ
ส่วนการเชื่อมต่อที่ขาดกลางคันระหว่างคำตอบยาว ๆ จะกู้คืนไม่ได้เลย
ความไม่สมมาตรนั้นคือราคาของ streaming และบทที่ 17 ได้ถกมันไว้แล้ว
...
เวอร์ชันนั้นมีใบเสร็จที่อ่านได้ การเพิ่ม cancellation เข้าไปแปลว่าต้องวางการตรวจ
`stop_requested` ไว้ในฟังก์ชันที่มีเก้าสาขา และการตรวจนั้นลงไปแค่แปดสาขา
เพราะสาขาที่เก้าคือช่วง sleep ของ retry ที่มีคนเขียนไว้ตั้งแต่สามบทก่อนและไม่มีใครอ่านซ้ำ
กดปุ่ม interrupt ระหว่างช่วง backoff แล้วการรันก็เดินต่อไปอีกสามสิบสองวินาที
บั๊กไม่ได้อยู่ในโค้ดของ cancellation มันอยู่ที่ cancellation ไม่มีที่อยู่อื่นนอกจากในสาขาของคนอื่น
...
| การตรวจจับการทำซ้ำด้วย `loose_signature` `recent` `warned` และ `REPEAT_LIMIT` | 15 |
...
ทีนี้มาถึงไฟล์ใหม่ มันยาวหนึ่งร้อยสิบห้าบรรทัดและไม่มี logic ของ agent อยู่เลย การตัดสินใจห้าอย่าง
...
เมื่อ `WORKSPACE` เป็น `.` `run_shell` ส่ง `cwd=WORKSPACE` ให้ `subprocess.Popen` และ
...
`tools.CANCELLATION = cancellation` เป็นบรรทัดที่เล็กที่สุดในไฟล์
และเป็นหนึ่งในไม่กี่บรรทัดที่เปลี่ยนว่าโปรแกรมนี้ทำอะไรได้ `tools.py`
มีการตรวจนี้มาตั้งแต่บทที่ 17 โดยอ่าน `CANCELLATION` ระดับโมดูลที่บทที่ 17
ไม่เคยตั้งค่าให้ มันจึงเป็นการตรวจที่ถูกต้องแต่ไม่ได้ป้องกันอะไร
บรรทัดนี้คือบรรทัดที่ส่งค่าให้มัน

มันเป็นการกำหนดค่าให้ attribute ของโมดูลแทนที่จะเป็น parameter
เพราะทางเลือกอีกทางคือการร้อย token ผ่าน `tools.run` ผ่านลายเซ็นของทุก tool
และผ่านผู้เรียกของทุก tool เพื่อไปให้ถึง tool เดียวที่ต้องใช้มัน
คอมเมนต์เหนือมันคือข้อจำกัดที่ทำให้ attribute ระดับโมดูลปลอดภัย มี token อยู่ตัวเดียว
loop กับ shell tool อ่าน object เดียวกัน
และ flag ตัวที่สองที่อยู่ที่อื่นก็จะเป็นสิ่งที่สองที่ลืมตรวจได้

จากนั้นคือ handler กดสองครั้ง พฤติกรรมสองแบบ และนั่นคือการออกแบบทั้งหมด
...
ทีนี้ลองเทียบ docstring ใน `cancel.py` กับสิ่งที่โปรแกรมทำจริง
...
ทั้งสองครึ่งเป็นจริงที่นี่ และครึ่งหลังเป็นจริงเพราะสองบรรทัดที่อยู่เหนือ handler นั่นเอง loop
...
ส่วน `run_shell` ตรวจหนึ่งครั้ง ก่อนที่มันจะ spawn อะไรก็ตาม
...
การตรวจนั้นอยู่ใน `tools.py` มาตั้งแต่บทที่ 17 และไม่ได้ทำอะไรเลยที่นั่น
เพราะบทที่ 17 ไม่เคยกำหนดค่าให้ `tools.CANCELLATION` `main.py`
คือที่ที่มันเริ่มทำงาน และเป็นเหตุผลที่สองบรรทัดนั้นไม่ใช่ import ที่หลงเข้ามา

ช่องโหว่ที่เหลืออยู่แคบลงและยังเป็นเรื่องจริง ทุกการตรวจคือการตรวจก่อนบางอย่างจะเริ่ม
กด Ctrl+C หนึ่งครั้งขณะที่ `run_shell` ที่กินเวลาหกสิบวินาทีกำลังทำงานอยู่แล้ว subprocess
ก็จะทำงานจนจบ เพราะ `communicate` บล็อกจนกว่ากระบวนการจะจบหรือ timeout จะมาถึง
และไม่มีที่ข้างในมันให้มองดู flag ได้เลย การหยุดจะมีผลหลังจากนั้น ก่อนการเรียกครั้งถัดไป
แบบฝึกหัดข้อสี่ในหัวข้อที่ 8 ว่าด้วยการปิดช่องโหว่นั้น
...
`raise SystemExit(main())` ที่ก้นไฟล์ให้ `main` คืนค่า exit code แทนที่จะเรียก `sys.exit`
จากข้างในตัวมัน `main` จึงยังเป็นฟังก์ชันธรรมดาที่โปรแกรมอื่นเรียกได้โดย interpreter ไม่ตาย
...
`python` ไม่อยู่บน `PATH` ข้างใน shell ที่ `subprocess.Popen` ใช้ การเรียกครั้งที่สี่นั้นจะกลับมา
แบบนี้
...
เลขของ tool ตัวที่เก้าแย่กว่าที่เห็น tool สำหรับ query ฐานข้อมูล Postgres แบบอ่านอย่างเดียว
ตัวหนึ่งออกมาราวหนึ่งร้อยยี่สิบบรรทัด เมื่อเขียน schema รายการ dispatch timeout ข้อความ error
และตัวตรวจครบแล้ว และมันกินเวลาไปหนึ่งบ่าย อีกหนึ่งสัปดาห์ต่อมา ทีมฐานข้อมูลก็ชี้ไปที่ server
ที่พวกเขารันอยู่แล้ว ตัวที่เปิด query เดียวกันนั้นผ่าน pipe ผ่านการทดสอบมาแล้ว
และจัดการ connection string ไว้ให้ฝั่งเขาเอง บ่ายนั้นไม่ได้หมดไปกับส่วนที่ยาก
มันหมดไปกับการสร้างของที่ยืมมาใช้ไม่ได้ขึ้นมาใหม่ เพราะการออกแบบนี้ไม่มีช่องให้ยืม

บทที่ 19 สร้าง MCP client ซึ่งเป็น protocol ที่ tool อาศัยอยู่ใน process
แยกและถูกอธิบายผ่าน pipe ดังนั้น tool จึงกลายเป็นสิ่งที่คุณเชื่อมต่อไป
แทนที่จะเป็นสิ่งที่คุณเขียน คุณเขียน client เองแบบ
...
หน้าตาของมันในบ่ายจริงเป็นแบบนี้ การรันเพื่อถอด flag ที่เลิกใช้แล้วออกจาก service ตัวหนึ่ง
เดินมาถึงเทิร์นที่สามสิบเอ็ด บน budget หนึ่งแสนสองหมื่น token และ `fit_to_budget`
ก็ทิ้งบล็อกที่เก่าที่สุดสี่บล็อก หนึ่งในนั้นคือบล็อกที่บอกว่า flag ตัวนั้นต้องยังทำงานให้ผู้เรียกเดิมได้
จนถึงรุ่นหน้า agent ทำงานจบอย่างเรียบร้อยและลบเส้นทางที่รองรับของเดิมทิ้งไป
ทุกการตรวจผ่านหมด ไฟล์ session ครบถ้วน และไม่มีอะไรในบทบันทึกบอกว่าประโยคนั้นเคยอยู่ตรงนั้น
...
บทที่ 20 ว่าด้วย subagent คือ agent ที่เริ่ม agent อีกตัวได้ด้วย context สดของตัวเอง
ยื่นงานแคบ ๆ ชิ้นเดียวให้มัน แล้วรับคำตอบกลับมาแทนที่จะเป็นบทบันทึกทั้งเส้น
บทสนทนาของตัวแม่โตขึ้นด้วยผลลัพธ์สั้น ๆ อันเดียว แทนที่จะโตด้วยการเรียก tool สี่สิบครั้ง
บทนั้นยังมีรูปแบบความล้มเหลวที่แถมมาด้วย ซึ่งก็คือตัวแม่และลูกตอนนี้เห็นโลกคนละเวอร์ชัน
ลูกแก้ไฟล์ไปแล้ว ตัวแม่ยังให้เหตุผลจากสิ่งที่มันอ่าน
ไว้ก่อนหน้า และการแยก context ของลูกออกมา ซึ่งคือสิ่งที่ทำให้มันคมชัด ก็คือสิ่งเดียวกับที่ซ้ำเติมเรื่องนี้
พอดี

บทที่ 21 ว่าด้วยรูปแบบการทำงานแบบ multi agent คือ orchestrator กับ worker
ที่ทำงานขนานกันบน
thread และ queue ซึ่งเป็นที่ที่คุณจะได้รู้ว่าส่วนไหนของ harness ของคุณแอบสมมติว่ามี agent เดียว
ลำดับคือสิ่งที่บทนั้นแกะออกมาดู เพราะงานสามงานที่เขียนลง stream เดียวกันจะสลับ output กัน
และแต่ละงานก็ยังต้องรักษาลำดับ event ของตัวเองไว้ให้ได้

ไฟล์ session คือข้อสมมติที่บทนั้นไม่ได้แกะ และควรรู้ไว้ก่อนคุณจะหยิบ `Session` ไปใช้ใน worker
`session.py` บอกใน docstring ของตัวเองว่ามันรองรับผู้เขียนคนเดียว
และผู้เขียนสองคนที่เขียนต่อท้ายไฟล์เดียวกันจะสลับบรรทัดกันแล้วทำให้มันเสียหาย
หัวข้อ 8 ของบทที่ 13 คือที่ที่ขีดจำกัดนั้นถูกถกไว้ รวมถึงว่าการแก้มันมีต้นทุนเท่าไร
บทที่ 21 หลบมันมากกว่าจะแก้มัน
...
การเรียบเรียงคำแบบเดียวคือจุดที่เรื่องนี้เริ่มแพง เพิ่มประโยคเรื่องรัน test เข้าไป
ลองกับ repository สองตัวที่เรียก `pytest` ได้ทั้งคู่ นั่งดูมันรัน test ทั้งสองครั้ง
แล้วเก็บประโยคนั้นไว้ อีกสามสัปดาห์ต่อมา agent ตัวเดิมถูกชี้ไปที่ repository ที่ test
รันผ่าน make target และมันก็เผาไปหกเทิร์นกับหนึ่งหมื่นหนึ่งพัน token
ตามหาคำสั่ง test ที่ไม่มีอยู่จริง เพราะประโยคนั้นสั่งให้มันรัน test เสมอ
และไม่เคยบอกว่าต้องทำอย่างไรเมื่อหาไม่เจอ สองการรันตอบว่าใช่ ไม่มีการรันไหนถามคำถามที่สำคัญ
...
บทที่ 22 ว่าด้วย eval และการเลือก model มันมีตัวรันงานที่รันชุดงานตายตัวแล้วตรวจผลลัพธ์
และ LLM ในฐานะผู้ตัดสินสำหรับผลลัพธ์ที่ตรวจด้วยเครื่องไม่ได้ mock server ทำให้กลไกทั้งหมดนี้ทดสอบได้ฟรี
...
เรื่องแรกคือการตรวจสอบไปอยู่ตรงไหนในลำดับ วาง `read_only` ไว้เหนือการตรวจ `SAFE_TOOLS`
แล้วการ
...
เรื่องที่สองคือ model ถูกบอกอะไร loop ส่งประโยคที่บอกว่าผู้ใช้ปฏิเสธกลับไปอยู่แล้ว ประโยคนั้นผิดในกรณีนี้
...
เรื่องที่สามคือ model ควรรู้ก่อนที่มันจะลองหรือไม่
อีกวิธีหนึ่งคือไม่ส่ง schema ของ tool ที่เขียนไปเลยในโหมด
...
เรื่องที่สี่คือคุณจะพิสูจน์มันอย่างไร เขียนการตรวจสอบก่อนเขียนโค้ด มันควรหน้าตาเหมือนข้ออ้างที่ห้าใน
...
ใช้นาฬิกาคนละตัวได้คำตอบคนละอย่าง ใช้ `time.time()`
แล้วคุณจะได้ตัวเลขนาฬิกาผนังที่อ่านเป็นวันที่ได้ ซึ่งเคลื่อนคลาด
...
ขอบเขตก็ไม่ชัดเจนเช่นกัน ปัจจุบัน `Session` ไม่รู้อะไรเกี่ยวกับเทิร์นเลย มันรับข้อความ ถ้าคุณอยากได้
...
จากนั้นก็พามันไปให้ถึงหน้าจอ ซึ่งคือครึ่งหลังและเป็นส่วนที่ยากจริง การ retry เกิดขึ้นใน
`open_stream` อยู่แล้ว ลึกลงไปสี่ชั้นจากอะไรก็ตามที่เคยพิมพ์ตัวอักษรออกมา
ลองไล่ดูว่า callback ของคุณต้องข้ามอะไรบ้างกว่าจะถึงคน `open_stream` ถูกเรียกโดย `stream` `stream` ถูกเรียกโดย `run` และ `run` ถูกเรียกโดย `main` ทุกชั้นในนั้นต้องแบก callback ที่มันไม่ได้ใช้
เพื่อข้อความที่มันไม่ได้รับอนุญาตให้พิมพ์

นั่นคือคำถามเรื่องรอยต่อ และมีคำตอบสามแบบที่มีต้นทุนต่างกัน ร้อย `on_retry`
ผ่านทุกลายเซ็น ซึ่งซื่อสัตย์และทำให้สี่ฟังก์ชันกว้างขึ้น วางมันไว้บน provider เป็น argument
ของ constructor เพื่อให้มีแค่ `main.py` กับ `providers.py` ที่รู้เรื่อง
โดยแลกกับการมีนโยบาย retry หนึ่งอันต่อ provider หนึ่ง instance หรือเอา `on_text`
มาใช้ซ้ำ โดยพิมพ์เวลารอออกไปราวกับว่ามันเป็น output ของ model
ซึ่งไม่มีต้นทุนเลยและเป็นสิ่งที่ผิด เพราะมันใส่คำที่ model ไม่เคยพูดลงไปในไฟล์ session
เลือกมาหนึ่งอย่างและอธิบายให้ได้ว่าอีกสองอย่างมีต้นทุนอะไร

จากนั้นตัดสินใจว่าข้อความจะบอกอะไร `attempt`, `attempts` กับ `wait`
เป็นตัวเลขที่ผู้เรียกมีที่ใช้อยู่แล้ว ส่วน `error` เป็น exception ของ `httpx`
และการส่งมันออกไปแปลว่าตอนนี้ terminal ต้องรู้วิธีแสดงมัน หรือไม่ `with_retries`
ก็ต้องแปลงมันเป็นสตริงก่อน แล้วตัดสินใจว่าคนคนหนึ่งต้องการรายละเอียดมากแค่ไหน
...
ปิดครึ่งหนึ่งของช่องโหว่ cancellation ที่หัวข้อที่ 4 เปิดค้างไว้ `run_shell` ปรึกษา token
อยู่แล้ว และ `main.py` ก็ส่งค่าให้อยู่แล้ว คำสั่งที่ยังไม่เริ่มจึงจะไม่เริ่ม
ทีนี้ทำให้ Ctrl+C ระหว่างคำสั่งที่กินเวลาหกสิบวินาทีซึ่งกำลังทำงานอยู่แล้วหยุดมันได้จริง

อันนี้ยากกว่าที่เห็น และนั่นคือเหตุผลที่มันอยู่ตรงนี้ `communicate(timeout=SHELL_TIMEOUT)` บล็อกจนกว่า process จะจบหรือ timeout จะมาถึง
ดังนั้นจึงไม่มีที่ข้างในมันให้ตรวจธง ให้แทน `communicate` ครั้งเดียวด้วย loop ที่เรียก
`process.wait(timeout=0.2)` ขณะที่ตรวจ `CANCELLATION` แล้วส่ง process ให้ `_kill_tree`
ที่มีอยู่แล้วเมื่อธงถูกตั้ง

สามอย่างที่ทำให้มันเป็นแบบฝึกหัดเรื่องการออกแบบมากกว่าเรื่อง thread

อย่างแรกคือ model จะถูกบอกว่าอะไร tool ต้องคืนอะไรสักอย่าง คำสั่งที่ถูกฆ่ามี stdout เพียงบางส่วน
ไม่มี exit code ที่ควรค่าแก่การรายงาน และมีเหตุผลหนึ่งข้อ ถ้าคืน output บางส่วนไป model
อาจใช้ log ของการ build ที่หยุดกลางคันมาให้เหตุผลราวกับว่ามันสมบูรณ์ ถ้าคืนแค่ `Cancelled`
คุณก็ทิ้ง output ที่คนคนนั้นอาจต้องการไป ข้อความก่อนเริ่มที่มีอยู่คือ
`Cancelled before the command started.` และถ้อยคำมีความหมาย
เพราะสองกรณีนี้ไม่ใช่เหตุการณ์เดียวกัน และ model ไม่ควรต้องเดาว่าเกิดกรณีไหน

อย่างที่สองคือ `terminate` ไปถึงอะไรจริง ๆ `shell=True` แปลว่า process
ที่คุณถือ handle อยู่คือ shell
ไม่ใช่คำสั่งที่มันรัน ฆ่า shell แล้วลูกที่อยู่รอดต่อจากมันก็ยังทำงานต่อ
โดยยังถือไฟล์ที่มันกำลังเขียนอยู่ การจัดการทั้งต้นไม้ต้องใช้ process group บน Unix กับ job
object บน Windows และสองอย่างนั้นต่างกันมากพอที่คุณต้องตัดสินใจว่าคุณเขียนสิ่งนี้เพื่อ
แพลตฟอร์มเดียวหรือทั้งสอง

อย่างที่สามคือ attribute ระดับโมดูลจะรอดหรือไม่ `tools.CANCELLATION`
เป็นค่าเดียวสำหรับทั้ง process
ซึ่งถูกต้องพอดีสำหรับ agent หนึ่งตัวกับ terminal หนึ่งอัน บทที่ 21 รัน worker บน thread
ใน process เดียวกันนั้น ลองหาว่าการที่ worker ตัวที่สองสั่งยกเลิกจะส่งผลอะไรกับตัวแรก
ภายใต้การออกแบบนี้ และคุณต้องเปลี่ยนอะไรบ้างเพื่อให้คำตอบคือไม่ส่งผลอะไรเลย
