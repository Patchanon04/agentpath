...
จากนั้นบวกสี่ต่อข้อความสำหรับ overhead นั่นคือโมเดลทั้งหมด `budget` คือจำนวนเต็ม
...
**นี่คืออะไร** language model เป็น stateless มันไม่มีความทรงจำเกี่ยวกับ request
...
**ทำไมมันสำคัญตรงนี้** มันหมายความว่าราคาของเทิร์นที่ N ไม่ใช่ราคาของเนื้อหาใหม่
ในเทิร์นที่ N แต่คือราคาของเทิร์นที่หนึ่งถึง N ลบหนึ่ง บวกเนื้อหาใหม่ ทุกครั้งไป
คุณไม่ได้จ่ายค่าบทสนทนา คุณกำลังจ่ายค่าผลรวมของทุก prefix ของบทสนทนา
...
**ทำไมมันโตแบบนี้** แต่ละเทิร์นเพิ่มข้อความใหม่ในปริมาณที่ค่อนข้างคงที่
...
กับ tool schema เจ็ดตัวเป็น prefix คงที่ประมาณ 1400 token และมีการอ่านไฟล์
...
**ทำไมเราจะไม่แค่ทำให้บทสนทนาสั้นลง** นั่นคือบทที่ 14 และมันจำเป็น
...
**นี่คืออะไร** ทุก provider บอกคุณใน response ว่า request นั้นมีต้นทุนจริงเท่าไร
ไม่ใช่ค่าประมาณ แต่คือตัวเลขที่ระบบคิดเงินของเขาเองใช้ ผลิตโดย tokeniser ของเขาเอง
บนไบต์ที่คุณส่งไปเป๊ะ ๆ

**ทำไมเราถึงใช้มัน** มันเป็นตัวเลขเดียวในระบบที่ไม่ใช่การเดา ทุกจำนวนอื่นในโปรแกรมนี้
...
**ทำไมไม่คำนวณเอง** หัวข้อ 4 ว่าด้วยคำถามนั้นทั้งหัวข้อ และคำตอบแย่กว่าที่คุณคาด
...
`cache_creation_input_tokens` และ `cache_read_input_tokens` เมื่อมีการใช้ caching
...
**การแก้ควรอยู่ที่ไหน และทำไม** ควรอยู่ใน provider ไม่ใช่ใน `Usage` provider
...
**tokeniser คืออะไร** โมเดลไม่ได้อ่านตัวอักษร มันอ่าน token ซึ่งคือก้อนของไบต์
ที่ถูกเลือกโดยอัลกอริทึมบีบอัดที่ถูก fit กับคลังข้อมูลฝึก การจับคู่จากข้อความ
ไปเป็น token คือตาราง และตารางนั้นเป็นส่วนหนึ่งของโมเดล ไม่ใช่ส่วนหนึ่งของภาษา

**ทำไมเรื่องนี้สำคัญกว่าที่ฟังดู** แต่ละบริษัท fit ตารางคนละแบบ ประโยคเดียวกัน
...
| `read_file` ของ `tools.py` | 4035 | 1008 | 1155 | 929 | 931 |
...
**ทำไมเรื่องนี้ถึงถึงตายสำหรับ harness ไม่ใช่แค่น่ารำคาญ** `providers.py`
...
**แล้วค่าประมาณมีไว้ทำอะไร** มันคือตัวจุดชนวน ไม่ใช่การวัด และ docstring
...
ที่รู้อยู่แล้วก็เพียงพอสำหรับคำถามที่คำตอบเป็นใช่หรือไม่ใช่ ตั้ง budget โดย
...
เล็กน้อย ราวหนึ่งในสี่ อัตราส่วนที่แน่นอนขึ้นอยู่กับ vendor และโมเดล
และมันเปลี่ยนได้ ดังนั้นให้ดูหน้าราคาของโมเดลที่คุณใช้จริง แทนที่จะเชื่อตัวเลข
...
คุณสมบัติที่ทำให้ agent แพงคือคุณสมบัติเดียวกันเป๊ะที่ทำให้มัน cache ได้
...
**99.8 เปอร์เซ็นต์** เมื่อเนื้อหาที่คงที่คงที่จริง สิ่งเดียวที่ต่างกันระหว่าง
request สองครั้งติดกันคือข้อความ user ตัวใหม่สุด ทุกอย่างก่อนหน้ามันนำกลับมาใช้ได้
นี่คือสิ่งที่คุณพยายามจะให้มีเป็น

**31.3 เปอร์เซ็นต์** ตัวระบุ session หนึ่งตัวและ timestamp หนึ่งตัว รวมสี่สิบสี่
...
**3.7 เปอร์เซ็นต์** tool schema ในลำดับที่ต่างออกไป tool เจ็ดตัวเดิม คำอธิบายเดิม
...
**timestamp** มีคนเพิ่ม `The current time is {datetime.now()}` ลงใน
`build_system_prompt` ด้วยเหตุผลที่ดี เพราะโมเดลแย่เรื่องการรู้วันที่
ตอนนี้มันคือบรรทัดที่แพงที่สุดในโปรแกรมของคุณ

**ตัวระบุ session** รูปแบบเดียวกัน `Session {session_id}` ใกล้ ๆ ด้านบนของ
...
**ชื่อผู้ใช้ หรืออะไรก็ตามที่ต่างกันไปตามผู้ใช้** `You are helping {user}`
...
**tool definition ในลำดับที่ไม่แน่นอน** ข้อนี้ร้ายที่สุด เพราะคุณไม่ได้เขียน
...
ซึ่งหมายความว่ามันอยู่ใน prefix ที่ถูก cache ซึ่งหมายความว่า request หลังการตัด
แทบไม่มีอะไรร่วมกับ request ก่อนหน้าเลย การตัดทุกครั้งทำให้คุณเสีย cache miss
เต็ม ๆ ใน call ถัดไป

นั่นไม่ได้ทำให้การตัดเป็นเรื่องผิด แต่มันทำให้รูปแบบของการตัดมีความสำคัญ
...
และ `tools.py` มี 13,372 ตัวอักษร สี่พันนั้นจึงเป็นสิ่งที่เหลือหลังจาก `truncate`
ตัดมันแล้ว ซึ่งหมายความว่า agent ได้ไฟล์ไปหนึ่งในสามพร้อมหมายเหตุว่า
`[truncated, 9372 more characters]`
...
**การบอกว่ามันตัดไปแล้วสำคัญพอ ๆ กับการตัด** ส่วนต่อท้ายบอกโมเดลว่ายังมีอีก
และมีอีกเท่าไร มันจึงขอชิ้นอื่นได้ การตัดที่โมเดลมองไม่เห็นคือการตัดที่มันจะ
คิดต่อข้ามไปอย่างมั่นใจ

### สาม ใช้โมเดลที่ถูกกว่าสำหรับงานกลไกเล็ก ๆ

ไม่ใช่ทุก call ใน harness ที่ต้องใช้โมเดลที่เขียนโค้ด การสรุป exchange ที่จบแล้ว
...
ตรวจสอบได้ และโมเดลเล็กก็ทำได้ดีพอ ๆ กัน
...
และวินาทีที่คุณอยากได้โมเดลราคาถูกสำหรับ call เชิงกลไก มันก็ต้องมีสองตัว
นั่นไม่ใช่ปัญหา เพราะบทที่ 06 สร้าง interface ที่รองรับเรื่องนี้ไว้พอดี
`OpenAICompatProvider` สองอินสแตนซ์ที่ใช้โมเดลต่างกัน ตัวหนึ่งส่งให้ `run`
อีกตัวถือไว้โดยอะไรก็ตามที่ทำหน้าที่สรุป และไม่มีอะไรอื่นเปลี่ยน

ต้นทุนของการประหยัดนี้คือโมเดลเล็กที่ทำการจำแนกบางครั้งก็ทำผิด ดังนั้นให้ใช้มัน
...
มนุษย์ ไม่ใช่ของโมเดลใด ๆ ทั้งสิ้น
...
ไม่ว่าโมเดลจะใช้ตัวใดตัวหนึ่งหรือไม่ใช้เลย ในการรันสิบเทิร์นจากหัวข้อ 2
...
คำตอบไม่ใช่การกรองมันต่อเทิร์น แต่คือการให้โมเดลมีวิธีค้นหาแคตตาล็อก
...
`signature` มาจาก `permissions.py` ที่บทที่ 12 ต้องการ string ที่คงที่สำหรับ
call หนึ่งครั้งเป๊ะ ๆ อยู่แล้ว และ `REPEAT_LIMIT` คือ 3 ถ้าสาม call ล่าสุด
เป็น tool เดียวกันด้วย argument เดียวกัน โมเดลจะถูกบอกเรื่องนั้นเป็นคำพูด
...
**ทำไมนี่เป็นการควบคุมต้นทุนมากกว่าจะเป็นฟีเจอร์ด้านความถูกต้อง** `max_turns=10`
...
**ทำไมถึงบอกโมเดลแทนที่จะข้าม call ไปเงียบ ๆ** เหตุผลเดียวกับที่บทที่ 12
พูดถึงเรื่องการปฏิเสธ โมเดลที่ไม่รู้ว่าเกิดอะไรขึ้นเลือกทางอื่นไม่ได้
...
**ทำไม prompt กับ completion ถึงตั้งราคาแยกกัน** เพราะมันเป็นอย่างนั้นทุกที่
...
**ทำไมราคาถึงเป็น argument ไม่ใช่ตารางในโค้ด** นี่คือการตัดสินใจที่ควรถกเถียง
...
และโค้ดก็ทำงานได้ ราคาเปลี่ยน vendor เพิ่ม tier โมเดลได้รุ่นต่อที่ถูกกว่า
พร้อมชื่อที่คล้ายกัน ทุกอย่างเหล่านั้นทำให้ตารางผิดโดยไม่ทำให้มันดูผิด
...
**ข้อจำกัดที่ต้องพูดตรง ๆ** `cost` รู้จักสองคอลัมน์ แต่มันมีมากกว่าสอง
...
check นี้เรียกโมเดลจริง มันต้องการ `AGENTPATH_BASE_URL` และ `AGENTPATH_MODEL`
...
**หนึ่ง การเติบโตถูกสาธิต ไม่ใช่ถูกกล่าวอ้าง**
...
**สี่และห้า กฎเรื่องลำดับ ในฐานะสองด้านของข้อเท็จจริงเดียว**
...
ลงในบทสนทนาที่โมเดลมองเห็น เศรษฐศาสตร์ทั้งหมดของบทนี้ว่าด้วยข้อความนั้น
