...
และฝั่ง Anthropic เพิ่มขึ้นสามบรรทัด loop เพิ่มสองบรรทัดสำหรับ usage (ตัวเลขการใช้ token ที่ provider รายงานกลับมา)
...
ข้อเท็จจริงที่อยู่ใต้เรื่องทั้งหมดนี้คือ language model เป็น stateless มันไม่มีความทรงจำเกี่ยวกับ request
...
คุณไม่ได้จ่ายค่าบทสนทนา คุณกำลังจ่ายค่าผลรวมของทุก prefix (ส่วนหน้าของบทสนทนาตั้งแต่ต้นจนถึงจุดหนึ่ง) ของบทสนทนา
...
กับ tool schema (คำอธิบายรูปแบบของ function ที่ model อ่าน) เจ็ดตัวเป็น prefix คงที่ประมาณ 1400 token และมีการอ่านไฟล์
...
`cache_creation_input_tokens` และ `cache_read_input_tokens` เมื่อมีการใช้ caching (การนำผลของส่วนหน้าที่เหมือนเดิมกลับมาใช้ซ้ำ)
...
tokeniser คือสิ่งที่แปลงข้อความเป็นหน่วยที่ model อ่านจริง model ไม่ได้อ่านตัวอักษร มันอ่าน token ซึ่งคือก้อนของไบต์
...
ที่รู้อยู่แล้วก็เพียงพอสำหรับคำถามที่คำตอบเป็นใช่หรือไม่ใช่ ตั้ง budget (เพดานจำนวน token ที่ยอมให้ใช้) โดย
