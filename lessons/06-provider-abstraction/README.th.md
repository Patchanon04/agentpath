...
บรรทัด `from llm import complete_stream` คือปัญหาทั้งหมดในสี่คำ
...
คุณเปลี่ยน environment variable (ตัวแปรสภาพแวดล้อม คือค่าที่ตั้งไว้ใน shell) ตัวเดียวก็ยิงไปที่ตัวไหนก็ได้
...
สรุปสถานการณ์จริงของคุณตอนจบ บทที่ 05 คุณมี streaming agent ที่ทำงานได้และเรียก tool วนเป็น loop
...
ความยุ่งเหยิงนั้นไม่ประกาศตัวเอง ลองนึกภาพ `llm.py` หกสัปดาห์หลังใส่ flag เข้าไป
มีหนึ่ง branch ในตัวสร้าง payload หนึ่ง branch ในส่วน URL
และ loop สำหรับ parse อีกสองชุดที่ใช้ dictionary ชื่อ `partial` ร่วมกัน
มีคนแก้ bug ที่ tool call ตัวที่สองในเทิร์นที่เรียกสอง tool ทำ argument หาย
เขาลองกับ provider ที่ตัวเองใช้อยู่แล้วส่งขึ้น
branch อีกอันยังมี bug นั้นอยู่ และไม่มี check ตัวไหนเรียกมัน คนแรกที่เจอจึงเป็นผู้ใช้
...
ภาคหนึ่งยังไม่ได้ใช้มันเลย และ บทที่ 10 ในภาคสองว่าด้วยการเขียนมันโดยเฉพาะ
...
คุณรู้จักรูปแบบ tool schema มาตั้งแต่ บทที่ 03 แล้ว ทั้งสอง API ใช้ JSON Schema สำหรับ argument
...
ใน บทที่ 04 คุณได้เรียนว่าผลลัพธ์ของ tool ถูกส่งกลับไปหา model เป็น message ใหม่ที่มี role เป็น `tool`
...
ข้อสุดท้ายควรมองซ้ำอีกรอบ เพราะ บทที่ 03 ใช้ทั้งหัวข้ออธิบายว่า `arguments`
...
แปลว่าชั้นแปลต้องเรียก `json.loads` ตอนขาออก คุณจะเห็นบรรทัดนั้นในหัวข้อ 6

### ความต่างที่สี่ รูปแบบ streaming

บทที่ 05 สอนว่าคำตอบแบบ stream มาถึงเป็นชุดของบรรทัด `data: `
...
มี `content` ไหม มี `tool_calls` ไหม นั่นคือเหตุผลที่ตัว parse ของ บทที่ 05
...
และ provider ก็ส่งต่อทันทีที่มันปรากฏ บทที่ 05 สอนคุณไปแล้วว่าให้ buffer ไว้แล้ว parse ครั้งเดียวตอนจบ
...
ซึ่งเป็นข้อเท็จจริงเดียวกับที่ บทที่ 02 ใช้สร้าง list ของประวัติ
เมื่อ model ผลิต thinking block ในเทิร์นหนึ่งและคุณอยากได้เทิร์นที่สอง
...
นี่คือรูปร่างของมันตอนเกิดจริง harness ตัวหนึ่งยุบ assistant message ทุกตัว
ให้เป็น record ภายในที่มีสาม field คือ role ข้อความหนึ่ง string และ list ของ tool call
แล้วมันรันแบบนั้นมาสี่เดือน ผ่านไปราวหกหมื่นเทิร์นโดยไม่มีใครบ่น
จากนั้นมีคนเปิด thinking ให้บัญชีเดียว และทุกเทิร์นหลังเทิร์นแรกคืน 400 กลับมา
โดยมี `messages.1.content.0.signature` ระบุอยู่ใน error body
บรรทัดที่ทิ้ง field นั้นถูกเขียนก่อนที่ใครในทีมนั้นจะเคยได้ยินคำว่า thinking block
...
ลองใส่ตัวเลขลงไป session ที่มีส่วนหน้ายาวหนึ่งหมื่นสองพัน token และเดินสี่สิบเทิร์น
จะส่งส่วนหน้านั้นซ้ำสี่สิบครั้ง และการอ่านจาก cache มักคิดเงินราวหนึ่งในสิบของการอ่านสด
ขยับ budget ตอนเทิร์นที่ห้า แล้วอีกสามสิบห้าเทิร์นที่เหลือจ่ายเต็มราคา
สำหรับ token หนึ่งหมื่นสองพันตัวนั้น ส่วนหน้าเดิมจึงแพงขึ้นราวสิบเท่าจากเทิร์นก่อนหน้า
ไม่มี request ไหนล้มเหลว และไม่มีอะไรใน log ดูต่างไปจากเดิม

กฎที่ใช้จริงจึงคือเลือกค่า thinking ตอน session เริ่ม แล้วอย่าไปแตะมันตลอดอายุของ session นั้น
ถ้าคุณต้องการค่าอื่น นั่นคือ session อื่น บทที่ 15 คือที่ที่เรื่องนี้ถูกวัดอย่างจริงจัง
...
จาก string ของข้อความกับ list ของ tool call ต่อให้ thinking block รอดจากตัว parse มาได้
...
`providers.py` ไม่เคยส่ง field `thinking` จึงไม่มี model ตัวไหนผลิต thinking block
...
ถ้าคุณทำแบบฝึกหัดข้อนั้น นี่คือข้อกำหนดที่ทำให้มันคุ้มที่จะเขียนให้ถูกต้องจริง ๆ
...
เมื่อชิ้นส่วน argument ต่อกันแล้วไม่เป็น JSON ที่ถูกต้อง key ตัวสุดท้ายมาจาก บทที่ 05
...
วิธีแรกคือใช้ flag ภายใน function เดียว เพิ่ม argument `provider="openai"` ให้ `complete_stream`
...
วิธีที่สองคือแปลทุกอย่างให้เป็นรูปแบบเดียวที่ขอบ เก็บ HTTP client ตัวเดียวไว้
...
วิธีที่สามคือสืบทอดจาก base class ที่มีโค้ดร่วม เขียน `BaseProvider` ที่จัดการ HTTP
...
เหลือให้แชร์แค่บรรทัด `with httpx.Client(...)` บรรทัดเดียว
...
การแลกนี้แย่ลงทันทีที่บรรทัดร่วมนั้นต้องเปลี่ยน สมมติ class แม่งอก helper ชื่อ `_send`
ที่เปิด client แล้วเรียก `raise_for_status` อีกหกเดือนต่อมา provider ตัวหนึ่ง
ต้องอ่าน body ของ 429 ก่อนจะ raise เพราะเวลาที่ให้รอใหม่อยู่ในนั้น
การแก้ลงไปที่ class แม่ และตอนนี้ provider อีกตัวก็ raise exception คนละตัวกับเมื่อวาน
ด้วยเหตุผลที่คุณมองไม่เห็นจากไฟล์ของตัวมันเอง
...
นั่นเป็นการเลือกโดยตั้งใจ และเป็นเหตุผลที่บทนี้มาหลัง บทที่ 05 แทนที่จะมาก่อน

ลองจินตนาการว่าเราทำสลับกัน บทที่ 05 สอน streaming ก็ลองนึกภาพคอร์สรุ่นที่
abstraction มาที่ บทที่ 05 และ streaming มาที่ บทที่ 06
ข้อตกลงคงถูกออกแบบตามสิ่งที่มีอยู่ในตอนนั้น ซึ่งก็คือ function ของ บทที่ 04
...
cancellation คือตัวที่คนเจอทีหลังสุดและเสียดายมากที่สุด
ข้อตกลงที่สัญญาค่าคืนค่าเดียวไม่มีที่ให้พูดว่าหยุด
ทางออกเดียวจากการเรียกที่จะกินเวลาเก้าสิบวินาทีจึงมีแค่ฆ่า thread หรือทนรอ
ทีมมักรู้เรื่องนี้ในวันที่ผู้ใช้กด escape แล้วมิเตอร์ยังเดินต่ออีกนาทีครึ่ง
การเติมมันทีหลังแปลว่าต้องเปลี่ยน signature เปลี่ยน provider ทั้งสองตัว
และเปลี่ยนผู้เรียกทุกตัว ซึ่งก็คือคอลัมน์ห้าการแก้ข้างบน
...
มันยังหมายความว่า provider ในอนาคตที่ไม่มี endpoint (URL ที่เรายิง request ไปหา) แบบ streaming ก็ยังเข้ากับข้อตกลงได้
...
โค้ดนี้ยกมาตรง ๆ จาก loop สำหรับ parse ของ บทที่ 05 แล้วตั้งชื่อให้
...
แล้ว `json.loads` จะปฏิเสธ การคืน error เป็นค่าแทนที่จะ raise คือการรักษาการตัดสินใจจาก บทที่ 05 ไว้
...
constructor คือที่ที่การตั้งค่าทั้งหมดมาอยู่แล้วตอนนี้ ใน บทที่ 05
...
และเป็นสิ่งที่จะให้ภาคสามรัน model ราคาถูกสำหรับงานย่อยแบบสรุปความ
...
เนื้อในนี้คือตัว parse ของ บทที่ 05 ที่ย้ายเข้ามาอยู่ใน method โดยไม่แตะอะไรอย่างอื่นเลย
...
การส่ง `{"type": "text", "text": ""}` จะถูก API ปฏิเสธ `if` ตัวนี้จึงรับน้ำหนักจริง
...
เพราะนั่นคือทรงที่คุณอ่านมาตั้งแต่ บทที่ 02 และการเปลี่ยนมันตอนนี้จะบดบังบทเรียนจริง
...
`blocks` จึงบรรจุแต่ tool call และแปลงเป็น list `calls` ได้เลยโดยไม่ต้องกรอง
...
เปิด `agent.py` แล้วเทียบกับของ บทที่ 05 มีสองอย่างที่เปลี่ยน
...
การเปลี่ยนที่หนึ่งคือ `from llm import complete_stream` หายไป
...
การเปลี่ยนที่สองคือ `schemas = [t["function"] for t in tools.SCHEMAS]`
บทที่ 05 ส่ง `tools.SCHEMAS` ผ่านไปตรง ๆ พร้อมซองห่อทั้งชุด
...
ส่วนที่เหลือในไฟล์ไม่ถูกแตะเลย งานจดบันทึกหลังการเรียก tool เหมือนกับ บทที่ 05 ทุกอย่าง
...
หนึ่งเทิร์นที่ข้ามรอยต่อหน้าตาแบบนี้ รูปร่างบนสายมีอยู่แค่ในกล่องกลางเท่านั้น
...
sequenceDiagram
    ...
    Note over L: ประวัติทรง OpenAI<br/>role tool และ tool_call_id
    L->>P: stream(messages schemas on_text)
    Note over P: _to_wire สร้างทรงของ dialect นั้น
    P->>S: POST พร้อม payload ของ dialect นั้น
    S-->>P: บรรทัด data ทีละบรรทัด
    Note over P: ต่อชิ้นส่วน<br/>แล้ว parse ครั้งเดียว
    P-->>L: text กับ calls
    Note over L: ต่อท้าย role tool<br/>ลงประวัติชุดเดิม
```

ยังมีอีกหนึ่งความต่างที่พลาดง่าย `agent.py` ของ บทที่ 05 จบด้วยแบบนี้
...
ของ บทที่ 06 ไม่มีแบบนั้น และมีไม่ได้ เพราะ `run` เสก provider ขึ้นมาจากอากาศไม่ได้อีกแล้ว
...
มันทดสอบได้โดยไม่ต้องมีเครือข่าย test สามารถยื่น object เล็ก ๆ ที่มี method `stream`
...
มันนำกลับมาใช้ได้โดยไม่ต้องแก้ บทเรียนในอนาคตที่รัน model ราคาถูก
...
provider พัฒนาแยกกันได้ provider ใหม่คือ class ใหม่ใน `providers.py`
...
ตัวจัดการ request ของมันแยกสาขาตาม path หลัง hook ที่ให้ test สั่งให้ล้มเหลวได้ และบรรทัดที่นับ token สำหรับก้อน usage
...
`decide` อ่าน message สุดท้ายผ่าน helper ที่รองรับผลลัพธ์ tool ทั้งสองรูปร่าง ซึ่งควรดูไว้
...
สาขาแรกของ helper ดักผลลัพธ์ที่เป็น message ทรง OpenAI สาขาที่สองดัก user message ทรง Anthropic
ที่มี `tool_result` block อยู่ข้างใน แล้ว `decide` คืน `The tool returned {last_result}.` ให้ทั้งสองแบบ
...
คาดหวังผลลัพธ์รูปแบบนี้เมื่อชี้ไปที่ endpoint แบบ OpenAI compatible ตัวจริง
...
เรื่องการเดินทางไปกลับ และหัวข้อ 9 ของ บทที่ 03 คือที่ที่ควรไปดู
...
บทที่ 03 ให้สัญญาเรื่องนี้ไว้ และถึงเวลาทวงสัญญาแล้ว
...
การเดาผิดอย่างมั่นใจกับบ่ายวันที่แย่ shell tool ในภาคสองเลยมาพร้อม
prompt ยืนยันตั้งแต่วันแรกที่มันมีตัวตน ไม่ใช่ของที่มาเติมทีหลัง
...
สวิตช์นั้นจะเติบโตเป็นระบบ permission (สิทธิ์ คือตัวตัดสินว่าอะไรทำได้บ้าง) เต็มรูปแบบในภาคสาม

คุณจะได้พบภัยคุกคามที่ทำให้คนประหลาดใจด้วย เมื่อ agent อ่านไฟล์และ output ของคำสั่งได้แล้ว
ข้อความที่คนอื่นเขียนจะไหลเข้ามาในบทสนทนา แล้ว model ก็รับมันเป็น input
ไฟล์ใน repository อาจมีประโยคที่จ่าหน้าถึง agent นั่นคือ prompt injection (การแทรกคำสั่ง คือ model ทำตามข้อความจากข้างนอกเหมือนคุณสั่งเอง)
มันเข้ามาทางผลลัพธ์ของ tool ไม่ได้เข้ามาทางผู้ใช้ ภาคสองแนะนำมัน และภาคสามจัดการมันอย่างจริงจัง
...
ข้อแรกให้เขียน `ScriptedProvider` จากหัวข้อ 7 แล้วใช้มันทดสอบ `run` โดยไม่ใช้เครือข่ายเลย
...
ข้อสองให้เพิ่ม class ของ provider ตัวที่สามสำหรับบริการที่คุณใช้จริง
...
ข้อสามให้เปลี่ยนรูปแบบบทสนทนาภายใน ตอนนี้ `agent.py` เก็บประวัติในทรง OpenAI
