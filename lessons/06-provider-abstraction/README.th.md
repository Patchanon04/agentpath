...
ดังนั้นชั้นแปลต้องเรียก `json.loads` ตอนขาออก คุณจะเห็นบรรทัดนั้นในหัวข้อ 5
...
และจริง ๆ แล้วหัวข้อ 5 ก็ทำการแปลงแบบนี้พอดี ความต่างอยู่ที่ตัวแปลงอยู่ตรงไหน
...
## 4. ทำไม interface จึงเอา streaming มาก่อน
...
## 5. เขียน providers.py ทีละ class
...
ส่วน `agent.py` จะแกะ `tools.SCHEMAS` ลงมาถึง object ชั้นในนั้นก่อนเรียก ซึ่งคุณจะเห็นในหัวข้อ 6
...
เพราะมันพิสูจน์ว่าตรรกะการรวมจากหัวข้อ 5 มาถึงอย่างถูกต้อง
...
ในโค้ด Python ประมาณสองร้อยบรรทัดที่คุณอ่านจากบนลงล่างได้ และไม่มี framework อยู่ในนั้นเลย
...
**ข้อหนึ่ง** เขียน `ScriptedProvider` จากหัวข้อ 6 แล้วใช้มันทดสอบ `run` โดยไม่ใช้เครือข่ายเลย
