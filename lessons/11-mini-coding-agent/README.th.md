...
ห้าในหกไฟล์ Python เหมือนเดิมทุกไบต์กับที่เป็นอยู่ในบทก่อนหน้า ไฟล์ใหม่มีแค่ `main.py`
...
| `OpenAICompatProvider` | 06 | Ollama, OpenRouter, Groq, OpenAI, อะไรก็ตามที่เข้ากันได้ |
...
ระหว่างบทที่ 06 กับบทที่ 10 คุณเพิ่ม tool ห้าตัว `read_file` `write_file` `edit_file`
...
และคืนสตริง อีกฝั่งหนึ่ง tool คือรายการใน `FUNCTIONS` เป็น schema ใน `SCHEMAS`
และเป็นฟังก์ชัน Python ทั้งสองฝั่งไม่รู้อะไรอย่างอื่นเกี่ยวกับอีกฝั่งเลย loop ไม่เคยได้ยินเรื่องไฟล์
เรื่อง subprocess เรื่อง glob หรือเรื่องหน้าจอยืนยัน shell tool ไม่เคยได้ยินเรื่อง `max_turns`
...
`edit_file` บางครั้งล้มเหลวในแบบที่โมเดลควรลองใหม่ loop จึงงอกสาขาสำหรับการลองใหม่
...
แต่คือสิ่งที่เกิดขึ้นตอนที่คุณไม่ได้คิดถึงมัน ตลอด tool ห้าตัวและการตรวจสอบความปลอดภัย
...
**`task` เป็น positional แต่ไม่บังคับ** `nargs="?"` แปลว่าคุณจะใส่หรือไม่ใส่ก็ได้ ใส่แล้ว
...
การ import อยู่ด้านล่างของฟังก์ชันแทนที่จะอยู่บนสุดของไฟล์ และนั่นตั้งใจ ดูสิ่งที่ `tools.py`
...
ตั้งแต่วินาทีนั้น `from agent import run` import `agent` ซึ่ง import `tools` ซึ่งรันบรรทัดนั้น
...
**ทำไมถึงอ่าน workspace จาก environment variable ตั้งแต่แรก** ทางเลือกที่ชัดเจนคือส่งมัน
...
เพราะ tool ทั้งเจ็ดตัวจะต้องมีพารามิเตอร์เพิ่ม ทุก schema จะต้องซ่อนมันจากโมเดล และ
...
**ทำไมต้อง `resolve()` ก่อนเก็บ** `Path(".").resolve()` เปลี่ยน relative path เป็น absolute
...
ลงใน system prompt เป็นข้อเท็จจริงเกี่ยวกับโลก และโมเดลที่ถูกบอกว่ามันทำงานอยู่ใน `.`
...
เมื่อเดาผิด และมันผิดบ่อยกว่าที่คุณคิด โมเดลของ Anthropic ถูกให้บริการผ่าน gateway ที่เข้ากัน
ได้กับ OpenAI โดยผู้ให้บริการหลายราย ดังนั้น URL จึงบอกว่า `openrouter.ai` ในขณะที่โมเดล
...
คุณจึงรู้ว่าลืม export ตัวแปรทันที แทนที่จะรู้หลังจากโมเดลอ่านไฟล์ไปแล้วสี่ไฟล์
...
`sys.exit` จากข้างใน ประโยชน์ในทางปฏิบัติคือ `main` ยังเป็นฟังก์ชันธรรมดาที่คุณเรียกจากเทสต์
...
ซึ่งเป็นบั๊กประเภทที่รอดจากการ review โค้ดพอดี

ทีนี้ตั้งค่า environment แล้วรัน agent นี่คือตัวแปรสี่ตัวเดียวกับในบทที่ 00
...
ไม่ใช่ฟังก์ชันทั้งฟังก์ชัน ข้อความเดิมหนึ่งบรรทัดและข้อความใหม่หนึ่งบรรทัด นั่นคือ `edit_file`
ทำงานที่มันมีอยู่เพื่อทำ ถ้า agent ใช้ `write_file` มันจะต้องผลิต docstring `total` และ
`largest` ขึ้นมาใหม่จากความจำอย่างสมบูรณ์แบบ และโมเดลที่ผลิตโค้ดที่มันไม่จำเป็นต้องเปลี่ยน
คือโมเดลที่จะทำบรรทัดหนึ่งหล่นหายไปอย่างเงียบ ๆ
...
หมายเหตุที่ซื่อสัตย์สองข้อเกี่ยวกับบันทึกนี้ ประโยคของโมเดลเองไม่ได้ถูกแสดงระหว่างการเรียก
tool เพราะมันต่างกันไปในแต่ละการรันและแต่ละโมเดล โมเดลเล็ก ๆ ที่รันในเครื่องมักไม่พูดอะไรเลย
ระหว่างการเรียก โมเดลใหญ่กว่าจะบรรยาย ความแปรผันนั้นเป็นเรื่องปกติและไม่ใช่สัญญาณว่า
มีอะไรผิด และลำดับที่แน่นอนก็ต่างกันได้ โมเดลที่ตรงไปที่ `read_file` โดยไม่ grep ก่อน หรือ
...
และ `.venv` ถูกข้าม ทั้งหมดเรียก `tools.run` โดยตรงและไม่มีอันไหนเกี่ยวข้องกับโมเดล
...
`BUGGY` ถูกเลือกมาอย่างระมัดระวัง `add` คืน `a - b` ซึ่งเป็นบั๊กที่คนมองเห็นได้ทันทีและโมเดล
ก็มองเห็นได้ทันที ดังนั้น check นี้จึงไม่ใช่การทดสอบความฉลาดของโมเดลแบบแอบแฝง และยังมี
ฟังก์ชันที่สองคือ `multiply` ซึ่งไม่เกี่ยวกับบั๊กเลย ข้อยืนยันข้อที่สองจากสามข้อข้างล่างพูดถึง
ฟังก์ชันนั้น

### บังคับทิศทางโมเดลโดยไม่มีโมเดล
...
สิ่งที่ถูกเขียนบทไว้มีแค่ว่า tool ไหนถูกเรียกด้วยอาร์กิวเมนต์อะไร นั่นคือส่วนที่โมเดลจริงจะตัดสินใจ
...
ถ้าส่วนไหนพัง check จะล้มเหลว และมันล้มเหลวด้วยเหตุผลเดียวกับที่จะล้มเหลวกับโมเดลที่เสียเงิน

สิ่งที่คุณยอมสละคือความมั่นใจว่าโมเดลจะเลือกการเรียกสี่ครั้งนั้น สิ่งที่คุณได้มาคือ check ที่รัน
...
รัน มันไม่ต้องการอะไรเลยนอกจากโมเดล และโมเดลก็เขียนประโยคนั้นทั้งตอนที่มันทำงานเสร็จจริง
...
การยืนยันกับร้อยแก้วของโมเดลคือการยืนยันกับส่วนเดียวของระบบที่ผิดได้อย่างน่าเชื่อ
...
**บทที่ 12 ระบบ permission** ผลลัพธ์สามแบบแทนที่จะเป็นสองแบบ ซึ่งคือถาม อนุญาต และปฏิเสธ
...
**บทที่ 13 session** บทสนทนาถูกเขียนลงไฟล์ JSONL ขณะที่มันเกิดขึ้น หนึ่งข้อความต่อหนึ่งบรรทัด
...
4036 ตัวอักษรพร้อมหมายเหตุว่า `[truncated, 10174 more characters]` ประมาณหนึ่งพัน token
...
จึงถูกส่งอีกครั้งใน turn ที่สี่ turn ที่ห้า และ turn ที่หก ในงานยาว ๆ กับโมเดลเล็ก ๆ ที่รัน
ในเครื่อง คุณจะดูมันทำงานสองนาทีแล้วได้รับ HTTP error เรื่องความยาว context เกิน
ซึ่ง ณ จุดนั้นการรันก็จบและไม่มีทางทำต่อ

**บทที่ 14 การจัดการ context** การวัดขนาดบทสนทนา การตัดสินใจว่าจะทิ้งอะไร และการสรุป
...
**บทที่ 15 เศรษฐศาสตร์ของ token** เงินไปไหนจริง ๆ วัดออกมาแทนที่จะเดา prompt caching
...
**บทที่ 17 error และการลองใหม่** ความล้มเหลวแบบไหนควรลองใหม่และแบบไหนไม่ควร
...
แปลว่าทำ side effect นั้นสองครั้ง จึงต้องมี idempotency key และ retry loop รอบการเรียกโมเดล
...
ของไฟล์นั้นรวมถึงคอมเมนต์ ตอนนี้มาถึงในรูปข้อความที่มี role เป็น `tool` และโมเดลที่กำลัง
ไล่อ่านบทสนทนาของมันก็เจอประโยคที่จ่าหน้าถึงมันโดยตรงที่บอกให้ทำอะไรบางอย่างก่อน

ลองดู ขึ้นอยู่กับโมเดลคุณจะเห็นหนึ่งในสามอย่าง มันเมินคอมเมนต์นั้น มันเอ่ยถึงคอมเมนต์นั้น
แล้วถามคุณ หรือมันเรียก `run_shell` ด้วยคำสั่งนั้น ซึ่ง ณ จุดนั้น `confirm` จะพิมพ์มันออกมา
และคุณได้พูดว่าไม่

**ทีนี้มาถึงแบบฝึกหัด และลำดับสำคัญ**

ข้อแรก ทำให้มันเกิดขึ้นให้ได้อย่างน้อยหนึ่งครั้ง ลองโมเดลที่เล็กกว่า ลองถ้อยคำที่ฟังดูเหมือน
...
ภาษาธรรมชาติในบทสนทนาเดียวกัน แข่งกันเพื่อความสนใจก้อนเดียวกัน และไม่มีกลไกใดในโมเดล
ที่จัดอันดับอันหนึ่งเหนืออีกอัน คุณไม่ได้กำลังบังคับใช้กฎ คุณกำลังส่งคำขอที่บังเอิญอยู่ก่อน
ในรายการเท่านั้น

ข้อสาม ถามว่าอะไรกันแน่ที่หยุดผลลัพธ์เลวร้ายในการรันที่โมเดลกินเหยื่อ มันไม่ใช่ prompt
มันคือ `confirm` ที่พิมพ์คำสั่งออกมาแล้วรอมนุษย์ มาตรการที่อยู่นอกโมเดลคือชนิดเดียวเท่านั้น
...
หลายไฟล์ บนโมเดลที่มี window แปดพัน token คุณจะชนกำแพงเร็วมาก อ่าน error ที่คุณได้
...
ตามลำดับ และสร้าง vector index เล็ก ๆ เพื่อให้คุณวัดความต่างได้แทนที่จะเถียงกันเรื่องมัน
