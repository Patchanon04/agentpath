...
ประโยคนั้นคือประเด็นของบทนี้ อ่านอีกครั้งแทนที่จะข้ามไป บทที่ 04 ถึง 10 สร้าง loop
...
และ `main.py` ไม่มี logic ของ agent อยู่เลย มันอ่าน argument ตั้ง environment variable
...
| `parse_arguments` | 05 | argument ของ tool ที่พังกลายเป็นข้อความ ไม่ใช่การ crash |
...
ท่อสี่สิบบรรทัดคร่อมเครื่องจักรแปดบทเรียน นี่คือภาพว่าสี่สิบบรรทัดนั้นต่ออะไรเข้ากับอะไร

```mermaid
flowchart TD
    M["main.py<br/>workspace provider task"] --> SP["build_system_prompt<br/>บทที่ 10"]
    M --> PR["OpenAICompatProvider<br/>หรือ AnthropicProvider"]
    SP --> R
    PR --> R["run ใน agent.py<br/>loop จากบทที่ 04"]
    R -->|"stream"| PR
    R -->|"tools.run<br/>ชื่อกับ argument"| T["FUNCTIONS ใน tools.py"]
    T --> F["read_file write_file<br/>edit_file list_files<br/>glob_files grep_files"]
    T --> S["run_shell"]
    F --> G["resolve_inside<br/>บทที่ 07"]
    S --> C["confirm<br/>บทที่ 08"]
    G --> D["โฟลเดอร์บนดิสก์"]
    C --> D
```

หัวข้อ 3 จะไล่ทีละบรรทัด รวมถึงสองบรรทัดที่ลำดับของมันแบกน้ำหนักไว้
...
| --- | --- |
| `complete(...)` กลายเป็น `provider.stream(...)` | 05 ให้ streaming และ 06 ให้ argument provider |
| ส่ง callback `on_text` เข้าไป | 05 |
| `tools.SCHEMAS` ถูกแกะเป็น `[t["function"] for t in ...]` | 06 ที่ provider เป็นคนห่อแทน |
| สาขา `call["error"]` | 05 ที่ JSON ของ argument ที่พังกลายเป็นข้อความ |
...
และคืนสตริง อีกฝั่งหนึ่ง tool คือรายการใน `FUNCTIONS` เป็น schema (บอกรูปร่างของข้อมูลด้วย JSON) ใน `SCHEMAS`
และเป็น function Python ทั้งสองฝั่งไม่รู้อะไรอย่างอื่นเกี่ยวกับอีกฝั่งเลย loop ไม่เคยได้ยินเรื่องไฟล์
เรื่อง subprocess เรื่อง glob (หาไฟล์จากรูปแบบของชื่อ) หรือเรื่องหน้าจอยืนยัน shell tool ไม่เคยได้ยินเรื่อง `max_turns`
...
แบบทดสอบว่ารอยต่ออยู่ในที่ที่ถูกต้องหรือไม่ ไม่ใช่ว่ามันดูสง่างามแค่ไหนตอนคุณวาดมัน
...
สาม argument และแต่ละอันถูกกำหนดรูปร่างด้วยความคิดเฉพาะอย่าง

`task` เป็น positional (argument ที่ดูตำแหน่ง ไม่ได้ดูชื่อ) แต่ไม่บังคับ `nargs="?"` แปลว่าคุณจะใส่หรือไม่ใส่ก็ได้ ใส่แล้ว
...
เมื่อ `run` มีตัวตนเป็นชื่อใน `main.py` workspace ก็ถูกตรึงไว้แล้วตลอดอายุของโปรเซส

ถ้าตั้ง environment variable หลัง import มันจะไม่มีผลอะไรเลย โปรแกรมจะไม่ crash
จะไม่เตือนคุณ มันจะแค่ resolve ทุก path เทียบกับ directory ที่คุณบังเอิญยืนอยู่ แปลว่า
...
นั่นคือกับดักจริงและมันสมควรได้รับการป้องกันจริง สังเกตว่า `check.py` ทำเหมือนกันเป๊ะ
...
workspace มาจาก environment variable ไม่ได้มาจาก argument ทางเลือกที่ชัดเจนคือส่งมัน
เป็น argument เพื่อให้ `read_file(workspace, path)` รับมันอย่างชัดแจ้งและไม่มีลำดับ import
...
ข้อที่สามนี้แย่กว่าแค่ไร้ประโยชน์ `Workspace directory .` อ่านแล้วเป็นข้อเท็จจริงสำหรับ model
มันจึงคิดต่อจากตรงนั้น ใน session หนึ่งมันจะส่ง `./src/app.py`
และใน session ถัดไปจะส่ง `/app/src/app.py` เพราะทั้งคู่เข้ากันได้กับสิ่งที่มันถูกบอก
และไม่มีอะไรใน prompt เลือกข้างไหน absolute path เอาทางเลือกนั้นออกไป
...
ได้กับ OpenAI โดยผู้ให้บริการหลายราย URL จึงบอกว่า `openrouter.ai` ในขณะที่ model
...
ลองอยู่กับกรณีหนึ่งสักครู่ บริษัทของคุณส่ง traffic ของ model ผ่าน
`https://llm-gateway.corp.internal/v1` ซึ่งเบื้องหลังคือ Anthropic
ไม่มีอะไรในชื่อ host นั้นบอกแบบนั้น การดมกลิ่นจึงเลือก class แบบ OpenAI compatible
และ request แรกก็ยิงไปที่ `/v1/chat/completions` คุณได้ 404
บน URL ที่คุณเอาไปวางในเบราว์เซอร์ได้ กับชื่อ model ที่คุณรู้ว่ามีจริง
แล้วคุณก็ใช้เวลายี่สิบนาทีถัดมาไปกับการตรวจ key ของตัวเอง
...
แทนที่จะคืนบทสรุป ผลลัพธ์ของ tool หนึ่งคือ argument ของ tool ถัดไป
...
ก็มองเห็นได้ทันที check นี้จึงไม่ใช่การทดสอบความฉลาดของ model แบบแอบแฝง และยังมี
...
แล้วกี่อัน แล้วตอบด้วยคำสั่งถัดไป คำสั่งสี่อันจึงผลิตการเรียก tool สี่ครั้งตามลำดับ
...
สิ่งที่ถูกเขียนบทไว้มีแค่ว่า tool ไหนถูกเรียกด้วย argument อะไร นั่นคือส่วนที่ model จริงจะตัดสินใจ
...
loop สะสมชิ้นส่วน argument ที่ stream มาจริง สร้าง assistant message จริง dispatch ผ่าน
...
และหนึ่งในนั้นต้องเป็น `5` เป๊ะ ๆ `2 + 3` คือ `5` และ `2 - 3` คือ `-1` แปลว่าเงื่อนไขนี้
...
ชิ้นส่วน argument ที่มาถึงทีละห้าตัวอักษรถูกสะสมเป็น JSON ที่ถูกต้องและถูก decode ชื่อ
...
มันจึงถูกต้องโดยบังเอิญไม่ได้ นั่นคือสิ่งที่ทำให้มันเป็นหลักฐาน
...
งานที่สองบนโปรเจกต์เดียวกันจึงเริ่มจากศูนย์ มัน grep หาสิ่งเดิม อ่านไฟล์เดิม
...
ตีราคาการทำซ้ำนั้นดู งานแรก grep สองครั้งและอ่านหกไฟล์ ราว ๆ เก้าพัน token
ก่อนจะเริ่มคิดถึงคำขอจริงของคุณด้วยซ้ำ ถามคำถามที่สองเกี่ยวกับโปรเจกต์เดิมอีกสิบนาทีต่อมา
มันก็จ่ายเก้าพัน token เดิมเพื่อไปถึงจุดเดิม ประโยคเดียวที่จะประหยัดทั้งหมดนั้นได้
คือประโยคที่บอกชื่อไฟล์ที่เก็บการตรวจรหัสผ่าน และมันอยู่ใน message list ที่ไม่มีอยู่แล้ว
...
ดูเลขคณิต `read_file` ตัดที่ 4000 ตัวอักษร `tools.py` ของบทนี้เองจึงกลับมาเป็น
...
แต่บทที่ 02 บอกไว้แล้วว่าบทสนทนาทั้งหมดถูกส่งซ้ำในทุก request token เหล่านั้น
...
สังเกตว่ามันลงตอนไหน ไม่ใช่เทิร์นแรกที่คุณไม่เสียอะไรเลย มันลงตอนเทิร์นที่เก้า
สี่สิบวินาทีหลังเริ่ม หลังจาก grep สองครั้ง อ่านสี่ไฟล์ และแก้ไฟล์ไปหนึ่งครั้งบนดิสก์แล้ว
429 ที่รอสองวินาทีก็หายกลับจบการรันแทน และทางเดียวที่จะไปต่อคือพิมพ์งานนั้นใหม่
แล้วจ่ายค่าเก้าเทิร์นนั้นอีกรอบ บน workspace ที่ตอนนี้ถูกแก้ไปครึ่งทางแล้ว
...
แปลว่าการอ่านหนึ่งครั้งมีต้นทุนราว 4036 ตัวอักษร เรียกว่าหนึ่งพัน token ทีนี้คำนวณสำหรับ
...
ที่มีคำสั่งย่อยแทนที่จะมี argument แบบตำแหน่งเพียงอันเดียว

ไม่มีอะไรในนั้นเปลี่ยนสิ่งที่ agent ทำในหนึ่ง turn ทั้งหมดนั้นเปลี่ยนว่าคุณจะยอมให้คนอื่น
รันมันหรือไม่

ภาค 3 มีเจ็ดบท บทที่ 12 คือระบบ permission ที่มีถาม อนุญาต และปฏิเสธ และมอง
prompt injection เป็นข้อจำกัดด้านการออกแบบตามที่มันเป็นจริง ไม่ใช่เชิงอรรถ บทที่ 13
...
ตามลำดับ และสร้าง vector index (ดัชนี vector ที่สร้างไว้ล่วงหน้า) เล็ก ๆ เพื่อให้คุณวัดความต่างได้แทนที่จะเถียงกันเรื่องมัน
