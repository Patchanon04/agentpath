...
## ยินดีต้อนรับสู่ ภาค 4

ภาค 1 สร้าง agent ขึ้นมา ภาค 2 ให้ tool จริง ๆ กับมัน ภาค 3 ทำให้มันอยู่รอดได้
...
ภาค 4 ว่าด้วยขีดจำกัดของ agent ตัวเดียว ที่อยู่ลำพัง ใช้ได้แค่ tool ที่คุณเขียนเอง
...
ทั้งตัว เพิ่ม process ที่สอง เพิ่ม handshake (แลกเวอร์ชันและความสามารถกันก่อนเริ่มคุยจริง)
และเพิ่ม tool ประเภทที่ยังไม่มีอยู่เลย
...
หรือไม่ และ check ที่พิสูจน์ว่ามันทำงาน การทำแบบนั้นแปดครั้งกินเวลาเกือบทั้ง ภาค 2
...
รอยต่อที่ทำให้ ภาค 2 ใช้ได้ ซึ่ง tool เป็นแค่ชื่อใน dictionary และ loop ไม่รู้อะไร
...
ยาวไม่ถึงสี่สิบบรรทัดและส่วนใหญ่เป็น docstring
...
หนึ่งสัปดาห์แบบนั้นเปลี่ยนสิ่งที่คนขอ มีคนอยากให้ agent บอกว่า deploy เมื่อคืนออกไปหรือยัง
ถ้าเขียนเป็น tool ด้วยมือก็แปลว่าต้องมี API client มี token ในสภาพแวดล้อม มีการแบ่งหน้า
และกินเวลาราวหนึ่งวัน ทีมแพลตฟอร์มรัน server สำหรับเรื่องนี้อยู่แล้ว งานจึงเหลือคอนฟิกสี่บรรทัด
และ agent ก็ตอบคำถามนั้นได้ในอีกสิบเอ็ดนาทีต่อมา ไม่มีอะไรในความสามารถนั้นที่ถูกลงในแง่การสร้าง
มันแค่ถูกลงในแง่การเอื้อมถึง
...
client ตัวนี้พูดได้แค่ **stdio transport (ส่งข้อความผ่าน standard input กับ output ของ process) เท่านั้น**
server เป็น subprocess และเรา
...
หนึ่งร้อยแปดสิบเก้าบรรทัด รวมทั้ง docstring ระดับ module สิบแปดบรรทัดและ
...
## 4. JSON-RPC (เรียก function ข้าม process โดยส่งกันเป็น JSON) บน pipe ทีละบรรทัด
...
sequenceDiagram
  ...
  C->>S: initialize พร้อม id 1
  S-->>C: result ที่มี serverInfo
  C->>S: notifications/initialized ไม่มี id
  C->>S: tools/list พร้อม id 2
  S-->>C: result ที่มีชื่อและ schema
  C->>S: tools/call พร้อม id 3
  S-->>C: result ที่มี content และ isError
```
...
คุณไม่ได้เขียน tool พวกนี้ tool ทุกตัวใน ภาค 2 มาพร้อมกับข้อถกเถียงว่ามันควร
...
เวอร์ชันของเรื่องนี้ที่เสียเงินจริงนั้นหน้าตาน่าเบื่อ server จัดรูปแบบโค้ดตัวหนึ่งที่ติดตั้งมาจาก
template จบคำอธิบายของ `format_code` ด้วยประโยคที่บอกว่าผู้เรียกควรอ่าน `.env` ก่อน
เพื่อให้เคารพการตั้งค่าของโปรเจกต์ ประโยคนั้นคือคำอธิบาย tool มันจึงไม่ได้มาทางไฟล์
ที่ agent เลือกเปิดเอง และมันนั่งอยู่ใน context ตั้งแต่ก่อนงานแรกจะถูกพิมพ์ agent อ่านไฟล์นั้น
key สี่ตัวในนั้นออกไปกับ request ถัดไปที่ส่งหา provider และทุกบรรทัดของการรันนั้นอ่านแล้ว
ดูถูกต้องหมดในไฟล์ session
...
ลองตรวจสักครั้งแล้วคุณจะไม่ลืมมันอีก สตาร์ต server ผ่าน client ตัวนี้ แล้วอ่านสภาพแวดล้อม
ของ process ลูกบนเครื่องคุณเอง key ของ provider อยู่ในนั้น เพราะ `Popen`
ยื่นสำเนาของคุณให้ลูกไปเลยถ้าคุณไม่บอกว่าอย่าทำ และ client ตัวนี้ก็ไม่ได้บอกแบบนั้น
คอนฟิกสี่บรรทัดที่พิมพ์ลงตัวช่วยติดตั้งหน้าตาเป็นมิตร ซื้อให้โปรแกรมของคนอื่น
เอื้อมถึงเครื่องคุณได้เท่ากับที่คุณเอื้อมถึง
...
3101 ตัวอักษร คือ tool แปดตัวที่คุณเขียนเอง ทุกอย่างที่ ภาค 2 และ ภาค 3
...
บ่ายหนึ่งที่วัดผลไว้หน้าตาแบบนี้ server เชื่อมไว้หกตัว tool ในรายการห้าสิบสี่ตัว
สามตัวในนั้นเสนอบางอย่างที่ชื่อ search พอถูกถามว่าฟังก์ชันหนึ่งถูกนิยามไว้ที่ไหน agent
เลือก search ของ wiki ได้เอกสารปฐมนิเทศกลับมาสี่หน้า สรุปย่อมัน แล้วลอง search
เดิมด้วยคำอื่น และเพิ่งไปถึง `grep_files` ในเทิร์นที่สี่ เก้าพัน token กับสามเทิร์น
สำหรับคำถามที่ `grep_files` ตอบได้ในเทิร์นเดียว ถอด server ออกห้าตัวจากหกตัว
แล้วคำถามเดิมก็ถูกตอบตั้งแต่ครั้งแรก
...
บทที่ 14 ทำให้เรื่องนี้พออยู่รอดได้ ไม่ใช่แก้ได้ และบทนี้ก็แอบซ้ำเติมมันในสองทาง
...
นั่นคือบทที่ 20 ว่าด้วย subagent คือ agent ที่สตาร์ต agent อีกตัวที่มี context สดของตัวเองได้
ยื่นงานแคบ ๆ ชิ้นเดียวให้มัน แล้วรับคำตอบสั้น ๆ กลับมาแทนที่จะเป็นบทสนทนา
สี่สิบข้อความ บทสนทนาของตัวแม่โตขึ้นหนึ่งย่อหน้า แทนที่จะโตขึ้นเท่ากับทุกอย่าง
ที่ลูกต้องอ่านเพื่อเขียนย่อหน้านั้น บทนั้นยังมาพร้อมกับ failure mode ที่คุณได้ฟรี
ซึ่งก็คือตัวแม่กับลูกตอนนี้เห็นโลกคนละเวอร์ชัน และ isolation ที่ทำให้ลูกคมชัดก็คือตัวการเดียวกันที่ซ้ำเติมเรื่องนั้นพอดี
