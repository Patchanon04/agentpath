...
`quantize.py` ปัดเศษ `quantize` แปลงแต่ละแถวเป็นจำนวนเต็มความกว้างที่กำหนดโดยมีไม้บรรทัด
หนึ่งตัวต่อแถว `dequantize` แปลงกลับ และ `bytes_for` บอกว่าตารางใช้ที่เท่าไหร่
`quantize_grouped` กับ `dequantize_grouped` ทำแบบสี่บิตที่มีไม้บรรทัดต่อกลุ่มเล็กๆ ซึ่งคือ
...
แปดบิตฟรี ไบต์ ลดลงเจ็ดเท่าและ loss ไม่ขยับในทศนิยมตำแหน่งที่สี่ นั่นคือเหตุผลที่ทุกการ
ให้บริการเริ่มที่ตรงนั้น และเหตุผลที่ตารางของบทให้บริการมีคอลัมน์ int8 เสมอ

สี่บิตไม่ฟรี และตารางแสดงการแลกทั้งสองทาง ไม้บรรทัดหนึ่งตัวต่อแถวลด ไบต์ ลงอีกครึ่งและมี
ราคา loss หกในพัน ไม้บรรทัดต่อกลุ่มสิบเจ็ดมีราคาสามในพัน แลกกับ ไบต์ ของไม้บรรทัดอีกร้อยสามสิบหก
บนตารางนี้ทั้งคู่ใช้ได้ บน model จริงสี่บิตที่มีไม้บรรทัดต่อแถวมักใช้ไม่ได้ และรูปแบบแบบกลุ่ม
มีอยู่เพราะตารางนี้เป๊ะๆ

สองบรรทัดสุดท้ายคือกลไก ตัวเลขที่ใหญ่ที่สุดในแถวกำหนดไม้บรรทัด และตัวเลขอื่นทุกตัวในแถวถูก
