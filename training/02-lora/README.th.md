...
สองวิธีบนข้อความใหม่ แล้วให้เห็นว่า LoRA ซื้ออะไรให้และไม่ซื้ออะไร ไฟล์นี้คือฉบับสั้นสำหรับ
รันโค้ด
...
`grid.py` คือ model ของบทที่ 4 ที่ยกเข้ามาในภาค 4 เพื่อให้ทุกการทดลองที่นี่ฝึกของชิ้นเดียวกัน
...
ตาราง attention และ feed-forward และ trl รัน loop บนไฟล์แชทที่บทที่ 1 เขียน ไม่ได้รันใน CI
...
อ่านตารางทีละคอลัมน์ คอลัมน์ข้อความใหม่คือสิ่งที่ fine-tuning มีไว้ทำ และ LoRA เข้าใกล้
full fine-tuning ภายในหนึ่งในห้าตรงนั้น โดยฝึก 164 ตัวเลขแทนที่จะเป็น 1681 อัตราส่วนนั้นคือ
...
ซึ่งคือสามเท่าของ model ก็หดตามไปด้วย
