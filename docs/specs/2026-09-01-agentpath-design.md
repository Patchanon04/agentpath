...
4. **ทุกบทจบด้วยของที่รันได้จริง** ไม่มีบทไหนจบครึ่งๆ กลางๆ และโฟลเดอร์บทหลังต้องสะสม
   ทุกอย่างที่บทก่อนสร้างไว้ ไม่ใช่ทิ้งไฟล์ที่เพิ่งสอนไปแล้ว เพราะการ diff สองบทติดกัน
   แล้วเห็นผู้เรียนลบไฟล์ที่ตัวเองเพิ่งเขียนคือการโกหกว่าอะไรขึ้นกับอะไร
...
- ข้อยกเว้นของกฎ colon คือเอกสารทำงานภายในใน `docs/plans/` และ `docs/specs/` ซึ่งเขียนเป็นภาษาไทยและไม่มีบทเรียนไหนลิงก์ไปหา `ci/prose_lint.py` บังคับกฎ colon เฉพาะไฟล์ที่ผู้เรียนอ่าน ส่วนกฎห้าม em-dash และ emoji บังคับทุกไฟล์
...
│   └── ...
├── src/agentpath/          framework ตัวเต็ม (pip install agentpath-kit)
├── tests/                  test ของ framework ตัวเต็ม
├── ci/                     script รันทุก lesson + prose lint (อยู่นอก lesson folders)
│                           ส่วน mock server อยู่ใน src/agentpath/testing/ เพราะผู้ใช้ package
│                           ต้องใช้มันทดสอบ agent ของตัวเองได้ด้วย ไม่ใช่ของ CI อย่างเดียว
├── docs/                   specs, plans, v2-ideas
...
นอกจากสามตัวนั้นยังมีอีกสามตัวที่ไม่ใช่ค่าเชื่อมต่อ ประกาศไว้ตรงนี้เพื่อไม่ให้หลุดจากสายตา
`AGENTPATH_AUTO_APPROVE` สำหรับการรันที่ไม่มีคนอยู่หน้าจอ `AGENTPATH_WORKSPACE` ที่โค้ดบทเรียน
ใช้บอกว่า agent ทำงานในโฟลเดอร์ไหน และ `AGENTPATH_HOME` ที่บอกว่า session เก็บที่ไหน สองตัวหลัง
มีอยู่เพื่อให้ check.py ชี้ไปที่โฟลเดอร์ชั่วคราวได้ ซึ่งเป็นสิ่งที่ทำให้ CI รันได้โดยไม่แตะเครื่องของใคร
...
รายการนี้คือจุดที่ tutorial สอน agent เกือบทั้งหมดข้ามไป แต่คนที่สร้าง harness จริง
เจอทุกคน ทุกข้อต้องปรากฏในบทที่ระบุ ไม่ใช่ในรูปเชิงอรรถ แต่เป็นหัวข้อที่มีโค้ดและ
มีการทดลองให้เห็นว่าพังยังไง หลายข้อมาจากการสำรวจ issue จริงของ harness ที่คนใช้กันอยู่

**บท 03 tool calling**
- tool calling คือกลไกเดียวกับที่ใช้บังคับให้ model ตอบเป็น JSON ตามโครงสร้าง คนมักคิดว่า
  เป็นคนละเรื่อง

**บท 04 agent loop**
- max_turns ไม่ได้แก้ปัญหา doom loop จริง agent วนเรียก tool เดิมด้วย argument ที่ต่างกัน
  นิดเดียวจนงบหมดได้ ทางแก้จริงคือ fingerprint การเรียกซ้ำและตรวจว่าไม่มีความคืบหน้า
  ซึ่งลงมือทำในบท 17
- ทำไม v1 รัน tool ทีละตัวแทนที่จะขนาน

**บท 05 streaming**
- ถ้า model ชนเพดาน output กลาง tool call ตัว JSON ของ argument จะขาดกลางคัน harness
  จำนวนมากเช็ค finish_reason เฉพาะตอนไม่มี tool call เลยเอา JSON ขาดๆ ไปรันต่อ
- JSON ที่ parse ไม่ผ่านห้ามแปลงเป็น dict ว่างเงียบๆ เพราะ model จะไม่มีทางรู้ว่าตัวเอง
  ทำพลาดและจะทำซ้ำทุกครั้งที่โหลด history ต้องส่ง error กลับไปให้ model แก้เอง

**บท 06 provider abstraction**
- thinking หรือ reasoning block ต้องส่งกลับใน history แบบไม่แตะต้องเลย รวมถึง field
  signature ที่ดูเหมือน metadata ไร้ประโยชน์ ถ้าแก้หรือตัดทิ้งแล้วมี tool use อยู่ด้วย
  API จะปฏิเสธ และการเปลี่ยนงบ thinking กลางเซสชันทำให้ prompt cache พัง

**บท 07 file tools**
- agent อ่านไฟล์ .env แล้วกุญแจไหลเข้า context และถูกส่งไปให้ผู้ให้บริการทุกรอบถัดไป
  ต้องมีรายการไฟล์ที่ปฏิเสธการอ่านตั้งแต่ต้น

**บท 08 shell tool**
- ความต่างของ Windows ที่ทำให้ tool พัง path separator, shell ที่ไม่ใช่ bash,
  และ encoding ของ output กลุ่มเป้าหมายของโครงการนี้ใช้ Windows เยอะ

**บท 12 permission system**
- prompt injection เข้ามาทางผลลัพธ์ของ tool ไม่ใช่แค่ทาง shell ไฟล์ที่ agent อ่านมา
  อาจมีข้อความสั่งให้ทำอย่างอื่น
- id ของ tool call ตอน stream อาจไม่ตรงกับ id ในข้อความสุดท้ายของบาง provider ทำให้
  ระบบขออนุญาตจับคู่คำขอกับการรันไม่เจอ

**บท 13 sessions**
- ไฟล์ session คือเครื่องมือ debug อันดับหนึ่ง เวลาสงสัยว่าทำไม agent ทำแบบนั้นให้เปิดอ่าน
- เปิด session เดียวกันจากสองหน้าต่างพร้อมกันทำให้ไฟล์พัง v1 ประกาศตรงๆ ว่ารองรับ
  ผู้เขียนคนเดียว และอธิบายว่า harness จริงแก้ด้วยการล็อกอย่างไร

**บท 14 context management**
- นี่คือกับดักใหญ่ที่สุดของบทนี้ ถ้าตัด history ตรงกลางระหว่าง tool call กับผลลัพธ์ของมัน
  คำขอครั้งถัดไปจะโดน API ปฏิเสธด้วย 400 ทันที เพราะทุก tool call ต้องมีผลลัพธ์คู่กันเสมอ
  ดังนั้นการตัดต้องมองคู่ tool call กับ tool result เป็นก้อนเดียวที่เก็บทั้งคู่หรือทิ้งทั้งคู่

**บท 15 token economy**
- ของที่เปลี่ยนทุกครั้ง เช่น เวลา ชื่อผู้ใช้ หรือ session id ถ้าวางไว้ก่อนจุด cache
  จะทำให้ cache ทั้งก้อนพังทุกคำขอ การสลับลำดับ tool definition ก็ให้ผลเดียวกัน
  อาการคือค่าใช้จ่ายพุ่งโดยไม่มีอะไร error ให้เห็น
- ตัวนับ token ของแต่ละเจ้าไม่เท่ากัน ใช้ tiktoken ประเมิน token ของ Claude แล้วเอาไป
  ตัดสินใจตัด context คือการตัดสินใจบนตัวเลขที่ผิด

**บท 17 errors & retries**
- การขัดจังหวะ agent กลางคันต้องหยุดของจริงทุกชั้นพร้อมกัน ปิด stream ที่ค้างอยู่
  ฆ่า subprocess ที่ spawn ไว้ และปลดการรอคำตอบ permission harness จริงเคยมี bug
  ที่หน้าจอบอกว่าหยุดแล้วแต่ tool ยังทำงานต่อ
- retry tool ที่มี side effect เท่ากับทำซ้ำ ส่งอีเมลสองฉบับ ตัดเงินสองครั้ง ต้องมี
  idempotency key ไม่ใช่แค่ retry ตัวเรียก LLM
- backoff ต้องเคารพ header ที่ผู้ให้บริการส่งมาก่อนสูตรของเราเอง และต้องมี jitter
  ไม่งั้นตอนบริการล่มเราจะกลายเป็นคนซ้ำเติม

**บท 19 MCP client**
- ต่อ MCP หลายตัวพร้อมกัน schema ของ tool กินได้ครึ่งหนึ่งของ context ก่อนเริ่มงานจริง
  และทำให้ model เลือก tool ผิดบ่อยขึ้นด้วย ไม่ใช่แค่เปลืองอย่างเดียว

**บท 20 และ 21 subagents**
- แม่กับลูกมองเห็น state คนละเวอร์ชัน ลูกแก้ไฟล์ไปแล้วแต่แม่ยังตัดสินใจจากของเก่า
  การแยก context ให้ลูกซึ่งทำเพื่อความคมชัดกลับทำให้ปัญหานี้หนักขึ้น

### กฎแช่แข็งหลักสูตร

รายชื่อบททั้ง 24 บทข้างบนนี้คือเพดาน ห้ามเพิ่มบทใหม่เข้าหลักสูตรอีก ไอเดียหัวข้อใหม่
ที่โผล่มาระหว่างทางให้ไปอยู่ในไฟล์ `docs/v2-ideas.md` เท่านั้น

เหตุผลคือหลักการข้อ 3 ที่บอกว่าทุก feature ต้องตอบได้ว่าสอนอะไร ป้องกันเรื่องนี้ไม่ได้จริง
เพราะไอเดียดีๆ ทุกอันตอบคำถามนั้นได้เสมอ สิ่งที่ฆ่าโปรเจกต์สอนไม่ใช่ไอเดียแย่ แต่คือ
ไอเดียดีที่เพิ่มเข้ามาเรื่อยๆ จนไม่มีวันเขียนจบ 24 บทคูณสองภาษาคือเพดานที่ยอมรับได้แล้ว

การเพิ่มหัวข้อย่อยเข้าบทที่มีอยู่ทำได้ การเพิ่มบทใหม่ทำไม่ได้ ถ้าจะเพิ่มบทจริงๆ
ต้องตัดบทเดิมออกหนึ่งบทเสมอ

**ข้อยกเว้นเดียว ตัดสินใจ 2 กันยายน 2026** มีภาคพื้นฐานเจ็ดบทอยู่นอกหลักสูตร 24 บท
ในโฟลเดอร์ `foundations/` และในหนังสือเป็นภาค 0 มันมีไว้สำหรับคนที่ไม่รู้ว่า token คืออะไร
ก่อนจะเริ่มบทที่ 01 ไม่มี API ไม่มี agent เป็น Python ล้วนกับ numpy เหตุผลที่มันไม่ผิดกฎ
คือกฎมีไว้กันไอเดียดีที่เพิ่มเข้ามาเรื่อยๆ จนเขียนไม่จบ ส่วนภาคพื้นฐานคือของที่อยู่ก่อน
บทที่ 01 ไม่ใช่ของที่อยู่หลังบทที่ 23 และมันถูกตรึงไว้ที่เจ็ดบทด้วยกฎเดียวกัน

**ข้อยกเว้นที่สอง ตัดสินใจ 3 กันยายน 2026** มีภาคฝึก model ห้าบทอยู่หลังบทที่ 23 ในโฟลเดอร์
`training/` และในหนังสือเป็นภาค 4 มันสอนตัว model ไม่ใช่ harness จึงไม่ใช่บทที่ 25 และถูก
ตรึงไว้ที่ห้าบทด้วยกฎเดียวกัน
